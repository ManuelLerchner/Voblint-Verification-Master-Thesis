#import "theme.typ": vb
#import "code.typ": decode-isabelle, isabelle-font, isabelle-syntax
...
#let stmt(name) = {
  assert(
    ...
    message: "no exported statement for `"
      + name
      + "` -- add it to "
      + "thesis/shared/facts.toml and run thesis/tools/facts.py --write",
  )
  _frame(text(font: isabelle-font, raw(
    decode-isabelle(_facts.facts.at(name).statement),
    lang: "isabelle",
    block: true,
    syntaxes: isabelle-syntax,
  )))
...
#let proved(name, note: none) = {
  block(above: 1.1em, below: 1.1em, {
    ...
    stmt(name)
    v(0.25em)
    align(right, text(size: 0.75em, font: "DejaVu Sans Mono", fill: vb.proved, name))
  })
}
