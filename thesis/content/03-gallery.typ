...
This chapter is a working reference outside the thesis content: one instance of
...
#block(
  ...
  *Every payload in this chapter is a placeholder.* The figure _kinds_ are
  ...
  `Called`/`Resumed` rather than `Call`/`Resume`; @tab:oracles has a `sorry`
  column, though the development contains none; and @tab:size and @fig:eval
  carry invented counts.
...
#figure(
  ...
  caption: [The session graph of #isasession("Voblint_CLI"), from Pure down to
    ...
    version it replaces named five theories that no longer exist. The generated
    graph lists only theories the build loads.],
) <fig:modules>
...
#figure(
  ...
  caption: [Output of #isacmd("thm_oracles") for the endpoint theorems,
    ...
    rest on no oracle and no admitted subgoal.],
) <tab:oracles>
...
leaves the region where $f$ can keep climbing.
...
@fig:simulation draws a lemma relating two levels of a pipeline as squares,
and the lemma states that they commute.
...
#subfigures(
  ...
  columns: (1fr, 1fr),
  align: bottom,
  caption: [Compiler correctness as two simulations. Correctness needs both
    directions.],
...
#figure(
  symbol-table((
    ...
    read("/shared/generated/snippets/analysis_contract.thy"),
...
@fig:claim applies the same idea to prose. The listing is the output of a
recorded command, regenerated and diffed by
...
#figure(
  table(
    ...
    [#isasession("Voblint_Domain")], [31], [18 400], [690],
    [#isasession("Voblint_Analysis_Int")], [28], [15 100], [520],
    [#isasession("Voblint_Result")], [9], [4 300], [130],
    table.hline(stroke: 0.5pt),
