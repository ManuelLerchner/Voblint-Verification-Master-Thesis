...
def ask_isabelle(
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        # CI exposes Isabelle through ISABELLE without adding its bin
        # directory to PATH; local installs commonly provide the command.
        ...
        for extra in (os.environ.get("AFP"), REPO / "vendor" / "td-verification"):
            if extra and Path(extra).is_dir():
                cmd += ["-d", str(extra)]
        proc = subprocess.run(
            cmd, input=f'use "{script}";\n', capture_output=True, text=True, cwd=REPO
        )
        if not outfile.is_file():
            sys.exit(
                f"facts: Isabelle produced nothing for session {session}.\n"
                f"Build it first: pixi run isabelle-build\n{proc.stdout[-2000:]}"
                f"{proc.stderr[-2000:]}"
            )
        result = {}
...
def main() -> int:
    ...
    if unresolved:
        ...
        for n in unresolved:
            print(
                f"  {n} -- expected in {facts[n].get('session', '(unknown session)')}"
            )
    ...
    if stored != payload:
        ...
        print(
            "If the new statement is correct, run thesis/tools/facts.py --write "
