...
let node_attrs (n : G.node) =
  ...
  | Some G.Proved, _ ->
      boxed ^ ",color=darkgreen,penwidth=2,fillcolor=palegreen"
  | Some G.Refuted, _ ->
      boxed ^ ",color=firebrick,penwidth=2,fillcolor=mistyrose"
  | Some G.Unknown, _ ->
      boxed ^ ",color=darkgoldenrod,penwidth=2,fillcolor=lightgoldenrod1"
...
let edge_attrs (e : G.edge) =
  ...
  | G.Enter ->
      Printf.sprintf
        "color=purple,penwidth=2,weight=6,minlen=2,label=\"call %s\"" text
  | G.Combine ->
        ...
        (fun id ->
          ...
          Buffer.add_string buf
            (Printf.sprintf "    %s [%s,label=%s,tooltip=%s];\n" id
               (node_attrs n) (node_label n) (node_tooltip n)))
        c.members;
      Buffer.add_string buf "  }\n")
    graph.clusters;
  List.iter
