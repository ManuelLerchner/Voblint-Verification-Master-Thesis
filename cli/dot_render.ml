(* GraphViz rendering of a solved analysis graph.
   ...
   Draws the Analysis_graph built from a run result. Presentation is deliberately
   ...
   is this file's own. Source text is presented separately by frontends that need it.
...
let style_of_status = function
  | G.Proved -> boxed ^ ",color=darkgreen,penwidth=2,fillcolor=palegreen"
  | G.Refuted -> boxed ^ ",color=firebrick,penwidth=2,fillcolor=mistyrose"
  | G.Unknown -> boxed ^ ",color=darkgoldenrod,penwidth=2,fillcolor=lightgoldenrod1"
  | G.Unreachable ->
      "shape=box,style=\"rounded,dashed,filled\",color=gray45,fillcolor=gray90,fontcolor=gray35"

let structural_attrs (n : G.node) =
  match n.kind with
  | G.Program_entry | G.Proc_entry | G.Program_exit | G.Proc_exit ->
      "shape=doublecircle,color=orange,style=filled,fillcolor=lightyellow"
  | G.Point -> "shape=box,style=filled,fillcolor=lightgreen"

(* Only a semantic finding overrides a node's structural appearance. *)
let node_attrs (n : G.node) =
  match n.status with None -> structural_attrs n | Some s -> style_of_status s
