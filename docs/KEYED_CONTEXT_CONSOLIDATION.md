...
| `Exec_Sign_Cmp_Keyed_Run` | Hand-built concrete keyed post-solution witness for sign. | `TD_Side_Eff_Cmp_Sound`, `Sign_Domain`, bool contexts. | `LOCAL_POST_inst`, `CMP_SOUND_inst`, `comb_bound`, read precision lemmas. | `Exec_Sign_Cmp_Keyed_Solve`, roadmap/design docs. |
| `Exec_Sign_Cmp_Keyed_Solve` | Runs the minimal keyed side solver abstraction. | `Exec_Sign_Cmp_Keyed_Run`, `TD_side_always_join_Interp_solve`. | Solver-computed separated slots and `keyed_slots_strictly_separate`. | Example/design documentation. |
...
Examples
  ...
  Exec_Sign_Cmp_Keyed_Run
  Exec_Sign_Cmp_Keyed_Solve
...
| `Exec_Sign_Cmp_Keyed_Run` | Hand-built keyed post-solution discharges `LOCAL_POST`, `CMP_SOUND`, and combine bound. | Semantic proof witness. | Abstract sign states. | Yes, read precision and context separation. | Shows premise-shape obligations for `post_fixpoint_sound_at_ctx_semantic_cmp_final`. | Hand-authored environment, not solver-produced. |
| `Exec_Sign_Cmp_Keyed_Solve` | Real TD side solver computes separated keyed slots for a minimal abstraction. | Executable. | Concrete executable `sign st`. | Yes, `SZero`/`SPos` slots versus `SNonNeg` join. | Does not itself prove CFG collecting soundness. | Small synthetic equation system, not compiled CFG. |
