...
| A1 | Bundled `run_analysis_source_sound` (hide the 4–5-step manual chain) | PLANNED | Near-term | none (bundles existing partial-correctness chain) | One named theorem/locale-export taking `(Π, ps, main, s0, v)` + `solve_c … = Some σ` and concluding source-level soundness at `v`, with **no** example re-composing `partial_post_solution` → `part_post_solution_dg_st_to_abs` → collect-sound → `source_activation_sound` | thesis |
| A2 | Domain-registration API (essential obligations in, plumbing derived) | PLANNED | Framework validation | A1 (shares the export shape) | A locale/bundle whose inputs are exactly `{lattice instance, gamma+bot/mono, transfer fns + sound_transfer + tf_mono (+ D/G step for D/G)}` and whose outputs derive `sound_effectful_transfer`, edge/combine-tree lemmas, `cone_compatible_etf`, `threefold_mono`, the mono solver-interface lemmas, and `side_X_analysis_sound` — with `strategy_tree` / `ltree` / `gtree` / `Inl-Inr` **absent from the instance file** | publication |
...
- **"the API is already minimal and fully packaged"** — the obligation set is near-minimal
  (`DOMAIN_INTERFACE_MINIMIZATION.md`), but packaging is open: `strategy_tree` / `ltree` / `gtree` /
  `Inl-Inr` still leak into instance lemma statements. Tracked as A2.
