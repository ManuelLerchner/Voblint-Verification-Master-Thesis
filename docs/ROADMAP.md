...
| Per-stage workflow | `docs/PIPELINE_WALKTHROUGH.md` |
...
`docs/PROOF_OVERVIEW.md` and `docs/PIPELINE_WALKTHROUGH.md` are the prose-level pipeline-narrative sources. The thesis chapter lifts from them; cross-references to `.thy` files are by file path, not by lemma name (those drift; `rg` finds them).
