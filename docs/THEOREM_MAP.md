...
| Successful verified TD solving yields a post-solution | [`Voblint_Solver.TD_Solver_Bridge:TD_side_upd_rule.part_post_solution_of_solve_c`](../src/Abstract_Interpreter/Solver/TD_Solver_Bridge.thy) | Bridges executable `solve_c` success to the vendored solver's `part_post_solution`. The domain registrations do not go through it: they discharge their post-solution obligation with the vendored `TD_side_rule_Interp.partial_post_solution` directly. |
