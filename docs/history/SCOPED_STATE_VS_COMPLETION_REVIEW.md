...
Status: historical. `Example_Interval_Placement.thy` and every lemma cited below
(`placement_*`, `dg_refines_on`, `complete_abs_on`, `hook_post_solution_collect_sound_ltr`) were
deleted in `c69881c5`; the total `abs_state` convention it evaluates still holds.
