...
Status: **historical.** `side_cfg_T_eff_cmp`, `side_env_cmp`, the framed-enter
contract and `EA_Enter` are deleted. The keyed generator is now
`routed_node_rhs`
(`src/Abstract_Interpreter/Framework/Constraints/DG_Keyed_Generator.thy`), and
calls live in the CFG's separate `calls` relation.

Original status: reviewed 2026-07-01. This pass adds no functionality and weakens no
...
- `docs/history/GLOBAL_CONTEXT_REDESIGN.md`: mostly historical, but its status header
