...
> Status: historical. The "this repository" side describes the earlier flat-CFG IMP2
> pipeline: `cfg_edges`, `cfg_collect`, `pipeline_invariant_sound`, `td_analyse` and the
> file paths below no longer exist. The current pipeline (VIMP, procedure-aware CFG,
> `valid_ltr`, D/G) is described in `docs/PROOF_OVERVIEW.md`; the HOL-IMP side is unaffected.
