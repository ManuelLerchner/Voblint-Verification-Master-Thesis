...
> **Status: superseded.** `Example_Interval_Placement.thy` was deleted in
> `c69881c5`, together with the hook/placement API this page walks through
> (`sound_dg_hooks`, `hook_gen`, `placed_dg_gen_of_strict`, `placement_*`,
> `dg_refines_on`, `se_constraint_holds`); most names below no longer exist.
> The current end-to-end walkthrough is `src/Examples/Parity/README.md` (one
> program from VIMP source to `parity_source_run_sound`);
> `Example_Interval_DG_Flagship.thy` is the Interval counterpart, registered
> through `Run_Analysis_Sound`'s `ownership_split_dg_exec_analysis`.
