...
> Historical. The seeded-activation spine discussed here (`seeded_activation_collecting_sound`,
> `restrict_global`) no longer exists; the current foundation is
...
1. **Concrete callee entry has locals 0.**
   ...
   (`src/Program_Model/VIMP/VIMP_Globals.thy:36`). Every local is 0.
