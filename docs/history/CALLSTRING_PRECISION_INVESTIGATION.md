...
Investigation only; no code changed. Question: what is the proof shape for a
...
Closed. The solver-level refinement this investigation converged on in
section 10 (`project_sigma_part_post_solution`) and the trace-level
inclusion of row 2 (`call_string_collecting_mono`) were proved at the time
and are no longer in the tree. What the tree keeps is the strictness
witness: `nest_k2_strictly_more_precise_than_k1`
(`Example_Interval_DG_CallString_K2`) and
`cs_generic_k2_strictly_more_precise_than_k1`
(`Example_Interval_Call_String_Generic_Parity`). No general `k1 <= k2`
refinement theorem exists. The sections below record the investigation
against the tree of the time; their theory, constant and line citations are
not maintained. `history/CALLSTRING_PROJECT_SIGMA_GENERALIZATION_DESIGN.md`
works out a general construction over arbitrary dependency graphs.
...
2. An idempotence fact, checked algebraically, not in
   `Call_String_Context.thy`:
...
corollary of what already exists.
...
| --- | --- | --- |
| 1 | Context projection (`cs_project`/commuting diagram) | done by hand (section 1), two-line corollary; not in the tree |
| 2 | Trace-set inclusion (`activation_collect` at k2 vs k1) | proved at the time as `call_string_collecting_mono`; not in the tree |
...
ingredient for this route and should not be forced into it.
