...
Status: closed. Every theory it names (`Exec_Bridge`, `Digest_Global_Read`, the `TD_Side_Eff_*`
family, ...) has since been deleted or renamed, and the `docs/history/` move that item L.3
declined has happened.
...
| ------- | ------------- | ------ | -------------- |
| **Named-global sign** | `Instances/NamedGlobalSign/Sign_Named_Global_Eff` (653) | superseded (2026-08): now imported by the `Voblint` capstone (`Voblint_Analysis.ln`); README updated to "Executable + sound through the solver on the constant route" | Retire recommendation no longer applies — the project kept and continued this file past this audit. Remaining open work is scoped narrowly: instantiate the already-generic `part_post_solution_st_to_abs_eff` at `'g = gname` and add one `value`-evaluated example. |
