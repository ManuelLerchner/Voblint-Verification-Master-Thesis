...
Status: executed. The audit it requests is `docs/history/DEADCODE_REACHABILITY_ARCHITECTURE_AUDIT.md`;
an unreachable point is now the explicit `Bot` of `lifted`
(`src/Abstract_Interpreter/Domain/Reachability_Lift.thy`), and the `etf` theorems cited below
(`sign_tf_local_edge_invariant`, `sign_sound_etf`) no longer exist.
