...
- call entry must use `edge_step (EA_Enter …)` and retain `bind_formals`. `EA_Enter` has since
  left `edge_action`: calls travel the separate `calls` relation, and `call_enter`
  (`CFG_Transfer`) is what applies `bind_formals`.
