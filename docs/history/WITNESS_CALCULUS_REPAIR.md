...
Status: superseded. [`ACTIVATION_SPINE_CONSOLIDATION.md`](ACTIVATION_SPINE_CONSOLIDATION.md)
deleted `twf`/`twfr`, `Activation_Witness_From` and `Twfr_Reach_Read`; the concrete foundation is
now `valid_ltr`, and `trace_witness`, `trace_witness_act` and `cfg_collect` no longer exist.
