...
the `context_domain` locale and the `fctx` witness discussed here no longer
exist, and neither does `TD_Side_Eff_Ctx_Shared`. Contexts are now
`routed_dg_analysis` registrations at the call-string and entry-state routes
(`Call_String_Context`, `Routed_Context`). The migration documents cited below
