...
Historical note: the `TD_Side_Eff_Ctx_Sound` / `side_env_ctx` path, `Context_Domain`,
the `fctx` witness and `Example_Finite_Sign_Context_Analysis` discussed here no
longer exist, and neither does `TD_Side_Eff_Ctx_Shared`. Contexts are now
`routed_dg_analysis` registrations at the call-string and entry-state routes
(`Call_String_Context`, `Routed_Context`). Value-dependent entry-state contexts
are live through that construction (`exec_formals_route`,
`entry_state_activation_collect_sound`), not through route C's entry-store spine.
