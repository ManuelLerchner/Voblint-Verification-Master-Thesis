...
> **Status:** historical. `make_side_rhs_tree` / `make_side_rhs_tree_ip` and
> both side stacks are deleted. The current generator `routed_node_rhs`
> (`src/Abstract_Interpreter/Framework/Constraints/DG_Keyed_Generator.thy`)
> publishes the initial global value `s0g` at the CFG entry.

Original status: **complete (E1–E4).** The side-effecting analysis is sound from an
