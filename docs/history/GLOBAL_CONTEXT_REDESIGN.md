...
Historical note: the `side_cfg_T_eff_cmp` generator, the `TD_Side_Eff_Ctx_Sound` / `side_env_ctx` path and `TD_Side_Eff_Ctx_Shared` discussed in this analysis are deleted. The keyed generator is now `routed_node_rhs` (`src/Abstract_Interpreter/Framework/Constraints/DG_Keyed_Generator.thy`), with callee seeds published by `routed_call_tree` (`src/Abstract_Interpreter/Framework/Context/Routed_Call_Trees.thy`).
