...
Historical note: the `TD_Side_Eff_Ctx_Sound` / `side_env_ctx` spine, `DG_Soundness` and `TD_Side_Eff_Ctx_Shared` discussed in this report are deleted. Keyed context soundness on the DG carrier is now the `routed_context_base_hetero` locale (`src/Abstract_Interpreter/Framework/Context/Routed_Context.thy`) over the generator `routed_node_rhs` (`DG_Keyed_Generator.thy`).
