...
> Status: historical. The triple-set choice survives as `intra` in `CFG_Def.thy`; calls
> moved to a separate four-place `calls` relation. `cfg_edges`, `cfg_path` and the
> `src/Analysis/...` paths below no longer exist; reachability is `cfg_reaches` in
> `CFG_Prune.thy`.
