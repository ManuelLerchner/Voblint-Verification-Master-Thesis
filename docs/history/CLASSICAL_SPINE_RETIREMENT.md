...
Status: superseded by [`IP_ONLY_CONSOLIDATION.md`](IP_ONLY_CONSOLIDATION.md), which deleted the
remaining intra spine and renamed `pcom` to `com`. `cfg_collect` and `cfg_collect_ip` no longer
exist; the collecting semantics is `ltr_collect` over `valid_ltr`.
