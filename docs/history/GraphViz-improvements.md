...
> Superseded. `CFG_GraphViz.thy` no longer exists: the graph layer in
> `src/Executable_Surface/CLI/` (`Analysis_Graph*.thy`) builds one cluster per
> procedure, the export hands clusters to OCaml (`xc_id`/`xc_label`/`xc_nodes`),
> and `cli/dot_render.ml` draws them. What follows is the pre-implementation
> design note.
