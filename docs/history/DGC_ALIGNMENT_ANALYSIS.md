...
Historical note: the `TD_Side_Eff_Ctx_Sound` / `side_env_ctx` path discussed in
this analysis has been deleted, and so have the theories it cites
(`Context_Domain`, `TD_Side_CFG`, `TD_Side_Eff_Cmp_Sound`, `Global_Cmp_Read`,
`Exec_Cmp_Bridge`, `Example_Finite_Sign_Context_Analysis`) and
`TD_Side_Eff_Ctx_Shared`. Line numbers below refer to those removed files. Contexts
are now `routed_dg_analysis` registrations at the call-string and entry-state
routes (`Call_String_Context`, `Routed_Context`).
