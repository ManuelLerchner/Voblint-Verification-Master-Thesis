...
Status: superseded. `Local_DG`, `clean_ctx_collect_rread` and the `Voblint_Analysis` session no
longer exist. Every analysis is a D/G instance discharging `sound_dg_spec_core`, and the
context-insensitive run is the routed pipeline at the unit context (`Unit_DG_Analysis`).

> **Status at the time:** probe DELIVERED, batch-green (`Voblint_Analysis` +
