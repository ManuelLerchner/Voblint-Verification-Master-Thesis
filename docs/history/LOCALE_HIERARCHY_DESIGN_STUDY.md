...
Status: **historical.** Every generator and locale counted below
(`side_cfg_T_eff*`, `context_domain`, `mixed_rhs_generator`, `sound_transfer`)
is deleted. The one generator is now `routed_node_rhs`
(`src/Abstract_Interpreter/Framework/Constraints/DG_Keyed_Generator.thy`), and
context soundness is the `routed_context_base_hetero` locale
(`src/Abstract_Interpreter/Framework/Context/Routed_Context.thy`).

Original status: design study, nothing implemented. Verdict below is evidence-driven
