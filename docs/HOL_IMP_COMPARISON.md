...
| **Collecting semantics**       | `CS c = lfp c (step UNIV)` on **annotated commands**; `step` pushes **store sets** through the tree | `collect c S` on AST; **`cfg_collect`** on CFG program points; bridge **`cfg_collect_exit_eq_collect`** |
...
| **Main soundness theorem**     | `AI_correct`: `AI c = Some C ⟹ CS c ≤ γ_c C`                                                        | **`pipeline_invariant_sound`** / **`pipeline_sound`**: `cfg_collect ⊆ γ ∘ env` (+ `big_step` for exit)  |
| **Solver**                     | Built-in **`pfp`** / `while_option` in Isabelle                                                     | **Separate verified session** (`TD`); we prove `td_analyse_post_fixpoint`                               |
| **CFG / equation system**      | None : control flow is implicit in recursive `Step` / `step'`                                       | **Central** : matches Goblint compile → eqsys → solve. Locally interprets AFP `Dijkstra_Shortest_Path.Graph` (labelled triples + `valid_graph`) and `Timed_Automata.Graphs.Graph_Defs` (label-erased reachability); see `docs/AFP_GRAPH_MIGRATION.md` |
