...
Status: **landed, apart from steps 3.3, 3.4 and 3.7.** The execution sequence for
splitting and cleaning `Voblint_Framework`. The survey, target layout and graph
below describe the tree when the plan was written; the `ROOT` files and
`AGENTS.md` hold the result. Edit it as phases land:
...
`Voblint_Framework` is 52 theories and 25,407 lines in three folders. The folders
...
(`DG_Constraint_Programs` 2472, `DG_Soundness` 2317, `Exec_St` 2231,
...
1. **The abstract framework** -- what a sound analysis is and why a solved
   system covers the collecting semantics. Carrier-agnostic: `analysis_contract`,
...
2. **The executable transport** -- 3,700 lines (`Exec_DG_Trees`,
   ...
   `DG_Local_State_Exec` and `Routed_Domain_Exec`, the owner-aware trees in
...
| `DG_Constraint_Programs -> Exec_Placement` | nothing | drop |
...
- `td_cfg_side_solver_dg` with `cfg_pkg_dg`, `stabl_at`, `nu_at`
  (`DG_Constraint_Programs` 2365-2470): no user outside the file.
- `sound_dg_hooks`, `sound_dg_hooks_ltr`, `hook_gen`, `gamma_join`,
  ...
  `Example_Interval_Placement`. The spec route (`analysis_contract`, interpreted
  12 times) is what Analysis and Soundness use.
- `Exec_Placement` (1134 lines): imported by `DG_Constraint_Programs`, which uses nothing
  ...
  algebra.
- Fourteen of `DG_Constraint_Programs`'s 45 definitions and nine of `DG_Soundness`'s
...
- `routed_dg_domain_exec` (`DG_Local_State_Exec` 168) is the whole per-domain
...
`DG_Constraint_Programs` and `DG_Soundness`, 10 `Activation_Local_Sound`), four
...
`routed_node_rhs_buffered_correspondence` (249).
...
`Voblint_Framework` is compiler-free for the same reason `Voblint_CFG` is: the
...
| 0.2 | Move `bounded_widening`, `bounded_narrowing`, `bounded_warrowing` and `instance lifted :: bounded_warrowing` from `Exec_St`'s preamble into `Abstract_Domain`. Drop `DG_Constraint_Programs`'s `Exec_St` and `Exec_Placement` imports. | landed |
...
- 2026-08-30: `DG_Coverage` imports `Exec_DG_Generator`, so it is in
  `Voblint_Exec`, not `Voblint_Framework` (the target table said Core).
...
- 2026-08-30: **`Exec_Placement` is not example-only.** The survey measured
  it only against `DG_Constraint_Programs` (zero names) and counted three Example
...
- 2026-08-30: `Routed_Context` cited `Voblint_Compile.VIMP_Proc_to_CFG` and
  ...
  in its import list.
- 2026-08-30: The hooks route is not example-only either: `analysis_contract`
  is proved by reduction to `sound_dg_hooks` (`sublocale analysis_contract
...
- 2026-08-30: **Spike 2.1 succeeds.** `Spike_Sign_Quotient.thy` proves a
  generic pullback inside `routed_dg_domain_exec` -- `analysis_contract` at the
  abstract Base spec gives `analysis_contract` at the executable Base spec with
  `gamma_exec d g = gamma_dg_local_state (readback d) (readback g)`, from the three
  commute facts alone -- and then interprets `analysis_contract`,
...
- 2026-08-30: The two generic halves of the spike are now framework facts:
  `routed_dg_domain_exec.analysis_contract_st` with `gamma_exec` (DG_Local_State_Exec)
  ...
  carrier interprets `analysis_contract` by `analysis_contract_st`, feeds
...
- 2026-08-30: `dg_analysis_adapter` is carrier-generic (step 2.5, done
  ...
  Int at `unit` context) pass `gamma_dg_local_state`, `gamma_state_lift` and `id`;
...
- 2026-08-30: Sign at unit context is the first instance on the executable
  ...
  `sign_conf_sound_exec` by `analysis_contract_st` and `sign_conf_pp_routed` by `pp_st`,
  ...
  with `*_pp_routed`, re-interpret `analysis_contract` and the routed locale at
...
- 2026-08-30: the four CallString examples solve the unbuffered
  `routed_node_rhs` at the executable spec already, so their
  migration is only the interpretation: `analysis_contract` at `*_S_st` via
  `routed_dg_domain_exec.analysis_contract_st`, `sigma := snd *_sol`, the covered
...
- 2026-08-30: what still keeps the transport theories alive is the flat
  ...
  (`analysis_contract_st` for the flat spec, the collecting endpoint fed by the
...
- 2026-08-30 (superseded by the entry above): the six contextual instances cannot follow the recipe yet.
  `pure_entry_routed_context` and `call_string_routed_context` are stated
  over `'a abs_state lifted` with `gamma_dg_local_state`/`gamma_state_lift` baked
...
- 2026-08-30: Core's layout inside `src/Abstract_Interpreter/Framework/` is `Equations/`, `DG/`,
...
- 2026-08-31: the flat bundle restatement from the entry above is done.
  ...
  soundness via `routed_dg_domain_exec.analysis_contract_st` directly at the
  ...
  gets the same treatment via a new, file-local `analysis_contract_st` built from
...
- 2026-08-31: **step 2.3 is not safely executable yet**, despite the above
  ...
  - `Routed_Domain_Exec` is load-bearing, not deletable: `routed_domain_exec`
    ...
    `.pp_st`/`.analysis_contract_st`. This was already known generically ("the
  ...
  (transport theories / `DG_Local_State_Exec`'s transport half / `Exec_DG_Refines`'s
...
- 2026-08-31: the precondition above is closed, and `Exec_DG_Bridge.thy` is
  ...
  `Example_Sign_DG_Custom_Combine`, not a migration. `DG_Local_State_Exec.thy`'s own
  ...
  from `src/Abstract_Interpreter/Exec/ROOT` and disk; `src/Executable_Surface/CLI/Analyse_Dispatch.thy`'s
...
- 2026-09-01: the batch build the deletion above claimed as verification
  ...
  `parity_pp_st_gen` all cite it. `src/Abstract_Interpreter/Exec/README.md`'s `Routed_Domain_Exec`
  ...
  *import*; a direct citation of one of Bridge's twelve own names inside a
  file that imports something else entirely (here, `DG_Local_State_Exec`) still
...
- 2026-09-01 (superseded by the merge/split entry below): scoped what step
  ...
  flagships interpret `analysis_contract` and get `routed_dg_domain_exec`'s
  generic `analysis_contract_st` pullback; the Placement examples interpret
  ...
  ("One `dead_code_lift` ... `analysis_contract S ==> analysis_contract
...
- 2026-09-01, merge/split research (supersedes the entry above): a deeper
  ...
  `dg_spec` record and proves `analysis_contract_unit_placed` -- the
  ...
  `ownership_split_dg_exec_analysis`'s `Hstep`/`Henter`/`Hcomb`/`Hcont` derivation for
  the unit shape). Moreover `ownership_split_dg_spec_for` and `ownership_split_dg_spec_placed`
  ...
  `docs/history/MERGE_SPLIT_GENERALIZATION.md`. Step 3.1 shrinks to the orthogonal
...
- 2026-09-01: `dg_combine_tree_at` landed in `DG_Constraint_Programs` (definition plus
...
- 2026-09-01: `Exec_DG_Trees` deleted whole, and ~360 lines of dead commute
  ...
  (`DG_Constraint_Programs` in Core defines the same name; the Exec-local copy shadows
...
- 2026-09-01: the statement-level half of the `_at` migration landed, closing
  ...
  `Strategy_Tree_Relabel.thy` deleted whole; `DG_Constraint_Programs`'s import of it
  ...
  with "Cannot load theory Strategy_Tree_Relabel" from `DG_Constraint_Programs` even
...
- 2026-09-01: `Context_Refinement.thy` (`Voblint_Solver`), and its sole
  consumer `Call_String_Solver_Projection.thy` (`Voblint_Framework`) and *its*
...
- **2026-09-01, later same day** -- a four-agent architecture audit (Solver
  ...
  1. `dg_edge_tree`/`dg_combine_tree` (`DG_Constraint_Programs.thy`) redefined as
...
- `Rel_Order_Domain` interprets `analysis_contract` at a carrier that is not an
...
- A change in `Voblint_Framework` rebuilds every session above it, which is longer
...
- The umbrella theory was not kept. A façade re-exporting all four was written
  ...
  `Exec_St.thy` is deleted; the layer index lives in `src/Abstract_Interpreter/Exec/README.md`.
