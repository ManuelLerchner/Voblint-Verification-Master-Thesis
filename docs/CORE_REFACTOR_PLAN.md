...
| 1.8 | Rewrite the READMEs from the final tree: `src/Domain`, `src/Solver`, `src/Framework`, `src/Exec`; delete `Solver/Context/Activation/README.md` and `Solver/Context/DG/README.md` (both list theories that do not exist). Fix `GLOSSARY.md`'s `dg_gen_of` path and the session graph in `AGENTS.md`. | landed |
...
- 2026-08-30: Core's layout inside `src/Framework/` is `Equations/`, `DG/`,
