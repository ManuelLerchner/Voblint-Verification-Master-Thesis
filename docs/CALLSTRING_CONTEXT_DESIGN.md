...
**Plugging into `routed_context`/`dg_ctx_activation`/`analysis_contract`:**
unchanged from how `Example_Interval_DG_CallString.thy` already plugs in
(`twice_cs_dg`, `twice_cs_routed`) — `analysis_contract` is untouched (still
`ivl_dg`/`analysis_contract_unit`), `dg_ctx_activation` is interpreted at
...
`routed_context`, `dg_ctx_activation`, or `analysis_contract`. Verified twice,
...
`analysis_contract`, no `routed_context`, no `TD_side` solver. Nothing about a
