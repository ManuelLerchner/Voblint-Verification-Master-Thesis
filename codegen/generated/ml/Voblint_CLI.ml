...
module Core : sig
  ...
  val char_0x7B : char
  val char_0x7D : char
  val equal_set : 'a equal -> 'a set -> 'a set -> bool
  ...
  val entry_state_check_report_prog :
    string -> unit imp_prog_ext -> (cfg_node * (exp * check_result)) list
      ...
      (base_dg_spec_st_for_lifted
        ...
        (bounded_semilattice_sup_bot_lifted
          (semilattice_sup_resolved_st_q
            (bounded_semilattice_sup_bot_int_dom_ext
              int_dom_record_lattice_unit)))
        gs is_bot_pred (int_tf_st_fixpoint_for gs)
        (int_dom_enter_fixpoint_st_for gs))
...
let rec analyse_int_report p = analyse_int_report_for (declared_global p) p;;
...
let rec analyse_int_dg_env_for
  is_bot_pred gs p =
    (let sol = snd (analyse_int_dg_for is_bot_pred gs p) in
      (fun v ->
        (match
          map_lift
...
let rec analyse_with_solver
  x0 x1 p = match x0, x1, p with
    ...
    | Int_Analysis, Solver_Join, p -> None
    | Int_Analysis, Solver_PerOrigin, p -> None
  ...
  report =
    Core.sorted_list_of_set (Core.equal_literal, Core.linorder_literal)
      (Core.sup_seta Core.equal_literal
        (Core.image (fun (_, (c, (_, _))) -> Core.exp_vnames c)
          (Core.Set report)));;
