...
def diagnostic(path, line=4, severity="error", operation="division", analysis="interval"):
    message = (
        f"possible {operation} by zero" if severity == "warning"
...
def test_annotations_preserve_duplicate_occurrences(case):
    ...
    assert runner.check_arithmetic(case, ["--analysis", "interval"], matching_stderr(case)) == (True, [])


@pytest.mark.parametrize("change, expected_problem", [
    (lambda path: diagnostic(path) + diagnostic(path, 5, "warning", "remainder"), "missing 1 error"),
    (lambda path: matching_stderr(path) + diagnostic(path), "unexpected 1 error"),
    (lambda path: matching_stderr(path) + diagnostic(path, 6), "line 6: unexpected"),
    (lambda path: matching_stderr(path) + diagnostic(path, 7), "line 7: unexpected"),
    (lambda path: diagnostic(path, 4, "warning") * 2 + diagnostic(path, 5, "warning", "remainder"), "missing 2 error"),
    (lambda path: matching_stderr(path).replace(":4:3:", ":3:3:"), "line 4: missing"),
])
def test_mismatches_fail_with_source_and_count(case, change, expected_problem):
    ok, problems = runner.check_arithmetic(case, ["--analysis", "interval"], change(case))
    assert not ok
    assert any(expected_problem in problem for problem in problems)


@pytest.mark.parametrize("annotation", [
    "WARN division-by-zerro", "WARNING division-by-zero", "", "NONE; ERROR division-by-zero",
])
...
def test_malformed_or_misattributed_output_fails(case, change):
    assert not runner.check_arithmetic(case, ["--analysis", "interval"], change(case))[0]
...
def test_directive_alone_requires_no_diagnostics(case):
    ...
    assert not runner.check_arithmetic(case, ["--analysis", "interval"], diagnostic(case))[0]


def test_existing_fixtures_do_not_acquire_warning_expectations(case):
    case.write_text("// PARAM: --analysis interval\nfun main() { x = 1 / 0; }\n")
    assert runner.expected_arithmetic(case) is None
    assert runner.check_arithmetic(case, ["--analysis", "interval"], diagnostic(case)) == (True, [])


def test_warning_only_fixture_runs_successfully(case, monkeypatch):
    monkeypatch.setattr(runner, "run_voblint", lambda *_: subprocess.CompletedProcess(
