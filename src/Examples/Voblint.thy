...
text \<open>
  ...
  \<^bold>\<open>9. Executable code generation.\<close> A runtime-program entry point per domain,
    ...
    \<^item> @{theory Voblint_Examples.Example_Analysis_Dispatch} --- \<^verbatim>\<open>analyse\<close>
      ...
      tracked under \<^verbatim>\<open>codegen/generated/\<close> (regenerated by \<^verbatim>\<open>make codegen\<close>;
      \<^verbatim>\<open>make codegen-check\<close> fails if they have drifted from the
      \<^verbatim>\<open>export_code\<close> declarations). Hand-written Haskell and OCaml drivers
      under \<^verbatim>\<open>codegen/regression/\<close> (\<^verbatim>\<open>make regression\<close>) construct that same
