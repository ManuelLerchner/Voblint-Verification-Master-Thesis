...
text \<open>
  ...
  \<^bold>\<open>9. Executable code generation.\<close> A runtime-program entry point per domain,
    ...
    parallel one, exported to Haskell and OCaml.
    ...
    \<^item> @{theory Voblint_Examples.Example_Analysis_Dispatch} --- \<^verbatim>\<open>analyse\<close>
      ...
      both Haskell and OCaml (\<^verbatim>\<open>export_code ... in Haskell\<close> /
      \<^verbatim>\<open>in OCaml\<close>), so external code can build a fresh \<^verbatim>\<open>imp_prog\<close> and call
      \<^verbatim>\<open>analyse\<close> without touching Isabelle. The generated sources are
      tracked under \<^verbatim>\<open>codegen/generated/\<close> (regenerated by \<^verbatim>\<open>pixi run codegen\<close>;
      \<^verbatim>\<open>pixi run codegen-check\<close> fails if they have drifted from the
      \<^verbatim>\<open>export_code\<close> declarations). Hand-written Haskell and OCaml drivers
      under \<^verbatim>\<open>codegen/regression/\<close> (\<^verbatim>\<open>pixi run regression\<close>) construct that same
      program purely through the exported constructors, call \<^verbatim>\<open>analyse\<close>, and
      check the result against the values \<^verbatim>\<open>dispatch_demo_sign_unknown\<close> and
