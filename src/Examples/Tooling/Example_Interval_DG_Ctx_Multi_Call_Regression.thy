...
text \<open>
  ...
  cannot exercise the case \<^const>\<open>routed_call_program\<close>'s routing comment warns about: a call site
...
text \<open>Both outgoing edges are visible from the shared call site, so a return-side
  ...
  the pattern \<^const>\<open>routed_call_program\<close> avoids --- returns the same call action
