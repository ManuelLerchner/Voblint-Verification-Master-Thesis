...
lemma dg_tree_st_commute_routed_cmb_1:
  "dg_tree_st_commute_for sign_nest_gs env (routed_cmb Spoly Global1 Seed1 (cs_route 1) ctx ca cc ex)
...
theorem sign_nest_1_pp_abs:
  "part_post_solution
     (side_cfg_T_eff_keyed_seed_dg intra_predecessor_list (\<lambda>_. Global1) (cs_route 1)
        (routed_cmb Sabs Global1 Seed1) (routed_extra Seed1 Global1) sign_nest_cfg Sabs
...
proof -
  have pp': "part_post_solution
       (side_cfg_T_eff_keyed_seed_dg intra_predecessor_list (\<lambda>_. Global1) (cs_route 1)
          (routed_cmb Spoly Global1 Seed1) (routed_extra Seed1 Global1) sign_nest_cfg Spoly
  ...
  "sign_ctx_sg_1 k =
     (case k of
        Inl (v, ctx) \<Rightarrow>
          ...
          then combine_env_abs sign_nest_gs (locals (sigma_1 (Inl (v, ctx)))) (globs (sigma_1 (Inr Global1)))
           else bot)
...
lemma sign_ctx_sg_1_covered:
  "(v, ctx) \<in> fst sign_nest_1_sol
   \<Longrightarrow> sign_ctx_sg_1 (Inl (v, ctx))
       = combine_env_abs sign_nest_gs (locals (sigma_1 (Inl (v, ctx)))) (globs (sigma_1 (Inr Global1)))"
