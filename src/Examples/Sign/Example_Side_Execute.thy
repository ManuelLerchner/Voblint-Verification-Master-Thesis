...
lemma x1_calls_eval: "calls (prog_cfg prog_main_name x1_prog) = {}"
...
lemma x1_terminates: "sctx_terminates_prog x1_gs prog_main_name x1_prog"
  by (rule sctx_terminates_prog_via_solve_c) eval

lemma x1_entry_cov:
  "(cfg_entry (prog_cfg prog_main_name x1_prog), ())
     \<in> fst (sctx_sol_prog x1_gs prog_main_name x1_prog)"
  by eval

lemma x1_fwd_ok_ball:
  "\<forall>(u, a, w) \<in> intra (prog_cfg prog_main_name x1_prog).
     (u, ()) \<in> fst (sctx_sol_prog x1_gs prog_main_name x1_prog) \<longrightarrow>
     (w, ()) \<in> fst (sctx_sol_prog x1_gs prog_main_name x1_prog)"
  by eval

lemma x1_fwd_ok:
  assumes "(u, ctx) \<in> fst (sctx_sol_prog x1_gs prog_main_name x1_prog)"
    and "(u, a, w) \<in> intra (prog_cfg prog_main_name x1_prog)"
  shows "(w, ctx) \<in> fst (sctx_sol_prog x1_gs prog_main_name x1_prog)"
  using assms x1_fwd_ok_ball by (cases ctx) auto

lemma x1_call_fwd_ok:
  assumes "(u, ctx) \<in> fst (sctx_sol_prog x1_gs prog_main_name x1_prog)"
    and "(u, CallEdge dst fs as, FunctionEntry q, k) \<in> calls (prog_cfg prog_main_name x1_prog)"
  shows "(FunctionEntry q, ()) \<in> fst (sctx_sol_prog x1_gs prog_main_name x1_prog)"
  using assms by (simp add: x1_calls_eval)

lemma x1_comb_fwd_ok:
  assumes "(cl, c1) \<in> fst (sctx_sol_prog x1_gs prog_main_name x1_prog)"
    and "(cl, CallEdge dst fs as, FunctionEntry q, k) \<in> calls (prog_cfg prog_main_name x1_prog)"
  shows "(k, c1) \<in> fst (sctx_sol_prog x1_gs prog_main_name x1_prog)"
  using assms by (simp add: x1_calls_eval)
...
definition x1_exit_env :: "sign abs_state" where
  "x1_exit_env =
     (case lookup_context (analyse_sign_result_for x1_gs x1_prog)
             (cfg_exit (prog_cfg prog_main_name x1_prog)) () of
