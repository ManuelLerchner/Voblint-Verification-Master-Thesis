...
proof -
  ...
  proof -
    have "pcompletes vpr_gs \<Pi> (imp \<lbrakk> x = 5; \<rbrakk>) s0
            (s0((STR ''x'') := \<lbrakk>N 5\<rbrakk>\<^sub>e s0))" by (rule pcompletes_assign)
    thus ?thesis by simp
  qed
  ...
  proof -
    have "pcompletes vpr_gs \<Pi> (imp \<lbrakk> x = 9; \<rbrakk>) (enter_state vpr_gs ?s1)
            ((enter_state vpr_gs ?s1)((STR ''x'') := \<lbrakk>N 9\<rbrakk>\<^sub>e (enter_state vpr_gs ?s1)))"
...
proof -
  ...
  proof -
    have "pcompletes vpr_gs \<Pi> (imp \<lbrakk> Gg = 9; \<rbrakk>) (enter_state vpr_gs s0)
            ((enter_state vpr_gs s0)((STR ''Gg'') := \<lbrakk>N 9\<rbrakk>\<^sub>e (enter_state vpr_gs s0)))"
      by (rule pcompletes_assign)
    thus ?thesis by simp
  qed
...
lemma call_return_completes:
  ...
  shows "\<G>, \<Pi> \<turnstile> (Call (Some x) p [], s, frs)
           \<rightarrow>\<^sub>p\<^sup>* (SKIP,
            (combine_env \<G> s
              ((enter_state \<G> s)(ret_var := \<lbrakk>e\<rbrakk>\<^sub>e (enter_state \<G> s))))
              (x := \<lbrakk>e\<rbrakk>\<^sub>e (enter_state \<G> s)),
            frs)"
proof -
  let ?s' = "(enter_state \<G> s)(ret_var := \<lbrakk>e\<rbrakk>\<^sub>e (enter_state \<G> s))"
  have "\<G>, \<Pi> \<turnstile> (Call (Some x) p [], s, frs)
          \<rightarrow>\<^sub>p (Seq (Return (Some e)) Restore, enter_state \<G> s, Frame s (Some x) # frs)"
    using q by (rule pstep_Call_parameterless)
  moreover have "\<G>, \<Pi> \<turnstile> (Seq (Return (Some e)) Restore,
       enter_state \<G> s, Frame s (Some x) # frs)
      \<rightarrow>\<^sub>p (Seq Unwind Restore, ?s', Frame s (Some x) # frs)"
    by (intro Seq2 ReturnSome)
  moreover have "\<G>, \<Pi> \<turnstile> (Seq Unwind Restore, ?s', Frame s (Some x) # frs)
                   \<rightarrow>\<^sub>p (SKIP, (combine_env \<G> s ?s')(x := \<lbrakk>e\<rbrakk>\<^sub>e (enter_state \<G> s)), frs)"
    using UnwindAct[of \<G> \<Pi> ?s' s "Some x" frs] by simp
...
lemma call_return_none_completes:
  ...
  shows "\<G>, \<Pi> \<turnstile> (Call None p [], s, frs)
           \<rightarrow>\<^sub>p\<^sup>* (SKIP, combine_env \<G> s (enter_state \<G> s), frs)"
proof -
  have "\<G>, \<Pi> \<turnstile> (Call None p [], s, frs)
          \<rightarrow>\<^sub>p (Seq (Return None) Restore, enter_state \<G> s, Frame s None # frs)"
    using q by (rule pstep_Call_parameterless)
  moreover have "\<G>, \<Pi> \<turnstile> (Seq (Return None) Restore, enter_state \<G> s, Frame s None # frs)
                   \<rightarrow>\<^sub>p (Seq Unwind Restore, enter_state \<G> s, Frame s None # frs)"
    by (intro Seq2 ReturnNone)
  moreover have "\<G>, \<Pi> \<turnstile> (Seq Unwind Restore, enter_state \<G> s, Frame s None # frs)
                   \<rightarrow>\<^sub>p (SKIP, combine_env \<G> s (enter_state \<G> s), frs)"
    using UnwindAct[of \<G> \<Pi> "enter_state \<G> s" s None frs] by simp
  ultimately show ?thesis by (meson star.refl star.step)
qed
...
theorem nested_call_return_trace:
  ...
  shows "\<G>, \<Pi> \<turnstile> (Call (Some rout) pout [], s0, [])
           \<rightarrow>\<^sub>p\<^sup>* (Seq after Restore,
            (combine_env \<G> (enter_state \<G> s0)
              ((enter_state \<G> (enter_state \<G> s0))
                (ret_var := \<lbrakk>e\<rbrakk>\<^sub>e (enter_state \<G> (enter_state \<G> s0)))))
                (rin := \<lbrakk>e\<rbrakk>\<^sub>e (enter_state \<G> (enter_state \<G> s0))),
            [Frame s0 (Some rout)])"
proof -
  let ?s1 = "enter_state \<G> s0"
  let ?Fout = "Frame s0 (Some rout)"
  let ?inner = "(combine_env \<G> ?s1
                    ((enter_state \<G> ?s1)(ret_var := \<lbrakk>e\<rbrakk>\<^sub>e (enter_state \<G> ?s1))))
                  (rin := \<lbrakk>e\<rbrakk>\<^sub>e (enter_state \<G> ?s1))"
  have outer: "\<G>, \<Pi> \<turnstile> (Call (Some rout) pout [], s0, [])
      \<rightarrow>\<^sub>p (Seq (Seq (Call (Some rin) pin []) after) Restore, ?s1, [?Fout])"
    using qout by (rule pstep_Call_parameterless)
  have inner: "\<G>, \<Pi> \<turnstile> (Seq (Seq (Call (Some rin) pin []) after) Restore, ?s1, [?Fout])
      \<rightarrow>\<^sub>p\<^sup>* (Seq (Seq SKIP after) Restore, ?inner, [?Fout])"
    by (intro psteps_Seq2 call_return_completes[where \<Pi> = \<Pi> and p = pin, OF qin])
  have resume: "\<G>, \<Pi> \<turnstile> (Seq (Seq SKIP after) Restore, ?inner, [?Fout])
      \<rightarrow>\<^sub>p (Seq after Restore, ?inner, [?Fout])"
    by (intro Seq2 Seq1)
...
proof -
  ...
  let ?t = "(combine_env vpr_gs s0
              ((enter_state vpr_gs s0)(ret_var := \<lbrakk>?e\<rbrakk>\<^sub>e (enter_state vpr_gs s0))))
              ((STR ''x'') := \<lbrakk>?e\<rbrakk>\<^sub>e (enter_state vpr_gs s0))"
  have call: "vpr_gs, \<Pi> \<turnstile> (Call (Some (STR ''x'')) pf [], s0, []) \<rightarrow>\<^sub>p\<^sup>* (imp \<lbrakk> skip; \<rbrakk>, ?t, [])"
...
proof -
  ...
  have inner_guard: "\<not> truthy (\<lbrakk>Less (N 0) (V (STR ''Gx''))\<rbrakk>\<^sub>e ?ei)"
  ...
  have outer_guard: "truthy (\<lbrakk>Less (N 0) (V (STR ''Gx''))\<rbrakk>\<^sub>e ?e0)"
    by (simp add: enter_state_def)
  have dec: "pcompletes vpr_gs \<Pi> (imp \<lbrakk> Gx = Gx - 1; \<rbrakk>) ?e0 ?e1"
  proof -
    have "pcompletes vpr_gs \<Pi> (imp \<lbrakk> Gx = Gx - 1; \<rbrakk>) ?e0
            (?e0((STR ''Gx'') := \<lbrakk>Minus (V (STR ''Gx'')) (N 1)\<rbrakk>\<^sub>e ?e0))"
      by (rule pcompletes_assign)
