...
lemma nested_valid_ltr_example:
  ...
  shows "\<exists>main0 f0 g1 f' final.
           g1 \<in> valid_ltr is_global nest_cfg S \<and> caller_of g1 = Some f0
         \<and> f' \<in> valid_ltr is_global nest_cfg S \<and> caller_of f' = Some main0
         \<and> final \<in> valid_ltr is_global nest_cfg S \<and> sink_node final = Statement 100"
proof -
  define main0 where "main0 = Root [(cfg_entry nest_cfg, s0)]"
  have main_mem: "main0 \<in> valid_ltr is_global nest_cfg S"
  ...
  have f_mem: "f0 \<in> valid_ltr is_global nest_cfg S"
  ...
  have g_mem: "g0 \<in> valid_ltr is_global nest_cfg S"
  ...
  have f'_mem: "f' \<in> valid_ltr is_global nest_cfg S"
    unfolding f'_def
  ...
  have final_mem: "final \<in> valid_ltr is_global nest_cfg S"
...
lemma multi_return_join:
  "\<exists>t1 t2 c1 c2.
      t1 \<in> valid_ltr is_global mret_cfg UNIV \<and> t2 \<in> valid_ltr is_global mret_cfg UNIV \<and> t1 \<noteq> t2
    ...
    \<and> Resume c1 t1 (path c1 @ [(Statement 100, combine_collect is_global None (sink_store c1) (sink_store t1))])
        \<in> valid_ltr is_global mret_cfg UNIV
...
proof -
  ...
  have R0: "r0 \<in> valid_ltr is_global mret_cfg UNIV" unfolding r0_def by (rule valid_ltr.init) simp
  ...
  have K0: "k0 \<in> valid_ltr is_global mret_cfg UNIV" unfolding k0_def by (rule valid_ltr.call[OF R0 ec0])
  ...
  have R1: "r1 \<in> valid_ltr is_global mret_cfg UNIV" unfolding r1_def by (rule valid_ltr.init) simp
  ...
  have K1: "k1 \<in> valid_ltr is_global mret_cfg UNIV" unfolding k1_def by (rule valid_ltr.call[OF R1 ec1])
...
lemma recursion_nesting:
  "\<exists>outer inner.
      outer \<in> valid_ltr is_global rec_cfg UNIV \<and> inner \<in> valid_ltr is_global rec_cfg UNIV
...
proof -
  ...
  have R: "root \<in> valid_ltr is_global rec_cfg UNIV" unfolding root_def by (rule valid_ltr.init) simp
  ...
  have INNER: "inner \<in> valid_ltr is_global rec_cfg UNIV"
...
lemma ltr_collect_multi_return:
  "\<exists>s1 s2 r1 r2.
      s1 \<in> ltr_collect is_global mret_cfg UNIV (FunctionResult pf)
    \<and> s2 \<in> ltr_collect is_global mret_cfg UNIV (FunctionResult pf)
    \<and> r1 \<in> ltr_collect is_global mret_cfg UNIV (Statement 100)
    \<and> r2 \<in> ltr_collect is_global mret_cfg UNIV (Statement 100)"
proof -
  from multi_return_join obtain t1 t2 c1 c2 where
    T1: "t1 \<in> valid_ltr is_global mret_cfg UNIV" and T2: "t2 \<in> valid_ltr is_global mret_cfg UNIV"
    ...
    and R2: "Resume c2 t2 (path c2 @ [(Statement 100, combine_collect is_global None (sink_store c2) (sink_store t2))])
               \<in> valid_ltr is_global mret_cfg UNIV"
    by blast
  have "sink_store t1 \<in> ltr_collect is_global mret_cfg UNIV (FunctionResult pf)"
    using ltr_collect_I[OF T1] sn1 by simp
  moreover have "sink_store t2 \<in> ltr_collect is_global mret_cfg UNIV (FunctionResult pf)"
    using ltr_collect_I[OF T2] sn2 by simp
  moreover have "combine_collect is_global None (sink_store c1) (sink_store t1)
                   \<in> ltr_collect is_global mret_cfg UNIV (Statement 100)"
...
lemma ltr_collect_flat_demo:
  fixes s0 :: store
  shows "s0 \<in> ltr_collect is_global flat_demo {s0} (Statement 0)"
    and "s0 \<in> ltr_collect is_global flat_demo {s0} (Statement 1)"
    and "s0 \<in> ltr_collect is_global flat_demo {s0} (Statement 2)"
proof -
  show c0: "s0 \<in> ltr_collect is_global flat_demo {s0} (Statement 0)"
    using ltr_collect_init[of s0 "{s0}" is_global flat_demo] by (simp add: flat_demo_def)
  have e0: "(Statement 0, EA_Nop, Statement 1) \<in> intra flat_demo" by (simp add: flat_demo_def)
  show c1: "s0 \<in> ltr_collect is_global flat_demo {s0} (Statement 1)"
  ...
  show "s0 \<in> ltr_collect is_global flat_demo {s0} (Statement 2)"
    using ltr_collect_intra[OF c1 e1] by simp
qed

lemma ltr_collect_flat_demo_no_call:
  "Call caller p \<notin> valid_ltr is_global flat_demo S"
