...
lemma nested_valid_ltr_example:
  ...
  shows "\<exists>main0 f0 g1 f' final.
           g1 \<in> \<T>\<^bsub>demo_gs,nest_cfg,S\<^esub> \<and> caller_of g1 = Some f0
         \<and> f' \<in> \<T>\<^bsub>demo_gs,nest_cfg,S\<^esub> \<and> caller_of f' = Some main0
         \<and> final \<in> \<T>\<^bsub>demo_gs,nest_cfg,S\<^esub> \<and> sink_node final = Statement 100"
proof -
  ...
  have f_mem: "f0 \<in> \<T>\<^bsub>demo_gs,nest_cfg,S\<^esub>"
  ...
  have g_mem: "g0 \<in> \<T>\<^bsub>demo_gs,nest_cfg,S\<^esub>"
  ...
  have g1_mem: "g1 \<in> \<T>\<^bsub>demo_gs,nest_cfg,S\<^esub>"
  ...
  have f2_mem: "f2 \<in> \<T>\<^bsub>demo_gs,nest_cfg,S\<^esub>"
...
lemma multi_return_join:
  "\<exists>t1 t2 c1 c2.
      t1 \<in> \<T>\<^bsub>demo_gs,mret_cfg,UNIV\<^esub> \<and> t2 \<in> \<T>\<^bsub>demo_gs,mret_cfg,UNIV\<^esub> \<and> t1 \<noteq> t2
...
proof -
  ...
  have R0: "r0 \<in> \<T>\<^bsub>demo_gs,mret_cfg,UNIV\<^esub>" unfolding r0_def by (rule valid_ltr.init) simp
  ...
  have K0: "k0 \<in> \<T>\<^bsub>demo_gs,mret_cfg,UNIV\<^esub>"
  ...
  have C0: "c0 \<in> \<T>\<^bsub>demo_gs,mret_cfg,UNIV\<^esub>"
  ...
  have T1: "t1 \<in> \<T>\<^bsub>demo_gs,mret_cfg,UNIV\<^esub>"
    unfolding t1_def by (rule valid_ltr.intra[OF C0 eR0 stR0])

  \<comment> \<open>negative branch through Statement 1\<close>
  have R1: "r1 \<in> \<T>\<^bsub>demo_gs,mret_cfg,UNIV\<^esub>" unfolding r1_def by (rule valid_ltr.init) simp
  ...
  have K1: "k1 \<in> \<T>\<^bsub>demo_gs,mret_cfg,UNIV\<^esub>"
  ...
  have C1: "c1 \<in> \<T>\<^bsub>demo_gs,mret_cfg,UNIV\<^esub>"
  ...
  have T2: "t2 \<in> \<T>\<^bsub>demo_gs,mret_cfg,UNIV\<^esub>"
...
proof -
  ...
  have R: "root \<in> \<T>\<^bsub>demo_gs,rec_cfg,UNIV\<^esub>" unfolding root_def by (rule valid_ltr.init) simp
  ...
  have OUTER: "outer \<in> \<T>\<^bsub>demo_gs,rec_cfg,UNIV\<^esub>"
  ...
  have INNER: "inner \<in> \<T>\<^bsub>demo_gs,rec_cfg,UNIV\<^esub>"
...
lemma ltr_collect_multi_return:
  "\<exists>s1 s2 r1 r2.
      s1 \<in> \<C>\<^bsub>demo_gs,mret_cfg,UNIV\<^esub> (FunctionResult pf)
    \<and> s2 \<in> \<C>\<^bsub>demo_gs,mret_cfg,UNIV\<^esub> (FunctionResult pf)
    \<and> r1 \<in> \<C>\<^bsub>demo_gs,mret_cfg,UNIV\<^esub> (Statement 100)
    \<and> r2 \<in> \<C>\<^bsub>demo_gs,mret_cfg,UNIV\<^esub> (Statement 100)"
proof -
  from multi_return_join obtain t1 t2 c1 c2 where
    ...
    and R2: "Resume c2 t2
        ...
        \<in> \<T>\<^bsub>demo_gs,mret_cfg,UNIV\<^esub>"
    by blast
  have "sink_store t1 \<in> \<C>\<^bsub>demo_gs,mret_cfg,UNIV\<^esub> (FunctionResult pf)"
    using ltr_collect_I[OF T1] sn1 by simp
  moreover have "sink_store t2 \<in> \<C>\<^bsub>demo_gs,mret_cfg,UNIV\<^esub> (FunctionResult pf)"
    using ltr_collect_I[OF T2] sn2 by simp
  moreover have "combine_collect demo_gs None (sink_store c1) (sink_store t1)
                   \<in> \<C>\<^bsub>demo_gs,mret_cfg,UNIV\<^esub> (Statement 100)"
...
proof -
  from recursion_nesting obtain outer inner where
    ...
    and INNER: "inner \<in> \<T>\<^bsub>demo_gs,rec_cfg,UNIV\<^esub>"
...
lemma valid_ltr_flat_no_call:
  "flat_cfg g \<Longrightarrow> Call caller p \<notin> \<T>\<^bsub>\<G>,g,S\<^esub>"
...
lemma ltr_collect_flat_demo:
  fixes s0 :: store
  shows "s0 \<in> \<C>\<^bsub>demo_gs,flat_demo,{s0}\<^esub> (Statement 0)"
    and "s0 \<in> \<C>\<^bsub>demo_gs,flat_demo,{s0}\<^esub> (Statement 1)"
    and "s0 \<in> \<C>\<^bsub>demo_gs,flat_demo,{s0}\<^esub> (Statement 2)"
proof -
  show c0: "s0 \<in> \<C>\<^bsub>demo_gs,flat_demo,{s0}\<^esub> (Statement 0)"
  ...
  show c1: "s0 \<in> \<C>\<^bsub>demo_gs,flat_demo,{s0}\<^esub> (Statement 1)"
  ...
  show "s0 \<in> \<C>\<^bsub>demo_gs,flat_demo,{s0}\<^esub> (Statement 2)"
    using ltr_collect_intra_step[OF c1 e1] by simp
qed

lemma ltr_collect_flat_demo_no_call:
  "Call caller p \<notin> \<T>\<^bsub>demo_gs,flat_demo,S\<^esub>"
