...
definition demo_cfg :: cfg where
  "demo_cfg =
     \<lparr> intra =
         { (FunctionEntry dpf, EA_Ret None dpf I32, FunctionResult dpf),
           (Statement 0,      EA_Ret None dpf I32, FunctionResult dpf) },
       calls =
...
lemma demo_returns_converge:
  "(FunctionEntry dpf, EA_Ret None dpf I32, FunctionResult dpf) \<in> intra demo_cfg"
  "(Statement 0, EA_Ret None dpf I32, FunctionResult dpf) \<in> intra demo_cfg"
...
definition nest_cfg :: cfg where
  "nest_cfg =
     \<lparr> intra =
         { (FunctionEntry pg, EA_Ret None pg I32, FunctionResult pg),
           (Statement 200,    EA_Ret None pf I32, FunctionResult pf) },
       calls =
...
lemma nested_valid_ltr_example:
  ...
  shows "\<exists>main0 f0 g1 f' final.
           g1 \<in> valid_ltr default_tyenv demo_gs nest_cfg S \<and> caller_of g1 = Some f0
         \<and> f' \<in> valid_ltr default_tyenv demo_gs nest_cfg S \<and> caller_of f' = Some main0
         \<and> final \<in> valid_ltr default_tyenv demo_gs nest_cfg S \<and> sink_node final = Statement 100"
proof -
  define main0 where "main0 = Root [(cfg_entry nest_cfg, s0)]"
  have main_mem: "main0 \<in> valid_ltr default_tyenv demo_gs nest_cfg S"
  ...
  have f_mem: "f0 \<in> valid_ltr default_tyenv demo_gs nest_cfg S"
  ...
  have g_mem: "g0 \<in> valid_ltr default_tyenv demo_gs nest_cfg S"
    unfolding g0_def by (rule valid_ltr.call[OF f_mem ecall_g])
  have g_sn: "sink_node g0 = FunctionEntry pg" by (simp add: g0_def)
  have eRg: "(sink_node g0, EA_Ret None pg I32, FunctionResult pg) \<in> intra nest_cfg"
    by (simp add: g_sn nest_defs)
  have stRg: "sink_store g0 \<in> edge_step (EA_Ret None pg I32) (sink_store g0)"
    by simp
  define g1 where "g1 = extend g0 (FunctionResult pg, sink_store g0)"
  have g1_mem: "g1 \<in> valid_ltr default_tyenv demo_gs nest_cfg S"
  ...
  have f'_mem: "f' \<in> valid_ltr default_tyenv demo_gs nest_cfg S"
    unfolding f'_def
  ...
  have eRf: "(sink_node f', EA_Ret None pf I32, FunctionResult pf) \<in> intra nest_cfg"
    by (simp add: f'_sn nest_defs)
  have stRf: "sink_store f' \<in> edge_step (EA_Ret None pf I32) (sink_store f')"
    by simp
  define f2 where "f2 = extend f' (FunctionResult pf, sink_store f')"
  have f2_mem: "f2 \<in> valid_ltr default_tyenv demo_gs nest_cfg S"
  ...
  have final_mem: "final \<in> valid_ltr default_tyenv demo_gs nest_cfg S"
...
text \<open>The graph is hand-built, so its guard is written as the elaborated
  \<^typ>\<open>texp\<close> a compiled \<open>EA_Assume\<close> edge carries: both operands at
  \<^const>\<open>I32\<close>, the kind an unannotated declaration gives.\<close>
definition bpos :: texp where "bpos = TLess (TN I32 0) (TVar I32 (STR ''Gx''))"

definition mret_cfg :: cfg where
  "mret_cfg =
     \<lparr> intra =
         { (FunctionEntry pf, EA_Assume bpos,    Statement 0),
           (Statement 0,      EA_Ret None pf I32,    FunctionResult pf),
           (FunctionEntry pf, EA_AssumeNot bpos, Statement 1),
           (Statement 1,      EA_Ret None pf I32,    FunctionResult pf) },
       calls =
         { (FunctionEntry mn, CallEdge None [] [], FunctionEntry pf, Statement 100) },
       cfg_entry = FunctionEntry mn,
       checks = {} \<rparr>"
...
lemma multi_return_join:
  "\<exists>t1 t2 c1 c2.
      t1 \<in> valid_ltr default_tyenv demo_gs mret_cfg UNIV \<and> t2 \<in> valid_ltr default_tyenv demo_gs mret_cfg UNIV \<and> t1 \<noteq> t2
    ...
    \<and> Resume c1 t1 (path c1 @ [(Statement 100, combine_collect demo_gs None (sink_store c1) (sink_store t1))])
        \<in> valid_ltr default_tyenv demo_gs mret_cfg UNIV
...
proof -
  ...
  have R0: "r0 \<in> valid_ltr default_tyenv demo_gs mret_cfg UNIV" unfolding r0_def by (rule valid_ltr.init) simp
  ...
  have K0: "k0 \<in> valid_ltr default_tyenv demo_gs mret_cfg UNIV" unfolding k0_def by (rule valid_ltr.call[OF R0 ec0])
  ...
  have C0: "c0 \<in> valid_ltr default_tyenv demo_gs mret_cfg UNIV" unfolding c0_def by (rule valid_ltr.intra[OF K0 eA0 stA0])
  have c0_sn: "sink_node c0 = Statement 0" by (simp add: c0_def)
  have eR0: "(sink_node c0, EA_Ret None pf I32, FunctionResult pf) \<in> intra mret_cfg"
    by (simp add: c0_sn mret_defs)
  have stR0: "sink_store c0 \<in> edge_step (EA_Ret None pf I32) (sink_store c0)" by simp
  define t1 where "t1 = extend c0 (FunctionResult pf, sink_store c0)"
  have T1: "t1 \<in> valid_ltr default_tyenv demo_gs mret_cfg UNIV" unfolding t1_def by (rule valid_ltr.intra[OF C0 eR0 stR0])

  \<comment> \<open>negative branch through Statement 1\<close>
  have R1: "r1 \<in> valid_ltr default_tyenv demo_gs mret_cfg UNIV" unfolding r1_def by (rule valid_ltr.init) simp
  ...
  have K1: "k1 \<in> valid_ltr default_tyenv demo_gs mret_cfg UNIV" unfolding k1_def by (rule valid_ltr.call[OF R1 ec1])
  ...
  have C1: "c1 \<in> valid_ltr default_tyenv demo_gs mret_cfg UNIV" unfolding c1_def by (rule valid_ltr.intra[OF K1 eA1 stA1])
  have c1_sn: "sink_node c1 = Statement 1" by (simp add: c1_def)
  have eR1: "(sink_node c1, EA_Ret None pf I32, FunctionResult pf) \<in> intra mret_cfg"
    by (simp add: c1_sn mret_defs)
  have stR1: "sink_store c1 \<in> edge_step (EA_Ret None pf I32) (sink_store c1)" by simp
  define t2 where "t2 = extend c1 (FunctionResult pf, sink_store c1)"
  have T2: "t2 \<in> valid_ltr default_tyenv demo_gs mret_cfg UNIV" unfolding t2_def by (rule valid_ltr.intra[OF C1 eR1 stR1])
  ...
  "rec_cfg =
     \<lparr> intra =
         { (FunctionEntry pr, EA_AssumeNot bpos, Statement 0),
           (Statement 0,      EA_Ret None pr I32,    FunctionResult pr),
...
lemma recursion_nesting:
  "\<exists>outer inner.
      outer \<in> valid_ltr default_tyenv demo_gs rec_cfg UNIV \<and> inner \<in> valid_ltr default_tyenv demo_gs rec_cfg UNIV
...
proof -
  ...
  have R: "root \<in> valid_ltr default_tyenv demo_gs rec_cfg UNIV" unfolding root_def by (rule valid_ltr.init) simp
  ...
  have OUTER: "outer \<in> valid_ltr default_tyenv demo_gs rec_cfg UNIV"
  ...
  have INNER: "inner \<in> valid_ltr default_tyenv demo_gs rec_cfg UNIV"
...
lemma ltr_collect_multi_return:
  "\<exists>s1 s2 r1 r2.
      s1 \<in> ltr_collect default_tyenv demo_gs mret_cfg UNIV (FunctionResult pf)
    \<and> s2 \<in> ltr_collect default_tyenv demo_gs mret_cfg UNIV (FunctionResult pf)
    \<and> r1 \<in> ltr_collect default_tyenv demo_gs mret_cfg UNIV (Statement 100)
    \<and> r2 \<in> ltr_collect default_tyenv demo_gs mret_cfg UNIV (Statement 100)"
proof -
  from multi_return_join obtain t1 t2 c1 c2 where
    T1: "t1 \<in> valid_ltr default_tyenv demo_gs mret_cfg UNIV" and T2: "t2 \<in> valid_ltr default_tyenv demo_gs mret_cfg UNIV"
    ...
    and R2: "Resume c2 t2 (path c2 @ [(Statement 100, combine_collect demo_gs None (sink_store c2) (sink_store t2))])
               \<in> valid_ltr default_tyenv demo_gs mret_cfg UNIV"
    by blast
  have "sink_store t1 \<in> ltr_collect default_tyenv demo_gs mret_cfg UNIV (FunctionResult pf)"
    using ltr_collect_I[OF T1] sn1 by simp
  moreover have "sink_store t2 \<in> ltr_collect default_tyenv demo_gs mret_cfg UNIV (FunctionResult pf)"
    using ltr_collect_I[OF T2] sn2 by simp
  moreover have "combine_collect demo_gs None (sink_store c1) (sink_store t1)
                   \<in> ltr_collect default_tyenv demo_gs mret_cfg UNIV (Statement 100)"
...
lemma ltr_collect_flat_demo:
  fixes s0 :: store
  shows "s0 \<in> ltr_collect default_tyenv demo_gs flat_demo {s0} (Statement 0)"
    and "s0 \<in> ltr_collect default_tyenv demo_gs flat_demo {s0} (Statement 1)"
...
proof -
  ...
  show "s0 \<in> ltr_collect default_tyenv demo_gs flat_demo {s0} (Statement 2)"
    using ltr_collect_intra[OF c1 e1] by simp
qed

lemma ltr_collect_flat_demo_no_call:
  "Call caller p \<notin> valid_ltr default_tyenv demo_gs flat_demo S"
