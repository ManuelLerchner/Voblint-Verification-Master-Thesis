...
lemma dispatch_demo_terminates:
  "Interval_Analyses.ictx_terminates_prog_warrow (declared_global dispatch_demo_prog) dispatch_demo_prog"
proof (rule Interval_Analyses.ictx_terminates_prog_warrow_via_solve_c)
...
lemma dispatch_demo_cover_edge_ball:
  "\<forall>(u, a, w) \<in> intra (prog_cfg dispatch_demo_prog).
     (w, ()) \<in> fst (Interval_Analyses.ictx_sol_prog_warrow (declared_global dispatch_demo_prog) dispatch_demo_prog)"
  by eval

lemma dispatch_demo_cover_edge:
  "\<And>u a w ctx. (u, ctx) \<in> fst (Interval_Analyses.ictx_sol_prog_warrow (declared_global dispatch_demo_prog) dispatch_demo_prog)
     \<Longrightarrow> (u, a, w) \<in> intra (prog_cfg dispatch_demo_prog)
     \<Longrightarrow> (w, ctx) \<in> fst (Interval_Analyses.ictx_sol_prog_warrow (declared_global dispatch_demo_prog) dispatch_demo_prog)"
  using dispatch_demo_cover_edge_ball by auto

lemma dispatch_demo_entry_cov:
  "(cfg_entry (prog_cfg dispatch_demo_prog), ())
     \<in> fst (Interval_Analyses.ictx_sol_prog_warrow (declared_global dispatch_demo_prog) dispatch_demo_prog)"
  by eval
...
proof (rule analyse_interval_proved_sound)
  ...
  show "Interval_Analyses.ictx_terminates_prog_warrow (declared_global dispatch_demo_prog) dispatch_demo_prog"
    by (rule dispatch_demo_terminates)
  show "(cfg_entry (prog_cfg dispatch_demo_prog), ())
          \<in> fst (Interval_Analyses.ictx_sol_prog_warrow (declared_global dispatch_demo_prog) dispatch_demo_prog)"
    by (rule dispatch_demo_entry_cov)
  show "\<And>u a w ctx. (u, ctx) \<in> fst (Interval_Analyses.ictx_sol_prog_warrow (declared_global dispatch_demo_prog) dispatch_demo_prog)
          \<Longrightarrow> (u, a, w) \<in> intra (prog_cfg dispatch_demo_prog)
          \<Longrightarrow> (w, ctx) \<in> fst (Interval_Analyses.ictx_sol_prog_warrow (declared_global dispatch_demo_prog) dispatch_demo_prog)"
    by (rule dispatch_demo_cover_edge)
  show "\<And>u ctx dst fs as q k. (u, ctx) \<in> fst (Interval_Analyses.ictx_sol_prog_warrow (declared_global dispatch_demo_prog) dispatch_demo_prog)
          \<Longrightarrow> (u, CallEdge dst fs as, FunctionEntry q, k) \<in> calls (prog_cfg dispatch_demo_prog)
          \<Longrightarrow> (FunctionEntry q, ()) \<in> fst (Interval_Analyses.ictx_sol_prog_warrow (declared_global dispatch_demo_prog) dispatch_demo_prog)"
    by (simp add: dispatch_demo_calls_eval)
  show "\<And>cl c1 dst fs as q k. (cl, c1) \<in> fst (Interval_Analyses.ictx_sol_prog_warrow (declared_global dispatch_demo_prog) dispatch_demo_prog)
          \<Longrightarrow> (cl, CallEdge dst fs as, FunctionEntry q, k) \<in> calls (prog_cfg dispatch_demo_prog)
          \<Longrightarrow> (k, c1) \<in> fst (Interval_Analyses.ictx_sol_prog_warrow (declared_global dispatch_demo_prog) dispatch_demo_prog)"
    by (simp add: dispatch_demo_calls_eval)
