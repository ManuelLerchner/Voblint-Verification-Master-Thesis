...
text \<open>
  ...
  (\<^const>\<open>entry_state_sg_st\<close>) for the activation that produced it, read back and then
  through the lifted concretization \<^const>\<open>gamma_state_lift\<close>.  The
...
proof -
  ...
  show ?thesis
    by (rule source_sound_toplevel_from_collecting_cap
              [where enterc = twice_ctx and gammaM = twice_ctx_gamma,
               OF twice_wf init run' twice_activation_collect_sound])
qed
