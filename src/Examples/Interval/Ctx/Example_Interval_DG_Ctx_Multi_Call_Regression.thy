...
text \<open>
  ...
  cannot exercise the case \<^const>\<open>routed_cmb_g\<close>'s routing comment warns about: a call site
...
text \<open>Both outgoing edges are visible from the shared call site, so a return-side
  ...
  the pattern \<^const>\<open>routed_cmb_g\<close> avoids --- returns the same call action
