...
lemma rc_entry_covered:
  "(cfg_entry (compile_prog (prog_tyenv rc_program) rc_pi rc_procs (STR ''main'') rc_main), [])
     \<in> fst (entry_state_sol rc_gs (prog_tyenv rc_program) rc_is_bot_pred rc_pi rc_procs (STR ''main'') rc_main)"
  unfolding rc_is_bot_pred_def by eval

lemma rc_fwd_closed_all:
  "\<forall>(u, c) \<in> fst (entry_state_sol rc_gs (prog_tyenv rc_program) rc_is_bot_pred rc_pi rc_procs (STR ''main'') rc_main).
     \<forall>(u', a, v) \<in> intra (compile_prog (prog_tyenv rc_program) rc_pi rc_procs (STR ''main'') rc_main).
       u = u' \<longrightarrow> (v, c) \<in> fst (entry_state_sol rc_gs (prog_tyenv rc_program) rc_is_bot_pred rc_pi rc_procs (STR ''main'') rc_main)"
  unfolding rc_is_bot_pred_def by eval

lemma rc_fwd_ok:
  assumes "(u, ctx) \<in> fst (entry_state_sol rc_gs (prog_tyenv rc_program) rc_is_bot_pred rc_pi rc_procs (STR ''main'') rc_main)"
    and "(u, a, v) \<in> intra (compile_prog (prog_tyenv rc_program) rc_pi rc_procs (STR ''main'') rc_main)"
  shows "(v, ctx) \<in> fst (entry_state_sol rc_gs (prog_tyenv rc_program) rc_is_bot_pred rc_pi rc_procs (STR ''main'') rc_main)"
...
lemma rc_call_caller_only_root:
  "\<forall>(p, ctx) \<in> fst (entry_state_sol rc_gs (prog_tyenv rc_program) rc_is_bot_pred rc_pi rc_procs (STR ''main'') rc_main).
     p = Statement 3 \<longrightarrow> ctx = []"
  unfolding rc_is_bot_pred_def by eval

lemma rc_covered_cont:
  "(Statement 4, []) \<in> fst (entry_state_sol rc_gs (prog_tyenv rc_program) rc_is_bot_pred rc_pi rc_procs (STR ''main'') rc_main)"
  unfolding rc_is_bot_pred_def by eval

lemma rc_comb_fwd_ok:
  assumes "(cl, c1) \<in> fst (entry_state_sol rc_gs (prog_tyenv rc_program) rc_is_bot_pred rc_pi rc_procs (STR ''main'') rc_main)"
    and "(cl, CallEdge dst pars args, FunctionEntry p, cont)
           \<in> calls (compile_prog (prog_tyenv rc_program) rc_pi rc_procs (STR ''main'') rc_main)"
  shows "(cont, c1) \<in> fst (entry_state_sol rc_gs (prog_tyenv rc_program) rc_is_bot_pred rc_pi rc_procs (STR ''main'') rc_main)"
...
lemma rc_route_abs_at_call:
  "entry_state_route_abs_gen rc_gs (Statement 3) []
     (enter_local (ectx_abs_spec rc_gs) [(STR ''a'')]
        (compile_actuals (prog_tyenv rc_program) [(STR ''a'')] [V (STR ''x'')])
        (locals (entry_state_sigma_abs_exec rc_gs (prog_tyenv rc_program) rc_is_bot_pred rc_pi rc_procs (STR ''main'') rc_main
           (Inl (Statement 3, []))))
        (globs (entry_state_sigma_abs_exec rc_gs (prog_tyenv rc_program) rc_is_bot_pred rc_pi rc_procs (STR ''main'') rc_main
           (Inr Global))))
...
lemma rc_call_fwd_ok:
  assumes cov: "(u, ctx) \<in> fst (entry_state_sol rc_gs (prog_tyenv rc_program) rc_is_bot_pred rc_pi rc_procs (STR ''main'') rc_main)"
...
proof -
  from ce rc_calls_shape[unfolded rc_cfg_def] have shape:
    "u = Statement 3" "dst = compile_dst (prog_tyenv rc_program) (Some (STR ''y''))" "pars = [(STR ''a'')]"
    "args = compile_actuals (prog_tyenv rc_program) [(STR ''a'')] [V (STR ''x'')]"
    "p = (STR ''p'')"
    by fastforce+
...
theorem rc_activation_collect_sound:
  "activation_collect (prog_tyenv rc_program) rc_gs
     (admiss_exact (entry_state_context rc_gs (prog_tyenv rc_program) rc_is_bot_pred rc_pi rc_procs (STR ''main'') rc_main))
     [] (compile_prog (prog_tyenv rc_program) rc_pi rc_procs (STR ''main'') rc_main) (cinit_stores rc_gs) v ctx
   \<subseteq> gamma_state_lift
       (entry_state_sg rc_gs (prog_tyenv rc_program) rc_is_bot_pred rc_pi rc_procs (STR ''main'') rc_main (Inl (v, ctx)))"
...
lemma rc_call_site_action:
  "call_action_at_call_site (compile_prog (prog_tyenv rc_program) rc_pi rc_procs (STR ''main'') rc_main) (Statement 3)
     = CallEdge (compile_dst (prog_tyenv rc_program) (Some (STR ''y''))) [(STR ''a'')]
         (compile_actuals (prog_tyenv rc_program) [(STR ''a'')] [V (STR ''x'')])"
proof (rule call_action_at_call_site_eq
    [OF rc_finC[unfolded rc_cfg_def] compile_prog_calls_source_unique])
  show "(Statement 3, CallEdge (compile_dst (prog_tyenv rc_program) (Some (STR ''y''))) [(STR ''a'')]
         (compile_actuals (prog_tyenv rc_program) [(STR ''a'')] [V (STR ''x'')]),
...
lemma rc_context_at_call:
  "entry_state_context rc_gs (prog_tyenv rc_program) rc_is_bot_pred rc_pi rc_procs (STR ''main'') rc_main
...
corollary rc_entry_state_coverage:
  assumes sm: "s \<in> gamma_state_lift
    (entry_state_sg rc_gs (prog_tyenv rc_program) rc_is_bot_pred rc_pi rc_procs (STR ''main'') rc_main (Inl (Statement 3, [])))"
  shows "call_enter rc_gs (CallEdge (compile_dst (prog_tyenv rc_program) (Some (STR ''y''))) [(STR ''a'')]
         (compile_actuals (prog_tyenv rc_program) [(STR ''a'')] [V (STR ''x'')])) s
           \<in> gamma_state_lift (entry_state_sg rc_gs (prog_tyenv rc_program) rc_is_bot_pred rc_pi rc_procs (STR ''main'') rc_main
                (Inl (FunctionEntry (STR ''p''), ctx_call)))"
proof -
  have ce: "(Statement 3, CallEdge (compile_dst (prog_tyenv rc_program) (Some (STR ''y''))) [(STR ''a'')]
         (compile_actuals (prog_tyenv rc_program) [(STR ''a'')] [V (STR ''x'')]),
              FunctionEntry (STR ''p''), Statement 4)
              \<in> calls (compile_prog (prog_tyenv rc_program) rc_pi rc_procs (STR ''main'') rc_main)"
    by eval
...
corollary rc_activation_ctx_key:
  "activation_collect (prog_tyenv rc_program) rc_gs
     (admiss_exact (entry_state_context rc_gs (prog_tyenv rc_program) rc_is_bot_pred rc_pi rc_procs (STR ''main'') rc_main))
     [] (compile_prog (prog_tyenv rc_program) rc_pi rc_procs (STR ''main'') rc_main) (cinit_stores rc_gs)
     (FunctionEntry (STR ''p'')) ctx_call
   = {sink_store t | t.
        t \<in> valid_ltr (prog_tyenv rc_program) rc_gs (compile_prog (prog_tyenv rc_program) rc_pi rc_procs (STR ''main'') rc_main) (cinit_stores rc_gs)
