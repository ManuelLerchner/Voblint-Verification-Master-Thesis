...
text \<open>Regression coverage for the shape \<^const>\<open>DG_Keyed_Generator.routed_node_rhs\<close>
  ...
  \<^const>\<open>routed_call_alternative_program\<close> --- still lives in \<open>Statement 3\<close>'s own equation, not in the
  ...
  \<^const>\<open>nest_S_st\<close>'s own entry transfer applied to it. The site is a single program: the
  ...
  \<^const>\<open>routed_call_program\<close>/\<^const>\<open>routed_node_rhs\<close> generation itself.\<close>
...
text \<open>The call metadata the continuation is pinned to: built from the call edge
  and the callee that \<^const>\<open>routed_call_program\<close> reads off the exit node, not from a
...
lemma nest_2_eqs_statement3:
  "nest_2_eqs (Statement 3, ctx)
     = QueryL (Statement 2, ctx)
         (\<lambda>d. routed_call_alternative_program nest_S_st Global Seed (cs_route 2)
                 (\<lambda>x. x = Bot) ctx
...
lemma nest_1_eqs_statement3:
  "nest_1_eqs (Statement 3, ctx)
     = QueryL (Statement 2, ctx)
         (\<lambda>d. routed_call_alternative_program nest_S_st Global Seed (cs_route 1)
                 (\<lambda>x. x = Bot) ctx
                 (CallEdge (Some (STR ''t'')) [(STR ''p'')] [VIMP_Syntax.V (STR ''p'')])
                 (Statement 2) (STR ''g'')
                 (locals d,
