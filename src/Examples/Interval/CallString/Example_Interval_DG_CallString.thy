...
definition twice_cs_eqs :: "(pp \<times> cfg_node, gk_cs, (ivl exec_dg_st, ivl exec_dg_st) dg_state) eqsT" where
  "twice_cs_eqs =
     side_cfg_T_eff_keyed_seed_dg intra_predecessor_list (\<lambda>_. GlobalCS) route_cs
      (routed_cmb Spoly GlobalCS SeedCS) (routed_extra SeedCS GlobalCS)
       twice_cfg Spoly bot cinit_ivl_st (restrict_global_resolved_q cinit_ivl_st)"

definition twice_cs_sol ::
  "(pp \<times> cfg_node) set \<times> (pp \<times> cfg_node + gk_cs \<Rightarrow> (ivl exec_dg_st, ivl exec_dg_st) dg_state)" where
...
lemma dg_tree_st_commute_routed_cmb_cs:
  "dg_tree_st_commute_for twice_gs env (routed_cmb Spoly GlobalCS SeedCS route_cs ctx ca cc ex)
                          (routed_cmb Sabs GlobalCS SeedCS route_cs ctx ca cc ex)"
...
lemma hextra_commute_routed_cs:
  "list_all2 (dg_tree_st_commute_for twice_gs env)
     (routed_extra SeedCS GlobalCS route_cs ctx w)
     (routed_extra SeedCS GlobalCS route_cs ctx w)"
  unfolding routed_extra_def Let_def
  by (auto simp: dg_tree_st_commute_frame_read_cs split: cfg_node.split)

lemma twice_cs_solve_dom:
  "TD_side_warrowing_apinis_Interp.solve_dom TYPE(gk_cs) TYPE((ivl exec_dg_st, ivl exec_dg_st) dg_state)
...
theorem twice_cs_pp_abs:
  "part_post_solution
     (side_cfg_T_eff_keyed_seed_dg intra_predecessor_list (\<lambda>_. GlobalCS) route_cs
        (routed_cmb Sabs GlobalCS SeedCS) (routed_extra SeedCS GlobalCS) twice_cfg Sabs
...
proof -
  have pp': "part_post_solution
       (side_cfg_T_eff_keyed_seed_dg intra_predecessor_list (\<lambda>_. GlobalCS) route_cs
          (routed_cmb Spoly GlobalCS SeedCS) (routed_extra SeedCS GlobalCS) twice_cfg Spoly
  ...
  show ?thesis
    by (rule part_post_solution_seed_dg_st_to_abs_for
          [where gs = twice_gs and pred_sel = intra_predecessor_list and gkey = "\<lambda>_. GlobalCS"
             ...
             and cmb_st = "routed_cmb Spoly GlobalCS SeedCS" and cmb_abs = "routed_cmb Sabs GlobalCS SeedCS"
             and extra_st = "routed_extra SeedCS GlobalCS"
             and extra_abs = "routed_extra SeedCS GlobalCS"
     ...
     (case k of
        Inl (v, ctx) \<Rightarrow>
          (if (v, ctx) \<in> fst twice_cs_sol
           then combine_env\<^sup># twice_gs (locals (sigma_cs (Inl (v, ctx)))) (globs (sigma_cs (Inr GlobalCS)))
           else bot)
...
lemma ivl_ctx_sg_cs_covered:
  "(v, ctx) \<in> fst twice_cs_sol
   \<Longrightarrow> ivl_ctx_sg_cs (Inl (v, ctx))
       = combine_env\<^sup># twice_gs (locals (sigma_cs (Inl (v, ctx)))) (globs (sigma_cs (Inr GlobalCS)))"
...
interpretation twice_cs_dg: dg_ctx_activation Sabs twice_gs twice_cfg GlobalCS route_cs
    "routed_cmb Sabs GlobalCS SeedCS" "routed_extra SeedCS GlobalCS"
    "fun_of_exec_dg_st_for twice_gs (bot::ivl exec_dg_st)" "fun_of_exec_dg_st_for twice_gs cinit_ivl_st"
    "fun_of_exec_dg_st_for twice_gs (restrict_global_resolved_q cinit_ivl_st)"
...
proof (rule activation_collect_sound[where sg = ivl_ctx_sg_cs and admiss = "admiss_exact enterc_cs"
        and startcontext = "cfg_entry twice_cfg"
...
definition twice_cs_graph_config ::
  "(cfg_node, gk_cs, (ivl exec_dg_st, ivl exec_dg_st) dg_state, ivl exec_dg_st) analysis_graph_config" where
