...
theorem proc_global_side_ivl_analysis:
  ...
  assumes collect_exit:
    "t \<in> ltr_collect (compile_prog inc_pi [''p''] (imp \<lbrakk> p() \<rbrakk>)) {s}
       (cfg_exit (compile_prog inc_pi [''p''] (imp \<lbrakk> p() \<rbrakk>)))"
  assumes side_solve_dom:
    "side_cfg_solve_dom_eff (compile_prog inc_pi [''p''] (imp \<lbrakk> p() \<rbrakk>)) ivl_etf bot
       side_proc_global_ivl_s0 ()
       (cfg_exit (compile_prog inc_pi [''p''] (imp \<lbrakk> p() \<rbrakk>)))"
  shows "t \<in> \<lbrakk>side_analyse_eff inc_pi [''p''] (imp \<lbrakk> p() \<rbrakk>) ivl_etf bot side_proc_global_ivl_s0 ()
