...
text \<open>
  ...
  \<^theory>\<open>Voblint_Examples_Interval.Example_Interval_Loop_Coverage\<close>, whose
...
text \<open>\<^const>\<open>loop_prog\<close> and its compiled \<open>loop_cfg\<close> come from
  \<^theory>\<open>Voblint_Examples_Interval.Example_Interval_Loop_Coverage\<close>, which also carries the
