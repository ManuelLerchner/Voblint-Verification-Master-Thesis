...
theorem ltr_post_fixpoint_sound_at_eff_cone:
  ...
  assumes enter_le:
    "\<And>u dst pars args p cont. (u, CallEdge dst pars args, FunctionEntry p, cont) \<in> calls g
       ...
       \<Longrightarrow> etf_full (etf_enter etf pars args u) \<sigma> \<le> side_env_lift \<sigma> (FunctionEntry p)"
  assumes combine_le:
    "\<And>cl dst pars args p cont. (cl, CallEdge dst pars args, FunctionEntry p, cont) \<in> calls g
       ...
       \<Longrightarrow> etf_full (etf_combine_collect etf (call_info_of (CallEdge dst pars args) p) cl (FunctionResult p)) \<sigma> \<le> side_env_lift \<sigma> cont"
...
proof -
  ...
  interpret G: ltr_gamma g S
      "\<lambda>v _. if cfg_reaches g v v0 then gamma_state_lift (side_env_lift \<sigma> v) else UNIV"
  ...
  next
    ...
    proof (cases "cfg_reaches g v v0")
      ...
      have s_in: "s \<in> gamma_state_lift (side_env_lift \<sigma> u)" using EDGE(2) ru by simp
      have "s' \<in> gamma_state_lift (side_env_lift \<sigma> v)"
  ...
  next
    ...
    proof (cases "cfg_reaches g (FunctionEntry p) v0")
      ...
      have s_in: "s \<in> gamma_state_lift (side_env_lift \<sigma> u)" using CALL(2) ru by simp
      have "call_enter gs (CallEdge dst pars args) s
              \<in> gamma_state_lift (side_env_lift \<sigma> (FunctionEntry p))"
  ...
  next
    ...
    proof (cases "cfg_reaches g cont v0")
      ...
      have s_in: "s \<in> gamma_state_lift (side_env_lift \<sigma> cl)" using COMB(2) rcl by simp
      ...
      have "combine_collect gs (ci_dst (call_info_of (CallEdge dst pars args) p)) s t
        ...
        using se.etf_sound_combine_collect inr s_in t_in unfolding side_env_lift_def by blast
  ...
  proof (rule subsetI)
    ...
    have "sink_store u
        \<in> (if cfg_reaches g (sink_node u) v0 then gamma_state_lift (side_env_lift \<sigma> (sink_node u))
           else UNIV)"
      using gt ck by blast
    then have "sink_store u \<in> gamma_state_lift (side_env_lift \<sigma> v0)"
      using u(2) by (simp add: cfg_reaches_refl)
    then show "x \<in> gamma_state_lift (side_env_lift \<sigma> v0)" using u(3) by simp
  qed
qed
...
proof (rule ltr_post_fixpoint_sound_at_eff_cone[OF se inr entry])
  ...
  show "etf_full (apply_etf etf a u) \<sigma> \<le> side_env_lift \<sigma> w"
...
next
  ...
  show "etf_full (etf_enter etf pars args u) \<sigma> \<le> side_env_lift \<sigma> (FunctionEntry p)"
...
next
  ...
  show "etf_full (etf_combine_collect etf (call_info_of (CallEdge dst pars args) p) cl (FunctionResult p)) \<sigma> \<le> side_env_lift \<sigma> cont"
...
proof -
  ...
  proof -
    ...
    show "etf_full (apply_etf etf a u) \<sigma> \<le> side_env_lift \<sigma> w"
      by (rule etf_combined_le_eff[OF pp wv e fin cone_compatible_etf_edge_coherent[OF cone]])
  qed
  have enter_le:
    "\<And>u dst pars args p cont. (u, CallEdge dst pars args, FunctionEntry p, cont) \<in> calls g
  ...
  proof -
    ...
    show "etf_full (etf_enter etf pars args u) \<sigma> \<le> side_env_lift \<sigma> (FunctionEntry p)"
      by (rule etf_enter_combined_le_eff[OF pp wv cmb finC
            cone_compatible_etf_enter_coherent[OF cone]])
  qed
  have combine_le:
    "\<And>cl dst pars args p cont. (cl, CallEdge dst pars args, FunctionEntry p, cont) \<in> calls g
...
corollary side_collect_sound_exit_eff_ltr_cone:
  ...
  assumes cone:  "cone_compatible_etf gs etf"
  assumes inr:   "inr_slot_locals_bot gs \<sigma>"
  shows "ltr_collect gs g S (cfg_exit g) \<subseteq> gamma_state_lift (side_env_lift \<sigma> (cfg_exit g))"
...
theorem side_analyse_eff_collect_sound_exit_ltr:
  ...
  assumes comb_dep1: "\<And>c2 e2 ci2 \<sigma>'. Inl c2 \<in> dep_aux \<sigma>' (etf_combine_collect etf ci2 c2 e2)"
  assumes comb_dep2: "\<And>c2 e2 ci2 \<sigma>'. Inl e2 \<in> dep_aux \<sigma>' (etf_combine_collect etf ci2 c2 e2)"
  ...
  assumes comb_static: "\<And>cc ex ci. static_deps (etf_combine_collect etf ci cc ex)"
  assumes edge_inr:
  ...
  assumes comb_coherent: "\<And>cc ex ci \<sigma>'. reachability_coherent_tree (etf_combine_collect etf ci cc ex) \<sigma>'"
...
proof -
  ...
  have entry_le: "Lifted s0 \<le> side_env_lift \<sigma> (cfg_entry g)"
    by (rule s0_le_side_env_entry_eff[OF pp entry_in])
  have entry_cov: "S \<le> gamma_state_lift (side_env_lift \<sigma> (cfg_entry g))"
  ...
  have cone: "cone_compatible_etf gs etf"
    ...
    by (intro conjI allI edge_dep enter_dep comb_dep1 comb_dep2
          ...
          edge_coherent comb_coherent enter_coherent)
  have collect: "ltr_collect gs g S (cfg_exit g) \<subseteq> gamma_state_lift (side_env_lift \<sigma> (cfg_exit g))"
    by (rule side_collect_sound_exit_eff_ltr_cone[OF se pp[unfolded v0_def] fin finC wf
          entry_cov cone inr])
  have analyse_eq:
    "side_analyse_eff gs \<Pi> ps mnm main etf bot s0 gseed (cfg_exit g)
       = side_env_lift \<sigma> (cfg_exit g)"
...
corollary side_analyse_eff_collect_sound_exit_ltr_cone:
  ...
  by (rule side_analyse_eff_collect_sound_exit_ltr[OF se
        ...
        cone_compatible_etf_edge_coherent[OF cone]
        cone_compatible_etf_enter_coherent[OF cone]
        cone_compatible_etf_comb_coherent[OF cone]])
