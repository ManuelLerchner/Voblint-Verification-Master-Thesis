...
abbreviation enter_global ::
  "('dl, 'dg) dg_spec \<Rightarrow> vname list \<Rightarrow> texp list \<Rightarrow> 'dl \<Rightarrow> 'dg \<Rightarrow> 'dg"
where
  "enter_global S fs as d g \<equiv> fst (dgs_enter S fs as d g)"

abbreviation enter_local ::
  "('dl, 'dg) dg_spec \<Rightarrow> vname list \<Rightarrow> texp list \<Rightarrow> 'dl \<Rightarrow> 'dg \<Rightarrow> 'dl"
where
...
abbreviation with_call ::
  "call_action \<Rightarrow> (typed_var option \<Rightarrow> vname list \<Rightarrow> texp list \<Rightarrow> 'a) \<Rightarrow> 'a"
where
