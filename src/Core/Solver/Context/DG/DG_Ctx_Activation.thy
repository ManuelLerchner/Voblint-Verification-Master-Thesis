...
subsection \<open>A carrier-generic activation locale, staged ahead of dg_ctx_activation's own migration (issue #123)\<close>

text \<open>
  Layer 4 (issue #123): \<open>dg_ctx_activation\<close> hardcoded \<open>'a abs_state\<close> and \<open>gamma_unit\<close> in its own
  carrier/concretization even though its lemmas -- all but two of which cite only inherited
  \<^locale>\<open>sound_dg_spec\<close> facts and generic order/strategy-tree reasoning -- never needed either
  choice. \<open>dg_ctx_activation_base\<close> makes \<open>gammaDG\<close> and the reader's own concretization \<open>gammaM\<close>
  free parameters, matching \<^locale>\<open>sound_dg_spec\<close>'s own genericity; \<open>dg_ctx_activation\<close> below
  stays a thin specialization at \<open>gammaDG := gamma_unit gs\<close>, \<open>gammaM := gamma_state\<close>, so existing
  callers' structural \<open>sg_cov\<close> obligation is unaffected.
\<close>
  ...
  assumes finE[intro]: "finite (intra g)"
    ...
    and sg_cov[simp]: "\<And>v c. (v, c) \<in> vars
        \<Longrightarrow> sg (Inl (v, c)) =
          combine_env_abs gs (locals (sigma (Inl (v, c)))) (globs (sigma (Inr gk0)))"
    and sg_uncov[simp]: "\<And>v c. (v, c) \<notin> vars
...
text \<open>
  \<open>dg_ctx_activation\<close> is exactly \<open>dg_ctx_activation_base\<close> at \<open>gammaDG := gamma_unit gs\<close>,
  \<open>gammaM := gamma_state\<close>: the raw carrier and \<^const>\<open>gamma_unit\<close> are not load-bearing for any
  of the fourteen lemmas below, only for this one instantiation.
\<close>
sublocale base: dg_ctx_activation_base S "\<lambda>d g. gamma_unit gs d g" gs
  g gk0 route cmb extra bot0 s0d s0g sigma vars x0 sg "\<lambda>s. \<lbrakk>s\<rbrakk>"
proof unfold_locales
  show "finite (intra g)" by (rule finE)
next
  show "part_post_solution
    ...
    by (rule pp)
next
  fix v c assume "(v, c) \<in> vars"
  then show "\<lbrakk>sg (Inl (v, c))\<rbrakk> =
      gamma_unit gs (locals (sigma (Inl (v, c)))) (globs (sigma (Inr gk0)))"
    by (simp add: gamma_unit_def)
next
  fix v c assume "(v, c) \<notin> vars"
  then show "\<lbrakk>sg (Inl (v, c))\<rbrakk> = {}"
    by (rule sg_uncov)
next
  fix u a v c assume "(u, c) \<in> vars" "(u, a, v) \<in> intra g"
  then show "(v, c) \<in> vars" by (rule fwd)
qed
  ...
  assumes covCl: "(cl, c1) \<in> vars"
    ...
    and bound:
      "combine_env_abs gs
         ...
         (fst (dgs_combine S dst (locals (sigma (Inl (cl, c1)))) (locals (sigma (Inl (ex, c2))))
                 (globs (sigma (Inr gk0)))))
       \<le> combine_env_abs gs (locals (sigma (Inl (v, cv)))) (globs (sigma (Inr gk0)))"
