theory Call_String_Collecting_Refinement
  imports Call_String_Context "Voblint_CFG.CFG_Local_Trace" "Voblint_CFG.Activation_Context"
begin
...
lemma call_string_collecting_mono:
  assumes le: "k1 \<le> k2"
  shows "activation_collect gs (cs_context k2) seed g S v c2
           \<subseteq> activation_collect gs (cs_context k1) (take k1 seed) g S v (take k1 c2)"
proof
  fix s assume "s \<in> activation_collect gs (cs_context k2) seed g S v c2"
  then obtain t where t1: "t \<in> valid_ltr gs g S" and t2: "sink_node t = v"
    and t3: "key (cs_context k2) seed t = c2" and t4: "sink_store t = s"
  ...
  show "s \<in> activation_collect gs (cs_context k1) (take k1 seed) g S v (take k1 c2)"
    using activation_collect_I[OF t1 t2 key1] t4 by simp
qed
