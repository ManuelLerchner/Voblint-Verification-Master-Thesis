...
theorem activation_collect_sound:
  ...
  shows "activation_collect gs enterc startcontext g S v ctx \<subseteq> \<lbrakk>sg (Inl (v, ctx))\<rbrakk>"
proof (rule subsetI)
  fix st assume "st \<in> activation_collect gs enterc startcontext g S v ctx"
  then obtain t where t: "t \<in> valid_ltr gs g S"
    and sn: "sink_node t = v" and kc: "key enterc startcontext t = ctx" and st: "sink_store t = st"
...
theorem activation_collect_sound_gen:
  ...
  shows "activation_collect gs enterc startcontext g S v ctx \<subseteq> gammaM (sg (Inl (v, ctx)))"
proof (rule subsetI)
  fix st assume "st \<in> activation_collect gs enterc startcontext g S v ctx"
  then obtain t where t: "t \<in> valid_ltr gs g S"
    and sn: "sink_node t = v" and kc: "key enterc startcontext t = ctx" and st: "sink_store t = st"
