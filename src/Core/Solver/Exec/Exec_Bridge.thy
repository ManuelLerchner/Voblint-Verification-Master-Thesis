...
where
  "unit_combine_tree_st gs dst cc ex =
     QueryL cc (\<lambda>sc. QueryL ex (\<lambda>se. QueryG () (\<lambda>g.
       let res = combine_collect_resolved_for_q gs dst (sc \<squnion> g) (se \<squnion> g) in
       Side () (restrict_global_resolved_q res)
         (Answer (restrict_local_resolved_q res)))))"
...
definition unit_etf_st_of_transfer ::
  "(vname \<Rightarrow> bool)
   \<Rightarrow> (edge_action \<Rightarrow> 'a::bounded_semilattice_sup_bot resolved_st_q \<Rightarrow> 'a resolved_st_q)
...
where
  "unit_etf_st_of_transfer gs tf_st enter_st = \<lparr>
    ...
    etf_st_combine    = unit_combine_tree_st gs
  \<rparr>"

lemma apply_etf_st_unit_of_transfer:
  assumes reduces: "action_reduces tf_st"
  shows "apply_etf_st (unit_etf_st_of_transfer gs tf_st enter_st) a u = unit_edge_tree_st (tf_st a) u"
proof -
  interpret action_reduces tf_st by (rule reduces)
  show ?thesis
...
lemma etf_combine_st_unit_of_transfer:
  "etf_combine_st (unit_etf_st_of_transfer gs tf_st enter_st) dst cc ex = unit_combine_tree_st gs dst cc ex"
  unfolding unit_etf_st_of_transfer_def by simp

lemma etf_st_enter_unit_of_transfer:
  "etf_st_enter (unit_etf_st_of_transfer gs tf_st enter_st) xs es u = unit_edge_tree_st (enter_st xs es) u"
  unfolding unit_etf_st_of_transfer_def by simp

lemma etf_st_enter_exists_unit_of_transfer:
  "\<exists>f. etf_st_enter (unit_etf_st_of_transfer gs tf_st enter_st) xs es u = unit_edge_tree_st f u"
  using etf_st_enter_unit_of_transfer by blast

lemma apply_etf_st_exists_unit_of_transfer:
  assumes reduces: "action_reduces tf_st"
  shows "\<exists>f. apply_etf_st (unit_etf_st_of_transfer gs tf_st enter_st) a u = unit_edge_tree_st f u"
...
lemma traverse_unit_combine_tree_st:
  "traverse_rhs (unit_combine_tree_st gs dst cc ex) \<sigma>_st =
   restrict_local_resolved_q (combine_collect_resolved_for_q gs dst (\<sigma>_st (Inl cc) \<squnion> \<sigma>_st (Inr ()))
...
lemma sides_unit_combine_tree_st_Inr:
  "sides_of_rhs (unit_combine_tree_st gs dst cc ex) \<sigma>_st (Inr ()) =
   restrict_global_resolved_q (combine_collect_resolved_for_q gs dst (\<sigma>_st (Inl cc) \<squnion> \<sigma>_st (Inr ()))
                                                  (\<sigma>_st (Inl ex) \<squnion> \<sigma>_st (Inr ())))"
  unfolding unit_combine_tree_st_def by (simp add: Let_def)
...
lemma dep_aux_unit_edge_tree_st:
  ...
  shows "dep_aux \<sigma>1 (unit_edge_tree_st f u) = dep_aux \<sigma>2 (unit_edge_tree gs g u)"
  unfolding unit_edge_tree_st_def unit_edge_tree_def Let_def by simp

lemma dep_aux_unit_combine_tree_st:
  "dep_aux \<sigma>1 (unit_combine_tree_st gs dst cc ex) = dep_aux \<sigma>2 (unit_combine_tree gs dst' cc ex)"
...
lemma side_rg_unit_combine_tree_st: "side_rg (unit_combine_tree_st gs dst cc ex)"
  unfolding unit_combine_tree_st_def by (simp add: Let_def)
...
lemma sides_unit_combine_tree_st_Inl:
  "sides_of_rhs (unit_combine_tree_st gs dst cc ex) \<sigma> (Inl u') = bot"
...
locale sound_rhs_generator_exec = sound_rhs_generator_static +
  ...
  assumes edge[simp]:
    ...
    and edge_st[simp]:
      "\<And>a u. apply_etf_st etf_st a u = unit_edge_tree_st (F_st a) u"
    and comb_st[simp]:
      "\<And>cc ex dst.
         etf_combine_st etf_st dst cc ex = unit_combine_tree_st gs dst cc ex"
    and commute[simp]:
      "\<And>a s.
...
lemma sides_apply_etf_st:
  "fun_of_resolved_st_q_for gs (sides_of_rhs (apply_etf_st etf_st a u) \<sigma>_st k)
   = sides_of_rhs (apply_etf etf a u) (fun_of_resolved_st_q_for gs \<circ> \<sigma>_st) k"
proof (cases k)
  case (Inl u')
  then show ?thesis
...
lemma sides_etf_combine_st:
  "fun_of_resolved_st_q_for gs (sides_of_rhs (etf_combine_st etf_st dst cc ex) \<sigma>_st k)
   = sides_of_rhs (etf_combine etf dst cc ex) (fun_of_resolved_st_q_for gs \<circ> \<sigma>_st) k"
proof (cases k)
  case (Inl u')
  then show ?thesis
...
lemma fold_rhs_values_fun_of_resolved_st_q_for:
  assumes rel: "list_all2
    (\<lambda>t_st t. \<forall>\<sigma>_st. fun_of_resolved_st_q_for gs (traverse_rhs t_st \<sigma>_st) =
                         traverse_rhs t (fun_of_resolved_st_q_for gs \<circ> \<sigma>_st)) ts_st ts"
  shows "fun_of_resolved_st_q_for gs (fold_rhs_values acc_st \<sigma>_st ts_st) =
         fold_rhs_values (fun_of_resolved_st_q_for gs acc_st) (fun_of_resolved_st_q_for gs \<circ> \<sigma>_st) ts"
  using rel
...
next
  ...
  have head: "fun_of_resolved_st_q_for gs (traverse_rhs t_st \<sigma>_st) =
              traverse_rhs t (fun_of_resolved_st_q_for gs \<circ> \<sigma>_st)"
...
lemma side_acc_eff_st_fun_of_resolved_st_q_for:
  ...
  assumes tr_edge:
    "\<And>a u \<sigma>_st. fun_of_resolved_st_q_for gs (traverse_rhs (apply_etf_st etf_st a u) \<sigma>_st)
               = traverse_rhs (apply_etf etf a u) (fun_of_resolved_st_q_for gs \<circ> \<sigma>_st)"
  assumes tr_enter:
    "\<And>cl fs as \<sigma>_st. fun_of_resolved_st_q_for gs (traverse_rhs (etf_st_enter etf_st fs as cl) \<sigma>_st)
                = traverse_rhs (etf_enter etf fs as cl) (fun_of_resolved_st_q_for gs \<circ> \<sigma>_st)"
  assumes tr_comb:
    "\<And>cc ex dst \<sigma>_st. fun_of_resolved_st_q_for gs (traverse_rhs (etf_combine_st etf_st dst cc ex) \<sigma>_st)
                = traverse_rhs (etf_combine etf dst cc ex) (fun_of_resolved_st_q_for gs \<circ> \<sigma>_st)"
  shows "fun_of_resolved_st_q_for gs (side_acc_eff_st etf_st acc_st \<sigma>_st es ens cs) =
         side_acc_eff etf (fun_of_resolved_st_q_for gs acc_st) (fun_of_resolved_st_q_for gs \<circ> \<sigma>_st) es ens cs"
proof -
...
lemma fold_rhs_trees_sides_fun_of_resolved_st_q_for:
  assumes rel: "list_all2
    (\<lambda>t_st t. \<forall>\<sigma>_st gk. fun_of_resolved_st_q_for gs (sides_of_rhs t_st \<sigma>_st gk) =
                            sides_of_rhs t (fun_of_resolved_st_q_for gs \<circ> \<sigma>_st) gk) ts_st ts"
  shows "fun_of_resolved_st_q_for gs (sides_of_rhs (fold_rhs_trees acc_st ts_st) \<sigma>_st gk) =
         sides_of_rhs (fold_rhs_trees acc ts) (fun_of_resolved_st_q_for gs \<circ> \<sigma>_st) gk"
  using rel
...
next
  case (Cons t_st ts_st t ts)
  have head: "fun_of_resolved_st_q_for gs (sides_of_rhs t_st \<sigma>_st gk) =
              sides_of_rhs t (fun_of_resolved_st_q_for gs \<circ> \<sigma>_st) gk"
    using Cons.hyps(1) by blast
  have rest: "fun_of_resolved_st_q_for gs (sides_of_rhs
        (fold_rhs_trees (acc_st \<squnion> traverse_rhs t_st \<sigma>_st) ts_st) \<sigma>_st gk) =
      sides_of_rhs (fold_rhs_trees (acc \<squnion> traverse_rhs t (fun_of_resolved_st_q_for gs \<circ> \<sigma>_st)) ts)
        (fun_of_resolved_st_q_for gs \<circ> \<sigma>_st) gk"
    by (rule Cons.IH)
  show ?case
...
lemma side_rhs_fold_eff_st_sides_fun_of_resolved_st_q_for:
  fixes etf_st :: "('g, ('a::bounded_semilattice_sup_bot) resolved_st_q) effectful_st_transfer"
    and etf :: "('g, 'a) effectful_domain_transfer"
  assumes sd_edge:
    "\<And>a u \<sigma>_st gk. fun_of_resolved_st_q_for gs (sides_of_rhs (apply_etf_st etf_st a u) \<sigma>_st gk)
                = sides_of_rhs (apply_etf etf a u) (fun_of_resolved_st_q_for gs \<circ> \<sigma>_st) gk"
  assumes sd_enter:
    "\<And>cl fs as \<sigma>_st gk. fun_of_resolved_st_q_for gs (sides_of_rhs (etf_st_enter etf_st fs as cl) \<sigma>_st gk)
                = sides_of_rhs (etf_enter etf fs as cl) (fun_of_resolved_st_q_for gs \<circ> \<sigma>_st) gk"
  assumes sd_comb:
    "\<And>cc ex dst \<sigma>_st gk. fun_of_resolved_st_q_for gs (sides_of_rhs (etf_combine_st etf_st dst cc ex) \<sigma>_st gk)
                = sides_of_rhs (etf_combine etf dst cc ex) (fun_of_resolved_st_q_for gs \<circ> \<sigma>_st) gk"
  shows "fun_of_resolved_st_q_for gs (sides_of_rhs (side_rhs_fold_eff_st etf_st acc es ens cs) \<sigma>_st gk)
         = sides_of_rhs (side_rhs_fold_eff etf (fun_of_resolved_st_q_for gs acc) es ens cs)
             (fun_of_resolved_st_q_for gs \<circ> \<sigma>_st) gk"
proof -
  have trees: "list_all2
      (\<lambda>t_st t. \<forall>\<sigma>_st gk. fun_of_resolved_st_q_for gs (sides_of_rhs t_st \<sigma>_st gk) =
...
lemma fold_rhs_trees_dep_fun_of_resolved_st_q_for:
  assumes rel: "list_all2
    (\<lambda>t_st t.
      (\<forall>\<sigma>_st. fun_of_resolved_st_q_for gs (traverse_rhs t_st \<sigma>_st) =
                    traverse_rhs t (fun_of_resolved_st_q_for gs \<circ> \<sigma>_st)) \<and>
  ...
  shows "dep_aux \<sigma>_st (fold_rhs_trees acc_st ts_st) =
         dep_aux (fun_of_resolved_st_q_for gs \<circ> \<sigma>_st) (fold_rhs_trees (fun_of_resolved_st_q_for gs acc_st) ts)"
  using rel
...
next
  case (Cons t_st ts_st t ts)
  have dep: "dep_aux \<sigma>_st t_st = dep_aux (fun_of_resolved_st_q_for gs \<circ> \<sigma>_st) t"
    using Cons.hyps(1) by blast
  have tr: "fun_of_resolved_st_q_for gs (traverse_rhs t_st \<sigma>_st) =
            traverse_rhs t (fun_of_resolved_st_q_for gs \<circ> \<sigma>_st)"
    using Cons.hyps(1) by blast
  have acc_tr: "fun_of_resolved_st_q_for gs (acc_st \<squnion> traverse_rhs t_st \<sigma>_st) =
      fun_of_resolved_st_q_for gs acc_st \<squnion> traverse_rhs t (fun_of_resolved_st_q_for gs \<circ> \<sigma>_st)"
  ...
  have ih': "dep_aux \<sigma>_st
      ...
      dep_aux (fun_of_resolved_st_q_for gs \<circ> \<sigma>_st)
        (fold_rhs_trees (fun_of_resolved_st_q_for gs (acc_st \<squnion> traverse_rhs t_st \<sigma>_st)) ts)"
    by (rule Cons.IH)
  have ih: "dep_aux \<sigma>_st
        (fold_rhs_trees (acc_st \<squnion> traverse_rhs t_st \<sigma>_st) ts_st) =
      dep_aux (fun_of_resolved_st_q_for gs \<circ> \<sigma>_st)
        (fold_rhs_trees (fun_of_resolved_st_q_for gs acc_st \<squnion>
          traverse_rhs t (fun_of_resolved_st_q_for gs \<circ> \<sigma>_st)) ts)"
...
lemma dep_aux_side_rhs_fold_eff_st_eq:
  ...
  shows "dep_aux \<sigma>_st (side_rhs_fold_eff_st etf_st acc es ens cs)
       = dep_aux (fun_of_resolved_st_q_for gs \<circ> \<sigma>_st) (side_rhs_fold_eff etf (fun_of_resolved_st_q_for gs acc) es ens cs)"
proof -
  have trees: "list_all2
      (\<lambda>t_st t.
        (\<forall>\<sigma>_st. fun_of_resolved_st_q_for gs (traverse_rhs t_st \<sigma>_st) =
...
context
  ...
  assumes tr_comb:
    "\<And>cc ex dst \<sigma>_st. fun_of_resolved_st_q_for gs (traverse_rhs (etf_combine_st etf_st dst cc ex) \<sigma>_st)
                = traverse_rhs (etf_combine etf dst cc ex) (fun_of_resolved_st_q_for gs \<circ> \<sigma>_st)"
  assumes sd_edge:
    "\<And>a u \<sigma>_st gg. fun_of_resolved_st_q_for gs (sides_of_rhs (apply_etf_st etf_st a u) \<sigma>_st gg)
               = sides_of_rhs (apply_etf etf a u) (fun_of_resolved_st_q_for gs \<circ> \<sigma>_st) gg"
  assumes sd_enter:
    "\<And>cl fs as \<sigma>_st gg. fun_of_resolved_st_q_for gs (sides_of_rhs (etf_st_enter etf_st fs as cl) \<sigma>_st gg)
                = sides_of_rhs (etf_enter etf fs as cl) (fun_of_resolved_st_q_for gs \<circ> \<sigma>_st) gg"
  assumes sd_comb:
    "\<And>cc ex dst \<sigma>_st gg. fun_of_resolved_st_q_for gs (sides_of_rhs (etf_combine_st etf_st dst cc ex) \<sigma>_st gg)
                = sides_of_rhs (etf_combine etf dst cc ex) (fun_of_resolved_st_q_for gs \<circ> \<sigma>_st) gg"
  assumes dep_edge:
    "\<And>a u \<sigma>1 \<sigma>2. dep_aux \<sigma>1 (apply_etf_st etf_st a u)
               = dep_aux \<sigma>2 (apply_etf etf a u)"
  assumes dep_enter:
...
private lemma fun_of_resolved_st_q_for_eq_cfg_eff_st:
  "fun_of_resolved_st_q_for gs (eq (side_cfg_T_eff_st g etf_st bot0_st s0_st gseed) v \<sigma>_st) =
   eq (side_cfg_T_eff gs g etf (fun_of_resolved_st_q_for gs bot0_st) (fun_of_resolved_st_q_for gs s0_st) gseed) v (fun_of_resolved_st_q_for gs \<circ> \<sigma>_st)"
...
private lemma fun_of_resolved_st_q_for_sides_cfg_eff_st:
  "fun_of_resolved_st_q_for gs (sides_of_rhs (side_cfg_T_eff_st g etf_st bot0_st s0_st gseed v) \<sigma>_st gkey)
   = sides_of_rhs (side_cfg_T_eff gs g etf (fun_of_resolved_st_q_for gs bot0_st) (fun_of_resolved_st_q_for gs s0_st) gseed v)
       (fun_of_resolved_st_q_for gs \<circ> \<sigma>_st) gkey"
proof (cases "v = cfg_entry g")
  case True
  have fold_sides:
    "\<And>gk. fun_of_resolved_st_q_for gs (sides_of_rhs (side_rhs_fold_eff_st etf_st (bot0_st \<squnion> restrict_local_resolved_q s0_st)
        (intra_predecessor_list g (cfg_entry g)) (entry_seed_list g (cfg_entry g)) (return_call_list g (cfg_entry g))) \<sigma>_st gk)
     = sides_of_rhs (side_rhs_fold_eff etf (fun_of_resolved_st_q_for gs bot0_st \<squnion> restrict_local_for gs (fun_of_resolved_st_q_for gs s0_st))
  ...
  proof (simp add: True)
    show "fun_of_resolved_st_q_for gs ((let m = sides_of_rhs (side_rhs_fold_eff_st etf_st
          ...
          = (let m = sides_of_rhs (side_rhs_fold_eff etf
                (fun_of_resolved_st_q_for gs bot0_st \<squnion> restrict_local_for gs (fun_of_resolved_st_q_for gs s0_st))
                (intra_predecessor_list g (cfg_entry g)) (entry_seed_list g (cfg_entry g)) (return_call_list g (cfg_entry g)))
                (fun_of_resolved_st_q_for gs \<circ> \<sigma>_st)
             in m(Inr gseed := m (Inr gseed) \<squnion> restrict_global_for gs (fun_of_resolved_st_q_for gs s0_st))) gkey"
    proof (cases gkey)
      case (Inl u)
      have "fun_of_resolved_st_q_for gs ((let m = sides_of_rhs (side_rhs_fold_eff_st etf_st
              (bot0_st \<squnion> restrict_local_resolved_q s0_st)
              (intra_predecessor_list g (cfg_entry g)) (entry_seed_list g (cfg_entry g)) (return_call_list g (cfg_entry g))) \<sigma>_st
            in m(Inr gseed := m (Inr gseed) \<squnion> restrict_global_resolved_q s0_st)) (Inl u))
          = fun_of_resolved_st_q_for gs (sides_of_rhs (side_rhs_fold_eff_st etf_st
              (bot0_st \<squnion> restrict_local_resolved_q s0_st)
              (intra_predecessor_list g (cfg_entry g)) (entry_seed_list g (cfg_entry g)) (return_call_list g (cfg_entry g)))
            \<sigma>_st (Inl u))"
        by (simp add: Let_def)
      also have "\<dots> = sides_of_rhs (side_rhs_fold_eff etf
            (fun_of_resolved_st_q_for gs bot0_st \<squnion> restrict_local_for gs (fun_of_resolved_st_q_for gs s0_st))
            (intra_predecessor_list g (cfg_entry g)) (entry_seed_list g (cfg_entry g)) (return_call_list g (cfg_entry g)))
          (fun_of_resolved_st_q_for gs \<circ> \<sigma>_st) (Inl u)"
        by (simp add: fold_sides)
      also have "\<dots> = (let m = sides_of_rhs (side_rhs_fold_eff etf
              (fun_of_resolved_st_q_for gs bot0_st \<squnion> restrict_local_for gs (fun_of_resolved_st_q_for gs s0_st))
              (intra_predecessor_list g (cfg_entry g)) (entry_seed_list g (cfg_entry g)) (return_call_list g (cfg_entry g)))
            (fun_of_resolved_st_q_for gs \<circ> \<sigma>_st)
          in m(Inr gseed := m (Inr gseed) \<squnion> restrict_global_for gs (fun_of_resolved_st_q_for gs s0_st))) (Inl u)"
        by (simp add: Let_def)
    ...
    next
      ...
      show ?thesis proof (cases "gk = gseed")
        ...
        also have "\<dots> = fun_of_resolved_st_q_for gs (sides_of_rhs (side_rhs_fold_eff_st etf_st
                (bot0_st \<squnion> restrict_local_resolved_q s0_st)
                (intra_predecessor_list g (cfg_entry g)) (entry_seed_list g (cfg_entry g)) (return_call_list g (cfg_entry g)))
              \<sigma>_st (Inr gseed)) \<squnion> restrict_global_for gs (fun_of_resolved_st_q_for gs s0_st)"
          by simp
        also have "\<dots> = sides_of_rhs (side_rhs_fold_eff etf
              (fun_of_resolved_st_q_for gs bot0_st \<squnion> restrict_local_for gs (fun_of_resolved_st_q_for gs s0_st))
              (intra_predecessor_list g (cfg_entry g)) (entry_seed_list g (cfg_entry g)) (return_call_list g (cfg_entry g)))
            (fun_of_resolved_st_q_for gs \<circ> \<sigma>_st) (Inr gseed) \<squnion> restrict_global_for gs (fun_of_resolved_st_q_for gs s0_st)"
          by (simp add: fold_sides)
        also have "\<dots> = (let m = sides_of_rhs (side_rhs_fold_eff etf
                (fun_of_resolved_st_q_for gs bot0_st \<squnion> restrict_local_for gs (fun_of_resolved_st_q_for gs s0_st))
                (intra_predecessor_list g (cfg_entry g)) (entry_seed_list g (cfg_entry g)) (return_call_list g (cfg_entry g)))
              (fun_of_resolved_st_q_for gs \<circ> \<sigma>_st)
            in m(Inr gseed := m (Inr gseed) \<squnion> restrict_global_for gs (fun_of_resolved_st_q_for gs s0_st))) (Inr gseed)"
          by (simp add: Let_def True)
      ...
      next
        ...
        also have "\<dots> = sides_of_rhs (side_rhs_fold_eff etf
              (fun_of_resolved_st_q_for gs bot0_st \<squnion> restrict_local_for gs (fun_of_resolved_st_q_for gs s0_st))
              (intra_predecessor_list g (cfg_entry g)) (entry_seed_list g (cfg_entry g)) (return_call_list g (cfg_entry g)))
            (fun_of_resolved_st_q_for gs \<circ> \<sigma>_st) (Inr gk)"
          by (simp add: fold_sides)
        also have "\<dots> = (let m = sides_of_rhs (side_rhs_fold_eff etf
                (fun_of_resolved_st_q_for gs bot0_st \<squnion> restrict_local_for gs (fun_of_resolved_st_q_for gs s0_st))
                (intra_predecessor_list g (cfg_entry g)) (entry_seed_list g (cfg_entry g)) (return_call_list g (cfg_entry g)))
              (fun_of_resolved_st_q_for gs \<circ> \<sigma>_st)
            in m(Inr gseed := m (Inr gseed) \<squnion> restrict_global_for gs (fun_of_resolved_st_q_for gs s0_st))) (Inr gk)"
          by (simp add: Let_def False)
...
theorem part_post_solution_st_to_abs_eff:
  ...
  shows "part_post_solution
           ...
           x (fun_of_resolved_st_q_for gs \<circ> \<sigma>_st) vars"
proof -
  ...
  have deps: "\<And>v. v \<in> vars \<Longrightarrow>
    ...
    = dep\<^sub>L (side_cfg_T_eff gs g etf (fun_of_resolved_st_q_for gs bot0_st) (fun_of_resolved_st_q_for gs s0_st) gseed)
             (fun_of_resolved_st_q_for gs \<circ> \<sigma>_st) v"
  proof -
    fix v
  ...
  proof (intro conjI x_in ballI conjI)
    ...
    show "dep\<^sub>L (side_cfg_T_eff gs g etf (fun_of_resolved_st_q_for gs bot0_st) (fun_of_resolved_st_q_for gs s0_st) gseed)
              (fun_of_resolved_st_q_for gs \<circ> \<sigma>_st) v \<subseteq> vars"
    ...
    show "eq (side_cfg_T_eff gs g etf (fun_of_resolved_st_q_for gs bot0_st) (fun_of_resolved_st_q_for gs s0_st) gseed) v
             (fun_of_resolved_st_q_for gs \<circ> \<sigma>_st) \<le> (fun_of_resolved_st_q_for gs \<circ> \<sigma>_st) (Inl v)"
    proof -
      have le_st: "eq (side_cfg_T_eff_st g etf_st bot0_st s0_st gseed) v \<sigma>_st
                   \<le> \<sigma>_st (Inl v)"
        using pp_st v_in by simp
      show ?thesis
        using fun_of_resolved_st_q_for_mono[where gs=gs, OF le_st] fun_of_resolved_st_q_for_eq_cfg_eff_st[where v=v] by simp
    qed
    show "sides_of_rhs (side_cfg_T_eff gs g etf (fun_of_resolved_st_q_for gs bot0_st) (fun_of_resolved_st_q_for gs s0_st) gseed v)
             (fun_of_resolved_st_q_for gs \<circ> \<sigma>_st) \<le> fun_of_resolved_st_q_for gs \<circ> \<sigma>_st"
    proof (rule le_funI)
      fix k
      show "sides_of_rhs
               (side_cfg_T_eff gs g etf (fun_of_resolved_st_q_for gs bot0_st) (fun_of_resolved_st_q_for gs s0_st) gseed v)
               (fun_of_resolved_st_q_for gs \<circ> \<sigma>_st) k \<le> (fun_of_resolved_st_q_for gs \<circ> \<sigma>_st) k"
...
lemma part_post_solution_st_to_abs_eff_unit_transfer:
  ...
  assumes comb_st: "\<And>cc ex dst. etf_combine_st etf_st dst cc ex = unit_combine_tree_st gs dst cc ex"
  ...
  shows "part_post_solution
           ...
           x (fun_of_resolved_st_q_for gs \<circ> \<sigma>_st) vars"
proof -
  interpret sound_rhs_generator_exec gs etf F etf_st F_st
  ...
  proof -
    ...
    show "fun_of_resolved_st_q_for gs (sides_of_rhs (etf_st_enter etf_st fs as cl) \<sigma>_st gg)
        = sides_of_rhs (etf_enter etf fs as cl) (fun_of_resolved_st_q_for gs \<circ> \<sigma>_st) gg"
    proof (cases gg)
      case (Inl u')
      then show ?thesis
...
lemma inr_slot_locals_bot_fun_of_resolved_st_q_for_restrict_global_abs:
  ...
  shows "inr_slot_locals_bot gs (fun_of_resolved_st_q_for gs \<circ> sigma_st)"
  unfolding inr_slot_locals_bot_iff_Inr_restrict_global
proof (intro allI)
  fix gg
  show "(fun_of_resolved_st_q_for gs \<circ> sigma_st) (Inr gg) = restrict_global_for gs ((fun_of_resolved_st_q_for gs \<circ> sigma_st) (Inr gg))"
  proof -
    have "fun_of_resolved_st_q_for gs (sigma_st (Inr gg)) = fun_of_resolved_st_q_for gs (restrict_global_resolved_q (sigma_st (Inr gg)))"
      using rg by simp
