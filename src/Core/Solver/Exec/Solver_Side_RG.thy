...
abbreviation rg_ug :: "('x, 'g, ('a::bot) resolved_st_q) ug_state \<Rightarrow> bool" where
  "rg_ug ug \<equiv> (\<forall>g orig. rg_val (rho_lookup (\<rho> ug) g orig))"
...
next
  ...
  next
    ...
    proof -
      have "rg_val (rho_lookup (\<rho> ug_state1) g' orig')" for g' orig'
      proof (cases "g' = y")
        case True
        then show ?thesis
    ...
    qed
    have rgsup: "rg_val (sup_over_origins ug_state1 y)"
      unfolding sup_over_origins_def
    proof (rule rg_val_sup_fset)
      show "\<And>z. z |\<in>| (rho_lookup (\<rho> ug_state1) y |`| fmdom (\<rho> ug_state1 y)) \<Longrightarrow> rg_val z"
        using rgug1 by auto
      show "rho_lookup (\<rho> ug_state1) y |`| fmdom (\<rho> ug_state1 y) \<noteq> {||}"
        using rho_eq by (auto simp: fmdom_fmupd)
    qed
    have rgold: "rg_val (\<sigma> state (Inr y))" using rgs by simp
    have rgd'': "rg_val d''"
      using d''eq rg_val_warrow[OF Wbb Nbb rgold rgsup] by simp
...
lemma TD_side_warrowing_apinis_solve_Inr_rg:
  fixes T :: "(pp, unit, ('a::bounded_warrowing) resolved_st_q) eqsT"
  assumes dom: "TD_side_warrowing_apinis_Interp.solve_dom TYPE(unit) TYPE('a resolved_st_q) T v"
  assumes sr: "\<And>z. side_rg (T z)"
    and Wbb: "widen (bot::'a) bot = bot" and Nbb: "narrow (bot::'a) bot = bot"
