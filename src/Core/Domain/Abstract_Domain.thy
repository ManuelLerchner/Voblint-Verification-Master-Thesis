...
subsection \<open>Derived less-queries from backward inversion\<close>
...
text \<open>
  ...
  without an extra premise. No comparable \<open>inv_eq\<close> operator exists in this codebase
  (\<open>bfilter\<close>'s own \<open>Eq\<close> case only ever narrows the \<open>True\<close> branch), so \<open>eq_true\<close>/
  \<open>eq_false\<close> are derived below from \<open>less_false\<close> and \<open>meet\<close> instead, not from
  \<open>inv_less\<close>.
\<close>
...
subsection \<open>Derived equality queries\<close>
