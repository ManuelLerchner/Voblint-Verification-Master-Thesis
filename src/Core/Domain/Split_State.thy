...
definition wf_split :: "(vname \<Rightarrow> bool) \<Rightarrow> ('l::bot, 'g::bot) split_state \<Rightarrow> bool" where
  "wf_split gs lg \<longleftrightarrow>
     (\<forall>x. gs x \<longrightarrow> fst lg x = bot) \<and>
     (\<forall>x. \<not> gs x \<longrightarrow> snd lg x = bot)"

lemma wf_splitI:
  "(\<And>x. gs x \<Longrightarrow> L x = bot) \<Longrightarrow> (\<And>x. \<not> gs x \<Longrightarrow> G x = bot)
   \<Longrightarrow> wf_split gs (L, G)"
  unfolding wf_split_def by simp

lemma wf_split_bot: "wf_split gs (bot, bot)"
  unfolding wf_split_def bot_fun_def by simp

lemma wf_split_sup:
  fixes lg1 lg2 ::
    "('l::bounded_semilattice_sup_bot, 'g::bounded_semilattice_sup_bot) split_state"
  assumes "wf_split gs lg1" and "wf_split gs lg2"
  shows "wf_split gs (fst lg1 \<squnion> fst lg2, snd lg1 \<squnion> snd lg2)"
  using assms unfolding wf_split_def by (simp add: sup_fun_def)

subsection \<open>Conversions\<close>

definition merge_state :: "(vname \<Rightarrow> bool) \<Rightarrow> ('a, 'a) split_state \<Rightarrow> 'a abs_state" where
  "merge_state gs lg = (\<lambda>x. if gs x then snd lg x else fst lg x)"

definition split_state :: "(vname \<Rightarrow> bool) \<Rightarrow> ('a::bot) abs_state \<Rightarrow> ('a, 'a) split_state" where
  "split_state gs \<sigma> =
     ((\<lambda>x. if gs x then bot else \<sigma> x),
      (\<lambda>x. if gs x then \<sigma> x else bot))"

lemma fst_split_state:
  "fst (split_state gs \<sigma>) x = (if gs x then bot else \<sigma> x)"
  unfolding split_state_def by simp

lemma snd_split_state:
  "snd (split_state gs \<sigma>) x = (if gs x then \<sigma> x else bot)"
  unfolding split_state_def by simp

lemma wf_split_split_state: "wf_split gs (split_state gs \<sigma>)"
  unfolding wf_split_def split_state_def by simp

subsection \<open>Isomorphism at the homogeneous instance\<close>

lemma merge_split_id [simp]: "merge_state gs (split_state gs \<sigma>) = \<sigma>"
  unfolding merge_state_def split_state_def by (rule ext) simp

lemma split_merge_id:
  assumes wf: "wf_split gs lg"
  shows "split_state gs (merge_state gs lg) = lg"
  using wf unfolding wf_split_def split_state_def merge_state_def
  by (cases lg) (auto simp: fun_eq_iff)

lemma merge_state_bij:
  "bij_betw (merge_state gs :: ('a::bot, 'a) split_state \<Rightarrow> 'a abs_state)
     {lg. wf_split gs lg} UNIV"
proof (rule bij_betw_byWitness[where f' = "split_state gs"])
  show "\<forall>lg \<in> {lg. wf_split gs lg}. split_state gs (merge_state gs lg) = lg"
    by (simp add: split_merge_id)
  show "\<forall>\<sigma> \<in> UNIV. merge_state gs (split_state gs \<sigma>) = \<sigma>" by simp
  show "merge_state gs ` {lg. wf_split gs lg} \<subseteq> UNIV" by simp
  show "split_state gs ` UNIV \<subseteq> {lg. wf_split gs lg}"
...
lemma merge_state_mono:
  ...
  shows "merge_state gs lg1 \<le> merge_state gs lg2"
...
lemma split_state_mono1:
  ...
  shows "fst (split_state gs sigma1) \<le> fst (split_state gs sigma2)"
  using assms unfolding split_state_def le_fun_def by simp

lemma split_state_mono2:
  fixes sigma1 sigma2 :: "('a::order_bot) abs_state"
  assumes "sigma1 \<le> sigma2"
  shows "snd (split_state gs sigma1) \<le> snd (split_state gs sigma2)"
  using assms unfolding split_state_def le_fun_def by simp

lemma merge_state_le_iff:
  fixes lg1 lg2 :: "('a::order_bot, 'a) split_state"
  assumes wf1: "wf_split gs lg1"
  shows "merge_state gs lg1 \<le> merge_state gs lg2 \<longleftrightarrow>
         fst lg1 \<le> fst lg2 \<and> snd lg1 \<le> snd lg2"
proof (rule iffI)
  assume le: "merge_state gs lg1 \<le> merge_state gs lg2"
...
next
  ...
  then show "merge_state gs lg1 \<le> merge_state gs lg2"
    using merge_state_mono by blast
qed

subsection \<open>Lattice-operation transport\<close>

lemma merge_state_bot: "merge_state gs (bot, bot) = (bot :: 'a::bot abs_state)"
...
lemma merge_state_sup:
  ...
  shows "merge_state gs (L1 \<squnion> L2, G1 \<squnion> G2)
         = merge_state gs (L1, G1) \<squnion> merge_state gs (L2, G2)"
  unfolding merge_state_def sup_fun_def by (rule ext) simp

lemma split_state_bot: "split_state gs (bot :: 'a::bot abs_state) = (bot, bot)"
...
lemma split_state_sup:
  ...
  shows "split_state gs (sigma1 \<squnion> sigma2) =
         (fst (split_state gs sigma1) \<squnion> fst (split_state gs sigma2),
          snd (split_state gs sigma1) \<squnion> snd (split_state gs sigma2))"
