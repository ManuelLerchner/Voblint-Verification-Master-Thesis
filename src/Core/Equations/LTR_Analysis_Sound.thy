...
lemma ltr_post_fixpoint_sound_at_for:
  ...
  assumes call_le:
    "\<And>c dst fs as p cont. (c, CallEdge dst fs as, FunctionEntry p, cont) \<in> calls g \<Longrightarrow>
       body\<^sup># tf p (enter\<^sup># tf fs as (env c)) \<le> env (FunctionEntry p)"
  assumes combine_le:
    "\<And>c dst fs as p cont. (c, CallEdge dst fs as, FunctionEntry p, cont) \<in> calls g \<Longrightarrow>
...
proof -
  ...
  proof -
    ...
    have "body\<^sup># tf p (enter\<^sup># tf fs as (env c)) \<le> rhs g tf (\<squnion>) bot s0 env (FunctionEntry p)"
      using tf_enter_le_rhs[OF finI finC e] by (simp add: entry_proc_def)
    also have "\<dots> \<le> env (FunctionEntry p)" using is_post_fixpointD[OF post_fp] .
    finally show "body\<^sup># tf p (enter\<^sup># tf fs as (env c)) \<le> env (FunctionEntry p)" .
  qed
