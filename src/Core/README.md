...
| --- | --- | --- |
| `Equations/` | What must a per-edge transfer satisfy, and how does the solver see the graph? | `Transfer_Interface` (the `domain_transfer` record, `apply_tf`, `sound_transfer_for`, per-edge transfer soundness), `CFG_Enumeration` (predecessors, call sites and returns as lists), `State_Restriction` (the locals/globals restriction algebra), `DG_State_Reconstruction` (`res_edge`/`res_combine`: the reassembled-environment value a lifted-carrier equation generator's transfer/combine reassembly lemmas state) |
