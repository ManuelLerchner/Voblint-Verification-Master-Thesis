...
text \<open>
  ...
  Parity's public soundness, in the vocabulary its runtime API returns: the branch the
  unified dispatcher takes when the configured domain is Parity. Nothing is derived
  here. Each published solver discipline has its own instance of the shared
...
context
  ...
  assumes solve: "parity_conf_terminates_prog (declared_global p) p"
    and entry_cov: "(cfg_entry (prog_cfg p), ())
        \<in> fst (parity_conf_sol_prog (declared_global p) p)"
    and fwd_ok: "\<And>u a w ctx. (u, ctx) \<in> fst (parity_conf_sol_prog (declared_global p) p)
        \<Longrightarrow> (u, a, w) \<in> intra (prog_cfg p)
        \<Longrightarrow> (w, ctx) \<in> fst (parity_conf_sol_prog (declared_global p) p)"
    and call_fwd_ok: "\<And>u ctx dst fs as q k.
        (u, ctx) \<in> fst (parity_conf_sol_prog (declared_global p) p)
        \<Longrightarrow> (u, CallEdge dst fs as, FunctionEntry q, k) \<in> calls (prog_cfg p)
        \<Longrightarrow> (FunctionEntry q, ()) \<in> fst (parity_conf_sol_prog (declared_global p) p)"
    and comb_fwd_ok: "\<And>cl c1 dst fs as q k.
        (cl, c1) \<in> fst (parity_conf_sol_prog (declared_global p) p)
        \<Longrightarrow> (cl, CallEdge dst fs as, FunctionEntry q, k) \<in> calls (prog_cfg p)
        \<Longrightarrow> (k, c1) \<in> fst (parity_conf_sol_prog (declared_global p) p)"
begin
...
lemma parity_conf_vars_cover_prog_of_exec:
...
text \<open>
  What a caller of \<^const>\<open>analyse_parity_result_for\<close> actually wants to know: run the source
...
theorem analyse_parity_source_sound_for:
  ...
  shows "\<exists>v stk. csim (prog_table p) (prog_cfg p) (residual, s, frs) (v, s, stk)
                 \<and> s \<in> \<lbrakk>case lookup_context
                                (analyse_parity_result_for (declared_global p) p) v () of
                                     Bot \<Rightarrow> bot | Lifted st \<Rightarrow> st\<rbrakk>"
...
text \<open>
  The headline pair, at \<^const>\<open>declared_global\<close> \<open>p\<close> and over \<^const>\<open>analyse_parity_result\<close> ---
  ...
  post-solution for this program (\<^const>\<open>parity_conf_terminates_prog\<close>; no result here proves
  the solver terminates on every input), and it solved enough keys (\<^const>\<open>vars_cover\<close>,
  decidable through \<open>parity_conf_vars_cover_prog_of_exec\<close>).
\<close>

corollary analyse_parity_source_sound:
  fixes p :: imp_prog and s0 s :: store
  assumes wf: "wf_compile_input (declared_global p) (prog_table p) (prog_procs p)"
    and solve: "parity_conf_terminates_prog (declared_global p) p"
    and cover: "vars_cover (prog_cfg p) (fst (parity_conf_sol_prog (declared_global p) p))"
    and s0: "s0 \<in> cinit_stores (declared_global p)"
    and run: "star (pstep (declared_global p) (prog_table p))
                (main_body (prog_table p), s0, []) (residual, s, frs)"
