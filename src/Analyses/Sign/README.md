...
[`../Base/README.md`](../Base/README.md). Executable witnesses live under `src/Examples/Sign/`;
...
| `Sign_Exec_Sound.thy` | the arbitrary-program runtime API: the equation system a whole `imp_prog` generates at the context-insensitive route, its solve, its termination side condition, and the result table — computation only, no soundness |
