...
text \<open>
  ...
  through \<^const>\<open>analyse_sign_result_for\<close>'s \<^type>\<open>analysis_result\<close> table --- \<^const>\<open>lookup_context\<close>,
...
text \<open>
  ...
  on one system (\<open>sign_po_equations_eq\<close> is what makes "one system" a theorem rather
