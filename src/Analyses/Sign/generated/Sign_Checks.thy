...
text \<open>
  ...
  Sign's public runtime API: the names a caller outside this session uses, each bound
...
abbreviation sign_conf_eqs_prog ::
  ...
  "sign_conf_eqs_prog \<equiv> sign_unit_equations"

abbreviation sign_conf_sol_prog ::
    "(vname \<Rightarrow> bool) \<Rightarrow> imp_prog
  ...
  "sign_conf_sol_prog \<equiv> sign_unit_solution"

abbreviation sign_conf_terminates_prog :: "(vname \<Rightarrow> bool) \<Rightarrow> imp_prog \<Rightarrow> bool" where
  "sign_conf_terminates_prog \<equiv> sign_unit_terminates"

text \<open>
  The one side condition a caller discharges per program. It is not a decision
  procedure: \<^const>\<open>sign_conf_terminates_prog\<close> follows when the solver's own executable
  entry point returns a result on this program's equations, and nothing here says that
  entry point returns on every input.
\<close>

lemmas sign_conf_terminates_prog_via_solve_c = sign_join.terminates_of_solve_c
lemmas sign_conf_vars_finite = sign_join.vars_finite_of_terminates
...
text \<open>
  ...
  \<^const>\<open>analyse_sign_result_for\<close>'s \<^type>\<open>analysis_result\<close> table --- \<^const>\<open>lookup_context\<close>, not a
  raw solver-environment lookup --- so a \<^const>\<open>Lifted\<close> point classifies at its projected
...
text \<open>
  The state-carrying sibling: same table, with the per-check Sign environment attached
  to each entry instead of discarded, and an \<open>unreachable\<close> flag read straight off
  \<^const>\<open>lookup_context\<close>'s \<^const>\<open>Bot\<close>/\<^const>\<open>Lifted\<close> case split. The flag is \<^term>\<open>True\<close> exactly when
...
definition analyse_sign_report_for_with_state ::
    "(vname \<Rightarrow> bool) \<Rightarrow> imp_prog \<Rightarrow> (pp \<times> exp \<times> check_result \<times> bool \<times> sign abs_state) list" where
...
text \<open>
  ...
  claim). These are bindings onto the assembly's own instance for this rule, so the
...
abbreviation sign_conf_sol_prog_per_origin ::
    "(vname \<Rightarrow> bool) \<Rightarrow> imp_prog
       \<Rightarrow> (pp \<times> unit) set
            \<times> (pp \<times> unit + (unit, unit) routed_gk
                 \<Rightarrow> (sign exec_dg_st lifted, sign exec_dg_st lifted) dg_state)" where
  "sign_conf_sol_prog_per_origin \<equiv> sign_po_solution"
...
definition analyse_sign_result_per_origin ::
    "imp_prog \<Rightarrow> (unit, sign abs_state) analysis_result" where
...
definition analyse_sign_report_per_origin :: "imp_prog \<Rightarrow> check_report_entry list" where
  "analyse_sign_report_per_origin p = sign_po_report (declared_global p) p"

subsection \<open>The published surface, one interpretation per discipline\<close>

text \<open>
  Sign's 2 disciplines through the shared \<^locale>\<open>analysis_surface\<close>. There is one
  interpretation for each discipline Sign publishes and none for any it does not, so
  the absent interpretation and the absent solver route agree by construction rather
  than by a separately maintained legality table. Why Sign publishes these disciplines
...
lemma sign_report_join_eq: "analyse_sign_report p = sign_join_surface.report p"
  by (simp add: analyse_sign_report_def analyse_sign_report_for_def analyse_sign_result_def
      analyse_sign_result_for_def sign_join.report_def surface_unfold)

lemma sign_report_per_origin_eq: "analyse_sign_report_per_origin p = sign_per_origin.report p"
