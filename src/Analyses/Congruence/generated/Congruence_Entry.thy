...
text \<open>
  ...
  Congruence's public soundness, in the vocabulary its runtime API returns: the branch
  the unified dispatcher takes when the configured domain is Congruence. Nothing is
...
context
  ...
  assumes solve: "congruence_conf_terminates_prog (declared_global p) p"
    and entry_cov: "(cfg_entry (prog_cfg p), ())
        \<in> fst (congruence_conf_sol_prog (declared_global p) p)"
    and fwd_ok: "\<And>u a w ctx. (u, ctx) \<in> fst (congruence_conf_sol_prog (declared_global p) p)
        \<Longrightarrow> (u, a, w) \<in> intra (prog_cfg p)
        \<Longrightarrow> (w, ctx) \<in> fst (congruence_conf_sol_prog (declared_global p) p)"
    and call_fwd_ok: "\<And>u ctx dst fs as q k.
        (u, ctx) \<in> fst (congruence_conf_sol_prog (declared_global p) p)
        \<Longrightarrow> (u, CallEdge dst fs as, FunctionEntry q, k) \<in> calls (prog_cfg p)
        \<Longrightarrow> (FunctionEntry q, ()) \<in> fst (congruence_conf_sol_prog (declared_global p) p)"
    and comb_fwd_ok: "\<And>cl c1 dst fs as q k.
        (cl, c1) \<in> fst (congruence_conf_sol_prog (declared_global p) p)
        \<Longrightarrow> (cl, CallEdge dst fs as, FunctionEntry q, k) \<in> calls (prog_cfg p)
        \<Longrightarrow> (k, c1) \<in> fst (congruence_conf_sol_prog (declared_global p) p)"
begin
...
lemma congruence_conf_vars_cover_prog_of_exec:
...
text \<open>
  What a caller of \<^const>\<open>analyse_congruence_result_for\<close> actually wants to know: run the
...
text \<open>
  \<^const>\<open>analyse_congruence_report\<close>'s own soundness corollaries: the check-report layer's
...
text \<open>
  The headline pair, at \<^const>\<open>declared_global\<close> \<open>p\<close> and over \<^const>\<open>analyse_congruence_result\<close>
  ...
  post-solution for this program (\<^const>\<open>congruence_conf_terminates_prog\<close>; no result here
  proves the solver terminates on every input), and it solved enough keys
  (\<^const>\<open>vars_cover\<close>, decidable through \<open>congruence_conf_vars_cover_prog_of_exec\<close>).
\<close>

corollary analyse_congruence_source_sound:
  fixes p :: imp_prog and s0 s :: store
  assumes wf: "wf_compile_input (declared_global p) (prog_table p) (prog_procs p)"
    and solve: "congruence_conf_terminates_prog (declared_global p) p"
    and cover: "vars_cover (prog_cfg p) (fst (congruence_conf_sol_prog (declared_global p) p))"
    and s0: "s0 \<in> cinit_stores (declared_global p)"
    and run: "star (pstep (declared_global p) (prog_table p))
                (main_body (prog_table p), s0, []) (residual, s, frs)"
