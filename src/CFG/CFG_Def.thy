...
definition combine_collect :: "vname option \<Rightarrow> store \<Rightarrow> store \<Rightarrow> store" where
  "combine_collect dst s t = combine_assign dst (t ret_var) (combine_states is_global s t)"

lemma combine_collect_None: "combine_collect None s t = combine_states is_global s t"
...
definition call_enter :: "call_action \<Rightarrow> store \<Rightarrow> store" where
  "call_enter ca s =
     (case ca of CallEdge dst pars actuals \<Rightarrow>
        bind_formals pars (map (\<lambda>e. aval e s) actuals) (enter_state is_global s))"
...
lemma call_enter_Nil [simp]:
  "call_enter (CallEdge dst [] []) s = enter_state is_global s"
