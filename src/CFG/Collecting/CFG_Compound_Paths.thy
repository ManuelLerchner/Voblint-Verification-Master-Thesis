...
lemma cfg_path_mono_edges[intro]:
  assumes sub: "edges g \<subseteq> edges h"
    and p: "g \<turnstile> u \<longrightarrow>\<^bsub>es\<^esub> v"
...
proof (induction es arbitrary: u)
  ...
  hence "(to_cfg c2) \<turnstile> (u - n1) \<longrightarrow>\<^bsub>[]\<^esub> (v - n1)" by (simp add: cfg_path.empty)
...
lemma cfg_path_If_factor_c1:
  ...
  shows "\<exists>es'. es = offset_path 1 es' @ [(EA_Nop, n20 + (n10 + 1))] \<and>
                (to_cfg c1) \<turnstile> (u - 1) \<longrightarrow>\<^bsub>es'\<^esub> ex10"
  using p u_in u_lt
proof (induction es arbitrary: u)
  case Nil
...
next
  ...
  proof cases
    ...
    from Cons.IH[OF ps w_in w_lt] obtain es' where
      ...
      and pe1: "(to_cfg c1) \<turnstile> (w - 1) \<longrightarrow>\<^bsub>es'\<^esub> ex10"
      by auto
    ...
    have p_full: "(to_cfg c1) \<turnstile> (u - 1) \<longrightarrow>\<^bsub>?es'_full\<^esub> ex10"
  ...
  next
    ...
    qed
    have empty_p: "(to_cfg c1) \<turnstile> (u - 1) \<longrightarrow>\<^bsub>[]\<^esub> ex10"
      using bridge(1) by (simp add: cfg_path.empty)
...
lemma cfg_path_If_factor_c2:
  ...
  shows "\<exists>es'. es = offset_path (n10 + 1) es' @ [(EA_Nop, n20 + (n10 + 1))] \<and>
                (to_cfg c2) \<turnstile> (u - (n10 + 1)) \<longrightarrow>\<^bsub>es'\<^esub> ex20"
  using p u_in u_lt
proof (induction es arbitrary: u)
  case Nil
  from Nil(1) have "u = n20 + (n10 + 1)" by (cases rule: cfg_path.cases) simp_all
...
next
  ...
  proof cases
    ...
    from Cons.IH[OF ps w_in w_lt] obtain es' where
      ...
      and pe2: "(to_cfg c2) \<turnstile> (w - (n10 + 1)) \<longrightarrow>\<^bsub>es'\<^esub> ex20"
      by auto
    have toc2_edges: "edges (to_cfg c2) = E20"
    ...
    have p_full: "(to_cfg c2) \<turnstile> (u - (n10 + 1)) \<longrightarrow>\<^bsub>?es'_full\<^esub> ex20"
      by (rule cfg_path.step[OF edge_c2 pe2])
  ...
  next
    ...
    proof (rule ccontr)
      ...
      from e_next w_n2 compound_If_no_source_n2[OF c1 c2] show False by blast
    qed
    have empty_p: "(to_cfg c2) \<turnstile> (u - (n10 + 1)) \<longrightarrow>\<^bsub>[]\<^esub> ex20"
      using bridge(1) by (simp add: cfg_path.empty)
...
proof -
  ...
  proof
    ...
    from pc1 w_minus have pc1': "(to_cfg c1) \<turnstile> en10 \<longrightarrow>\<^bsub>es1\<^esub> ex10" by simp
    have es_eq:
  ...
  next
    ...
    from pc2 w_minus have pc2': "(to_cfg c2) \<turnstile> en20 \<longrightarrow>\<^bsub>es2\<^esub> ex20" by simp
    have es_eq:
...
proof -
  ...
  proof
    ...
    with ps have "(to_cfg (WHILE b DO c)) \<turnstile> (n10 + 1) \<longrightarrow>\<^bsub>tl\<^esub> 0" by simp
    moreover have "\<not> (to_cfg (WHILE b DO c)) \<turnstile> (n10 + 1) \<longrightarrow>\<^bsub>tl\<^esub> 0"
    proof (cases tl)
      case Nil
...
next
  ...
  then obtain es_pre where es_eq: "es = es_pre @ [(EA_AssumeNot b, n10 + 1)]"
    and p0: "(to_cfg (WHILE b DO c)) \<turnstile> 0 \<longrightarrow>\<^bsub>es_pre\<^esub> 0"
    by auto
  have Ew: "edges (to_cfg (WHILE b DO c)) =
            {(0, EA_Assume b, en10 + 1), (0, EA_AssumeNot b, n10 + 1)}
            \<union> offset_edges 1 E10 \<union> {(ex10 + 1, EA_Nop, 0)}"
