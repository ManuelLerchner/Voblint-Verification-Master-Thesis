...
definition ltr_F :: "tyenv \<Rightarrow> (vname \<Rightarrow> bool) \<Rightarrow> cfg \<Rightarrow> store set \<Rightarrow> ltr set \<Rightarrow> ltr set" where
...
lemma ltr_F_mono: "mono (ltr_F \<Gamma> gs g S)"
proof (rule monoI)
  fix T T' :: "ltr set"
  assume "T \<subseteq> T'"
  then show "ltr_F \<Gamma> gs g S T \<subseteq> ltr_F \<Gamma> gs g S T'"
    unfolding ltr_F_def by blast
qed

lemma ltr_F_lfp_fold:
  "x \<in> ltr_F \<Gamma> gs g S (lfp (ltr_F \<Gamma> gs g S)) \<Longrightarrow> x \<in> lfp (ltr_F \<Gamma> gs g S)"
...
lemma ltr_F_valid_ltr_closed:
  "ltr_F \<Gamma> gs g S (valid_ltr \<Gamma> gs g S) \<subseteq> valid_ltr \<Gamma> gs g S"
  unfolding ltr_F_def
  by (blast intro: valid_ltr.intros)

lemma lfp_ltr_F_subset_valid_ltr:
  "lfp (ltr_F \<Gamma> gs g S) \<subseteq> valid_ltr \<Gamma> gs g S"
...
text \<open>The converse follows by \<^const>\<open>valid_ltr\<close> rule induction: every constructor step lands
  in \<open>ltr_F \<Gamma> gs g S (lfp (ltr_F \<Gamma> gs g S))\<close> via the induction hypotheses, then folds back into the
  least fixed point.\<close>
lemma valid_ltr_subset_lfp:
  "valid_ltr \<Gamma> gs g S \<subseteq> lfp (ltr_F \<Gamma> gs g S)"
proof (rule subsetI)
  fix t assume "t \<in> valid_ltr \<Gamma> gs g S"
  then show "t \<in> lfp (ltr_F \<Gamma> gs g S)"
  proof (induction rule: valid_ltr.induct)
    case (init s)
    have "Root [(cfg_entry g, s)] \<in> ltr_F \<Gamma> gs g S (lfp (ltr_F \<Gamma> gs g S))"
      unfolding ltr_F_def[of \<Gamma> gs g S "lfp (ltr_F \<Gamma> gs g S)"] using init by blast
    then show ?case by (rule ltr_F_lfp_fold)
  next
    case (intra t a v s')
    have "extend t (v, s') \<in> ltr_F \<Gamma> gs g S (lfp (ltr_F \<Gamma> gs g S))"
      unfolding ltr_F_def[of \<Gamma> gs g S "lfp (ltr_F \<Gamma> gs g S)"] using intra.hyps intra.IH by blast
    then show ?case by (rule ltr_F_lfp_fold)
  next
...
theorem valid_ltr_eq_lfp:
  "valid_ltr \<Gamma> gs g S = lfp (ltr_F \<Gamma> gs g S)"
...
text \<open>Enlarging the initial store set only adds traces.  \<open>ltr_F\<close>'s dependence on \<open>S\<close> is
  ...
  \<open>S\<close> --- so \<open>ltr_F \<Gamma> gs g S\<close> is pointwise below \<open>ltr_F \<Gamma> gs g S'\<close> and \<open>lfp_mono\<close> applies directly,
  with no case split over \<^const>\<open>valid_ltr\<close>'s four constructors.\<close>
lemma valid_ltr_mono_S:
  assumes "S \<subseteq> S'"
  shows "valid_ltr \<Gamma> gs g S \<subseteq> valid_ltr \<Gamma> gs g S'"
proof -
  have "ltr_F \<Gamma> gs g S T \<subseteq> ltr_F \<Gamma> gs g S' T" for T
    unfolding ltr_F_def using assms by blast
  then have "lfp (ltr_F \<Gamma> gs g S) \<subseteq> lfp (ltr_F \<Gamma> gs g S')"
    by (rule lfp_mono)
...
lemma valid_ltr_eq_UN_singleton:
  "valid_ltr \<Gamma> gs g S = (\<Union>s\<in>S. valid_ltr \<Gamma> gs g {s})"
proof (rule equalityI)
  show "valid_ltr \<Gamma> gs g S \<subseteq> (\<Union>s\<in>S. valid_ltr \<Gamma> gs g {s})"
  proof (rule subsetI)
    fix t assume "t \<in> valid_ltr \<Gamma> gs g S"
    then show "t \<in> (\<Union>s\<in>S. valid_ltr \<Gamma> gs g {s})"
    proof (induction rule: valid_ltr.induct)
      case (init s) then show ?case using valid_ltr.init[where s = s and S = "{s}" and g = g and gs = gs and \<Gamma> = \<Gamma>] by auto
    next
      case (intra t a v s')
      then obtain s where "s \<in> S" "t \<in> valid_ltr \<Gamma> gs g {s}" by auto
      then show ?case using valid_ltr.intra[OF _ intra.hyps(2,3)] by auto
    next
      case (call caller dst pars args p cont)
      then obtain s where "s \<in> S" "caller \<in> valid_ltr \<Gamma> gs g {s}" by auto
      then show ?case using valid_ltr.call[OF _ call.hyps(2)] by auto
    next
      case (ret callee caller p dst pars args cont)
      then obtain s where "s \<in> S" "callee \<in> valid_ltr \<Gamma> gs g {s}" by auto
      then show ?case using valid_ltr.ret[OF _ ret.hyps(2,3,4)] by auto
    qed
  qed
next
  show "(\<Union>s\<in>S. valid_ltr \<Gamma> gs g {s}) \<subseteq> valid_ltr \<Gamma> gs g S"
    using valid_ltr_mono_S by blast
qed

text \<open>Collection distributes over a union of initial sets.\<close>
lemma valid_ltr_Un_S:
  "valid_ltr \<Gamma> gs g (S \<union> S') = valid_ltr \<Gamma> gs g S \<union> valid_ltr \<Gamma> gs g S'"
  using valid_ltr_eq_UN_singleton[of \<Gamma> gs g "S \<union> S'"]
        valid_ltr_eq_UN_singleton[of \<Gamma> gs g S] valid_ltr_eq_UN_singleton[of \<Gamma> gs g S']
  by auto
...
definition ltr_collect :: "tyenv \<Rightarrow> (vname \<Rightarrow> bool) \<Rightarrow> cfg \<Rightarrow> store set \<Rightarrow> cfg_node \<Rightarrow> store set" where
  "ltr_collect \<Gamma> gs g S v =
     {sink_store t | t. t \<in> valid_ltr \<Gamma> gs g S \<and> sink_node t = v}"

text \<open>\<open>collect_result\<close> is a convenience view --- collection at a procedure result --- not a
  distinct mechanism.  Whole-program completion is \<open>collect_result \<Gamma> gs g S main\<close>.\<close>
definition collect_result :: "tyenv \<Rightarrow> (vname \<Rightarrow> bool) \<Rightarrow> cfg \<Rightarrow> store set \<Rightarrow> pname \<Rightarrow> store set" where
  "collect_result \<Gamma> gs g S p = ltr_collect \<Gamma> gs g S (FunctionResult p)"

lemma ltr_collect_I:
  "t \<in> valid_ltr \<Gamma> gs g S \<Longrightarrow> sink_store t \<in> ltr_collect \<Gamma> gs g S (sink_node t)"
  unfolding ltr_collect_def by blast

text \<open>Every collected state has a valid trace witness.\<close>
lemma ltr_collect_E:
  assumes "s \<in> ltr_collect \<Gamma> gs g S v"
...
proof -
  ...
  have "extend t (v, s') \<in> valid_ltr \<Gamma> gs g S"
...
next
  ...
  from step.IH[OF ext] y obtain t' where
    "t' \<in> valid_ltr \<Gamma> gs g S" "sink_node t' = fst z" "sink_store t' = snd z"
...
lemma ltr_collect_init:
  "s \<in> S \<Longrightarrow> s \<in> ltr_collect \<Gamma> gs g S (cfg_entry g)"
...
lemma valid_ltr_flat_no_call:
  "flat_cfg g \<Longrightarrow> Call caller p \<notin> valid_ltr \<Gamma> gs g S"
  using valid_ltr_flat_root by blast

lemma valid_ltr_flat_no_resume:
  "flat_cfg g \<Longrightarrow> Resume caller callee p \<notin> valid_ltr \<Gamma> gs g S"
  using valid_ltr_flat_root by blast

lemma ltr_collect_flat:
  assumes "flat_cfg g"
  shows "ltr_collect \<Gamma> gs g S v = {sink_store t | t. t \<in> valid_ltr \<Gamma> gs g S \<and> (\<exists>q. t = Root q) \<and> sink_node t = v}"
...
lemma ltr_collect_outside_nodes:
  "v \<notin> cfg_nodes g \<Longrightarrow> ltr_collect \<Gamma> gs g S v = {}"
...
lemma ltr_collect_mono_S:
  "S \<subseteq> S' \<Longrightarrow> ltr_collect \<Gamma> gs g S v \<subseteq> ltr_collect \<Gamma> gs g S' v"
...
lemma ltr_collect_Un_S:
  "ltr_collect \<Gamma> gs g (S \<union> S') v = ltr_collect \<Gamma> gs g S v \<union> ltr_collect \<Gamma> gs g S' v"
...
theorem activation_collect_le_ltr_collect:
  "activation_collect \<Gamma> gs admiss startcontext g S v c \<subseteq> ltr_collect \<Gamma> gs g S v"
...
theorem ltr_collect_eq_Union_activation:
  ...
  shows "ltr_collect \<Gamma> gs g S v = (\<Union>c. activation_collect \<Gamma> gs admiss startcontext g S v c)"
proof
  show "ltr_collect \<Gamma> gs g S v \<subseteq> (\<Union>c. activation_collect \<Gamma> gs admiss startcontext g S v c)"
  proof
    fix x assume "x \<in> ltr_collect \<Gamma> gs g S v"
    then obtain t where t: "t \<in> valid_ltr \<Gamma> gs g S" "sink_node t = v" "sink_store t = x"
      by (rule ltr_collect_E)
    obtain c where "ctx_key admiss startcontext t c"
      using ctx_key_exists[where admiss = admiss, OF tot] by blast
    with t show "x \<in> (\<Union>c. activation_collect \<Gamma> gs admiss startcontext g S v c)"
      unfolding activation_collect_def by blast
  qed
next
  show "(\<Union>c. activation_collect \<Gamma> gs admiss startcontext g S v c) \<subseteq> ltr_collect \<Gamma> gs g S v"
    unfolding activation_collect_def ltr_collect_def by blast
qed
...
theorem valid_ltr_Resume_caller_matched:
  "Resume caller callee p \<in> valid_ltr \<Gamma> gs g S \<Longrightarrow> caller_of callee = Some caller"
