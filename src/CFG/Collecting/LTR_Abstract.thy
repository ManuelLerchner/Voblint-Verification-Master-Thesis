...
abbreviation bnd :: "ltr \<Rightarrow> bool" where
  "bnd u \<equiv> \<forall>c. ctx_key admiss seedc u c \<longrightarrow> sink_store u \<in> acc (sink_node u) c"
...
text \<open>\<open>root_closed\<close>: the main activation's seed store is admitted at every context admissibly
  reachable from the entry seed --- only \<open>seedc\<close> itself, per \<open>ctx_key_Root\<close>.\<close>
...
proof (intro allI impI)
  fix c assume "ctx_key admiss seedc (extend t (v, s')) c"
  hence ck: "ctx_key admiss seedc t c" using pne by (simp add: ctx_key_extend_nonempty)
...
proof (intro allI impI)
  fix c' assume "ctx_key admiss seedc
      (Call caller [(FunctionEntry p, call_enter gs (CallEdge dst pars args) (sink_store caller))]) c'"
  then obtain c where c: "ctx_key admiss seedc caller c"
...
proof (intro allI impI)
  fix c1 assume "ctx_key admiss seedc (Resume caller callee
      (path caller @ [(cont, combine_collect gs dst (sink_store caller) (sink_store callee))])) c1"
  hence ck1: "ctx_key admiss seedc caller c1" by (auto elim: ctx_key_ResumeE)
  ...
  have ck2: "ctx_key admiss seedc callee c2"
...
lemma ltr_gamma_UNIV:
  ...
  shows "ltr_gamma g S (\<lambda>_ _. UNIV) admiss seedc gs"
