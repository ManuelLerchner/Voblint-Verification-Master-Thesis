...
| `Example_Side_Branch_Calls.thy` | canonical spine | branching procedure called twice; flow-sensitive locals (`ec_certified_sound_trace`) |
| `Example_Mixed_Flow_Sign.thy` | canonical spine | `mixed_flow_analysis_sound` / `_optimal` applied to sign |
| `Example_Seed_Clean_Context.thy` | required support | Goblint-faithful seeded-clean spine on a two-call program (`seed_clean_example_sharper_than_retain`) |
