...
| `Exec_Sign_Cmp_Keyed_DG_Run.thy` | migration | DG-spine port of `Exec_Sign_Cmp_Keyed_Run`; keyed slots as a `dg_state` solution read through `sound_dg_spec.dg_gamma_c`; meaning provably equal to the homogeneous read; combine via the DG locale's `combine_sound` |
