...
| --- | --- | --- |
| `Equations/` | What algebra does a whole-state transfer compute in, and how does the solver see the graph? | `Transfer_Algebra` (the abstract-state operations a Base-style transfer is assembled from -- entry frame reset and formal binding, the structural return combine -- with their soundness and monotonicity against `gamma_state`), `CFG_Enumeration` (predecessors, call sites and returns as lists), and `State_Restriction` (local/global projections derived from the generic `combine_env` selector) |
