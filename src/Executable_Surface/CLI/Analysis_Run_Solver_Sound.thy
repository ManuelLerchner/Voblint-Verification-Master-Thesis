...
lemma interval_es_join_table:
  assumes cov: "interval_es_join.terminates (declared_global p) p"
      "ctx_vars_cover (prog_cfg p) (interval_es_join.ctx_succ (declared_global p) p) []
...
proof (rule sound_table_of_activation
  ...
  case (1 u)
  show ?case by (simp add: interval_es_join.entry_state_ltr_collect_eq_Union_of_cover [OF cov])
next
  case (2 u ctx)
  show ?case
...
next
  case 3
  show ?case
    using interval_es_join.vars_finite_of_terminates [OF cov(1)]
...
lemma interval_es_po_table:
  assumes cov: "interval_es_po.terminates (declared_global p) p"
      "ctx_vars_cover (prog_cfg p) (interval_es_po.ctx_succ (declared_global p) p) []
...
proof (rule sound_table_of_activation
    [where R = "interval_es_po.admitted_contexts (declared_global p) p" and rc = "[]",
     OF _ _ _ interval_classify_check_proved interval_classify_check_refuted], goal_cases)
  case (1 u)
  show ?case by (simp add: interval_es_po.entry_state_ltr_collect_eq_Union_of_cover [OF cov])
next
  case (2 u ctx)
  show ?case
...
next
  case 3
  show ?case
    using interval_es_po.vars_finite_of_terminates [OF cov(1)]
    by (simp add: finite_analysis_result_def interval_es_po.result_def
        interval_es_po.sol_vars_def)
qed

lemma interval_es_wpo_table:
  assumes cov: "interval_es_wpo.terminates (declared_global p) p"
      "ctx_vars_cover (prog_cfg p) (interval_es_wpo.ctx_succ (declared_global p) p) []
...
proof (rule sound_table_of_activation
    [where R = "interval_es_wpo.admitted_contexts (declared_global p) p" and rc = "[]",
     OF _ _ _ interval_classify_check_proved interval_classify_check_refuted], goal_cases)
  case (1 u)
  show ?case by (simp add: interval_es_wpo.entry_state_ltr_collect_eq_Union_of_cover [OF cov])
next
  case (2 u ctx)
  show ?case
...
next
  case 3
  show ?case
    using interval_es_wpo.vars_finite_of_terminates [OF cov(1)]
    by (simp add: finite_analysis_result_def interval_es_wpo.result_def
        interval_es_wpo.sol_vars_def)
qed

lemma int_es_join_table:
  assumes cov: "int_es_join_terminates (declared_global p) p"
      "ctx_vars_cover (prog_cfg p) (int_es_join_ctx_succ (declared_global p) p) []
...
proof (rule sound_table_of_activation
  ...
  case (1 u)
  show ?case by (simp add: analyse_int_entry_state_ltr_collect_eq_Union_of_cover [OF cov])
next
  case (2 u ctx)
  show ?case
...
next
  case 3
  show ?case
    using analyse_int_entry_state_vars_finite [OF cov(1)]
...
lemma interval_cs_join_table:
  assumes cov: "interval_cs_join_terminates k (declared_global p) p"
      "ctx_vars_cover (prog_cfg p) (interval_cs_join_ctx_succ k (declared_global p) p) []
...
next
  case 3
  show ?case
    using analyse_interval_call_string_vars_finite_join [OF cov(1)]
...
lemma interval_cs_po_table:
  assumes cov: "interval_cs_po_terminates k (declared_global p) p"
      "ctx_vars_cover (prog_cfg p) (interval_cs_po_ctx_succ k (declared_global p) p) []
...
next
  case 3
  show ?case
    using analyse_interval_call_string_vars_finite_po [OF cov(1)]
    by (simp add: finite_analysis_result_def routed_dg_pipeline.result_def
        routed_dg_pipeline.sol_vars_def)
qed

lemma interval_cs_wpo_table:
  assumes cov: "interval_cs_wpo_terminates k (declared_global p) p"
      "ctx_vars_cover (prog_cfg p) (interval_cs_wpo_ctx_succ k (declared_global p) p) []
...
next
  case 3
  show ?case
    using analyse_interval_call_string_vars_finite_wpo [OF cov(1)]
    by (simp add: finite_analysis_result_def routed_dg_pipeline.result_def
        routed_dg_pipeline.sol_vars_def)
qed

lemma int_cs_join_table:
  assumes cov: "int_cs_join_terminates k (declared_global p) p"
      "ctx_vars_cover (prog_cfg p) (int_cs_join_ctx_succ k (declared_global p) p) []
...
next
  case 3
  show ?case
    using analyse_int_call_string_vars_finite [OF cov(1)]
