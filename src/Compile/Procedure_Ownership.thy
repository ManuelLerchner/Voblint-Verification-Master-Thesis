...
lemma compile_prog_calls_source_stmt:
  assumes "(u, ce, tgt, af) \<in> calls (compile_prog \<Pi> ps mnm main)"
...
proof -
  ...
  next
    case 2
    then have "p = mnm" by (rule compile_proc_entry_mem[OF mainc])
...
lemma compile_prog_entry_frag:
  ...
  shows "(\<forall>u a v. (u, a, v) \<in> intra (compile_prog \<Pi> ps mnm main) \<longrightarrow> u \<in> pfn r m m'
             \<longrightarrow> (u, a, v) \<in> Ep)
       \<and> (\<forall>u ce tgt af. (u, ce, tgt, af) \<in> calls (compile_prog \<Pi> ps mnm main)
...
proof -
  ...
  obtain n1 Eprocs Kprocs n2 Emain Kmain where
    ...
    by (rule compile_prog_intra_split)
  have distinct: "distinct ps" and setps: "set ps = {p. \<Pi> p \<noteq> None} - {mnm}"
    and mnmnotin: "mnm \<notin> set ps"
  ...
  proof (cases "r = mnm")
    ...
    have entmain: "(FunctionEntry mnm, EA_Nop, Statement n1) \<in> intra ?g"
      using EI compile_proc_entry_edge[OF mainc] by simp
    have entr: "(FunctionEntry mnm, EA_Nop, Statement m) \<in> intra ?g" using ent True by simp
    ...
    show ?thesis unfolding True mn mout
      using EI KC compile_procs_intra_src_notin_pfn[OF procs _ mnmnotin]
        compile_procs_calls_src_notin_pfn[OF procs _ mnmnotin] by blast
  next
    case False
    ...
    from compile_procs_member_frag[OF procs distinct rin decl] obtain m0 m0' Er Kr where
      ...
      and ownK: "\<forall>u ce tgt af. (u, ce, tgt, af) \<in> Kprocs \<longrightarrow> u \<in> pfn r m0 m0'
                   \<longrightarrow> (u, ce, tgt, af) \<in> Kr"
      by blast
    have ent0: "(FunctionEntry r, EA_Nop, Statement m0) \<in> intra ?g"
...
lemma frag_okE [elim]:
  assumes "frag_ok \<Pi> ps mnm main u"
...
proof -
  ...
  proof (rule caller_chain_closure)
    ...
    obtain m m' Ep Kp where
      cb: "compile_proc \<Pi> mnm \<lparr>formals = [], body = main\<rparr> m = (m', Ep, Kp)"
  ...
  next
    fix t a v s'
    assume ht: "t \<in> valid_ltr gs ?g S" and ch: "\<forall>u \<in> callers t. frag_ok \<Pi> ps mnm main u"
      and e: "(sink_node t, a, v) \<in> intra ?g"
    have "frag_ok \<Pi> ps mnm main t" using ch callers_refl by blast
    then show "frag_ok \<Pi> ps mnm main (extend t (v, s'))"
  ...
  next
    ...
    show "frag_ok \<Pi> ps mnm main (Call caller
  ...
  next
    ...
    assume cvcallee: "callee \<in> valid_ltr gs ?g S"
      and ch: "\<forall>u \<in> callers callee. frag_ok \<Pi> ps mnm main u"
    ...
    have "frag_ok \<Pi> ps mnm main caller" using ch cin by blast
    then show "frag_ok \<Pi> ps mnm main (Resume caller callee (path caller
