...
(*
  Direct Equation System — AST → equations, no CFG intermediate.

  Alternative to the CFG-based route:
    CFG route:  compile c n  →  cfg record  →  rhs via cfg_edges
    Direct:     predecessors_of c n v        →  rhs directly
  ...
  Why bother?
    ...
    - Opens the possibility of going AST → equations directly without
...
*)
lemma compile_size:
  "compile c n = (n', en, ex, E) ⟹ n' = n + size_com c"
  by (induct c arbitrary: n n' en ex E) (auto simp: Let_def split: prod.splits)

lemma compile_exit:
  "compile c n = (n', en, ex, E) ⟹ ex = exit_of c n"
  by (induct c arbitrary: n n' en ex E) (auto simp: Let_def split: prod.splits)

lemma compile_entry:
  "compile c n = (n', en, ex, E) ⟹ en = n"
  by (induct c arbitrary: n n' en ex E) (auto simp: Let_def split: prod.splits)
...
fun predecessors_of :: "com ⇒ nat ⇒ pp ⇒ (pp × edge_action) set"
where
  ...
  | "predecessors_of (c1 ;; c2) n v =
       (let n1  = n + size_com c1;
        ...
        in     predecessors_of c1 n  v
             ∪ (if v = n1 then {(ex1, EA_Nop)} else {})
             ∪ predecessors_of c2 n1 v)"
  ...
  | "predecessors_of (IF b THEN c1 ELSE c2) n v =
       (let n1  = n + 1;
        ...
        in     (if v = en1 then {(n, EA_Assume b)}    else {})
             ∪ (if v = en2 then {(n, EA_AssumeNot b)} else {})
             ∪ predecessors_of c1 n1 v
             ∪ predecessors_of c2 n2 v
             ∪ (if v = xn  then {(ex1, EA_Nop), (ex2, EA_Nop)} else {}))"
  ...
  | "predecessors_of (WHILE b DO c) n v =
       (let en_body = n + 1;
        ...
        in     (if v = en_body then {(n, EA_Assume b)}    else {})
             ∪ (if v = xn      then {(n, EA_AssumeNot b)} else {})
             ∪ predecessors_of c (n + 1) v
             ∪ (if v = n       then {(ex_body, EA_Nop)}   else {}))"
...
lemma predecessors_eq_compile:
  "compile c n = (n', en, ex, E) ⟹
   predecessors_of c n v = {(u, a) | u a. (u, a, v) ∈ E}"
  sorry
...
where
  "direct_rhs c n tf join_abs bot_abs s0 env v =
     (let preds = predecessors_of c n v;
          vals  = (λ(u, a). apply_tf tf a (env u)) ` preds;
...
theorem direct_rhs_eq_cfg_rhs:
  ...
  shows "direct_rhs c n tf join_abs bot_abs s0 env v
       = rhs (⦇cfg_entry = en, cfg_exit = ex, cfg_edges = E⦈)
             tf join_abs bot_abs s0 env v"
  (*
    Both definitions reduce to abs_join_set over the same set of values.
    Key steps:
      predecessors_eq_compile[OF comp] : predecessors_of c n v = {(u,a) | (u,a,v)∈E}
...
lemma direct_rhs_mono:
  ...
  assumes le:   "∀v. env1 v ≤ env2 v"
  shows "direct_rhs c n tf join_abs bot_abs s0 env1 v
       ≤ direct_rhs c n tf join_abs bot_abs s0 env2 v"
  using direct_rhs_eq_cfg_rhs[OF comp]
        rhs_mono[where g = "⦇cfg_entry = en, cfg_exit = ex, cfg_edges = E⦈",
                 simplified, OF fin cfu le]
  by simp
...
where
  "direct_tree c n tf join_abs bot_abs s0 v =
     (let preds = predecessors_of c n v;
          ps    = SOME xs. distinct xs ∧ set xs = preds;
...
lemma direct_tree_eq_make_rhs_tree:
  ...
  shows "direct_tree c n tf join_abs bot_abs s0 v
       = make_rhs_tree
           (⦇cfg_entry = en, cfg_exit = ex, cfg_edges = E⦈)
           tf join_abs bot_abs s0 v"
  unfolding direct_tree_def make_rhs_tree_def Let_def
  using predecessors_eq_compile[OF comp] compile_entry[OF comp]
  by (simp add: entry_of.simps)
...
where
  "direct_analyse c tf join_abs bot_abs s0 =
     (let T  = direct_tree c 0 tf join_abs bot_abs s0;
          σ  = TD_plain_Interp_solve T 0
      in  (λv. lookup_bot σ v))"
...
theorem direct_eq_cfg_analyse:
  ...
  (*
    ...
    2. compile_entry  gives en = 0;
       to_cfg_def     gives to_cfg c = ⦇cfg_entry=0, cfg_exit=ex, cfg_edges=E⦈.
    ...
    5. lookup_bot applied to the same σ is equal.
  *)
  sorry
