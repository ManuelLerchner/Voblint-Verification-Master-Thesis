...
theorem source_sound_from_collecting_cap:
  ...
  assumes wf: "wf_compile_input source_global Pi ps mnm main"
    ...
    and cap: "\<And>v ctx. activation_collect \<Gamma> source_global admiss startcontext (compile_prog \<Gamma> Pi ps mnm main) S v ctx
                       \<subseteq> gammaM (sg (Inl (v, ctx)))"
  shows "\<exists>v stk t c. csim \<Gamma> Pi (compile_prog \<Gamma> Pi ps mnm main) (residual, s, frs, rk) (v, s, stk)
...
proof -
  ...
  from source_store_in_activation_collect
         [where mnm=mnm and admiss=admiss and startcontext=startcontext,
          OF wf s0 sty0 run tot]
  obtain v stk t c where m: "csim \<Gamma> Pi (compile_prog \<Gamma> Pi ps mnm main) (residual, s, frs, rk) (v, s, stk)"
    and ck: "ctx_key admiss startcontext t c"
    and mem: "s \<in> activation_collect \<Gamma> source_global admiss startcontext ?g S v c"
    by blast
...
theorem source_sound_toplevel_from_collecting_cap:
  ...
  assumes wf: "wf_compile_input source_global Pi ps mnm main"
    ...
    and cap: "\<And>v ctx. activation_collect \<Gamma> source_global admiss startcontext (compile_prog \<Gamma> Pi ps mnm main) S v ctx
                       \<subseteq> gammaM (sg (Inl (v, ctx)))"
  shows "\<exists>v. csim \<Gamma> Pi (compile_prog \<Gamma> Pi ps mnm main) (residual, s, [], rk) (v, s, [])
             \<and> s \<in> gammaM (sg (Inl (v, startcontext)))"
proof -
  let ?g = "compile_prog \<Gamma> Pi ps mnm main"
  from source_toplevel_in_activation_collect
         [where mnm=mnm and admiss=admiss and startcontext=startcontext, OF wf s0 sty0 run]
  obtain v where m: "csim \<Gamma> Pi (compile_prog \<Gamma> Pi ps mnm main) (residual, s, [], rk) (v, s, [])"
    and mem: "s \<in> activation_collect \<Gamma> source_global admiss startcontext ?g S v startcontext" by blast
...
theorem source_activation_sound:
  ...
  assumes wf: "wf_compile_input source_global Pi ps mnm main"
    ...
    and sty0: "styped \<Gamma> s0"
    and run: "star (pstep \<Gamma> source_global Pi) (main, s0, [], proc_ret_kind Pi mnm) (residual, s, frs, rk)"
    and ENTRY_G: "\<And>x. x \<in> S \<Longrightarrow> x \<in> \<lbrakk>sg (Inl (cfg_entry (compile_prog \<Gamma> Pi ps mnm main), startcontext))\<rbrakk>"
    and EDGE: "\<And>u a v c x x'. (u, a, v) \<in> intra (compile_prog \<Gamma> Pi ps mnm main)
    ...
    and ADMISS_TOTAL: "\<And>u c x. \<exists>c'. admiss u c x c'"
    and CALL: "\<And>u dst pars args p cont c x c'.
        (u, CallEdge dst pars args, FunctionEntry p, cont) \<in> calls (compile_prog \<Gamma> Pi ps mnm main)
        \<Longrightarrow> x \<in> \<lbrakk>sg (Inl (u, c))\<rbrakk>
        \<Longrightarrow> admiss u c (call_enter source_global (CallEdge dst pars args) x) c'
        \<Longrightarrow> call_enter source_global (CallEdge dst pars args) x \<in> \<lbrakk>sg (Inl (FunctionEntry p, c'))\<rbrakk>"
    and COMB: "\<And>cl dst pars args p cont c1 c2 x t es.
        (cl, CallEdge dst pars args, FunctionEntry p, cont) \<in> calls (compile_prog \<Gamma> Pi ps mnm main)
        \<Longrightarrow> x \<in> \<lbrakk>sg (Inl (cl, c1))\<rbrakk> \<Longrightarrow> admiss cl c1 es c2 \<Longrightarrow> t \<in> \<lbrakk>sg (Inl (FunctionResult p, c2))\<rbrakk>
        \<Longrightarrow> call_enter_store \<Gamma> source_global (compile_prog \<Gamma> Pi ps mnm main) cl x es
        \<Longrightarrow> combine_collect source_global dst x t \<in> \<lbrakk>sg (Inl (cont, c1))\<rbrakk>"
  shows "\<exists>v stk t c. csim \<Gamma> Pi (compile_prog \<Gamma> Pi ps mnm main) (residual, s, frs, rk) (v, s, stk)
                   \<and> ctx_key admiss startcontext t c
                   \<and> s \<in> \<lbrakk>sg (Inl (v, c))\<rbrakk>"
proof -
  have cap: "\<And>v ctx. activation_collect \<Gamma> source_global admiss startcontext (compile_prog \<Gamma> Pi ps mnm main) S v ctx
  ...
  show ?thesis
    by (rule source_sound_from_collecting_cap[where mnm=mnm and gammaM=gamma_state,
          OF wf s0 sty0 run ADMISS_TOTAL cap])
qed
...
theorem source_reaches_ltr_collect:
  ...
  assumes wf: "wf_compile_input source_global Pi ps mnm main"
    ...
    and sty0: "styped \<Gamma> s0"
    and run: "star (pstep \<Gamma> source_global Pi) (main, s0, [], proc_ret_kind Pi mnm) (residual, s, frs, rk)"
  shows "\<exists>v stk. csim \<Gamma> Pi (compile_prog \<Gamma> Pi ps mnm main) (residual, s, frs, rk) (v, s, stk)
                 \<and> s \<in> ltr_collect \<Gamma> source_global (compile_prog \<Gamma> Pi ps mnm main) S v"
proof -
  let ?g = "compile_prog \<Gamma> Pi ps mnm main"
  have tot: "\<And>u c s. \<exists>c'. admiss_exact (\<lambda>_ _ _. ()) u c s c'" by (simp add: admiss_exact_def)
  from source_store_in_activation_collect
         [where mnm=mnm and admiss = "admiss_exact (\<lambda>_ _ _. ())" and startcontext = "()",
          OF wf s0 sty0 run tot]
  obtain v stk t c where m: "csim \<Gamma> Pi (compile_prog \<Gamma> Pi ps mnm main) (residual, s, frs, rk) (v, s, stk)"
    and mem: "s \<in> activation_collect \<Gamma> source_global (admiss_exact (\<lambda>_ _ _. ())) () ?g S v c" by blast
  have "s \<in> ltr_collect \<Gamma> source_global ?g S v"
...
theorem source_completes_valid_ltr_result:
  ...
  assumes wf: "wf_compile_input source_global Pi ps mnm main"
    and s0: "s0 \<in> S"
    and sty0: "styped \<Gamma> s0"
    and run: "star (pstep \<Gamma> source_global Pi) (main, s0, [], proc_ret_kind Pi mnm) (SKIP, s, [], rk)"
  shows "\<exists>t p. t \<in> valid_ltr \<Gamma> source_global (compile_prog \<Gamma> Pi ps mnm main) S
               \<and> caller_of t = None
               \<and> fst (hd (path t)) = cfg_entry (compile_prog \<Gamma> Pi ps mnm main)
               \<and> sink_node t = FunctionResult p
               \<and> sink_store t = s"
proof -
  let ?g = "compile_prog \<Gamma> Pi ps mnm main"
  from source_run_has_ltr[OF wf s0 sty0 run]
  obtain v stk t where sim: "csim \<Gamma> Pi ?g (SKIP, s, [], rk) (v, s, stk)"
    and rep: "ltr_repr \<Gamma> source_global ?g S (v, s, stk) t" by blast
  have stk0: "stk = []" using csim_Nil_baseD[OF sim] by simp
  from rep stk0 have tv: "t \<in> valid_ltr \<Gamma> source_global ?g S" and sn: "sink_node t = v"
  ...
  from sim obtain p c0 k n where
    ca: "control_at Pi p c0 k n SKIP v" and cat: "compiled_at \<Gamma> Pi ?g p c0 k n"
  ...
  from cat obtain n' en E K where
    cc: "compile \<Gamma> Pi p c0 k n = (n', en, E, K)" and Esub: "E \<subseteq> intra ?g"
    and ret: "(k, EA_Ret None p (proc_ret_kind Pi p), FunctionResult p) \<in> intra ?g"
  ...
  obtain t' where t'v: "t' \<in> valid_ltr \<Gamma> source_global ?g S" and t'n: "sink_node t' = FunctionResult p"
    and t's: "sink_store t' = s" and t'c: "caller_of t' = caller_of t"
    and t'h: "fst (hd (path t')) = fst (hd (path t))" by blast
  show ?thesis
  proof (intro exI conjI)
    show "t' \<in> valid_ltr \<Gamma> source_global ?g S" by (rule t'v)
...
corollary source_completes_ltr_collect_exit:
  ...
  assumes wf: "wf_compile_input source_global Pi ps mnm main"
    ...
    and sty0: "styped \<Gamma> s0"
    and run: "star (pstep \<Gamma> source_global Pi) (main, s0, [], proc_ret_kind Pi mnm) (SKIP, s, [], rk)"
  shows "s \<in> ltr_collect \<Gamma> source_global (compile_prog \<Gamma> Pi ps mnm main) S
              (cfg_exit (compile_prog \<Gamma> Pi ps mnm main))"
proof -
  let ?g = "compile_prog \<Gamma> Pi ps mnm main"
  from source_completes_valid_ltr_result[OF wf s0 sty0 run]
  obtain t p where tv: "t \<in> valid_ltr \<Gamma> source_global ?g S" and hd_t: "fst (hd (path t)) = cfg_entry ?g"
