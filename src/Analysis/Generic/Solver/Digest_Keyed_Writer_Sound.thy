...
proof (rule part_post_solution_st_to_abs_transport[OF _ _ _ pp_st])
  fix v \<sigma>
  show "fun_of_st (eq (side_cfg_T_eff_digest_st dg_st cmb_st g etf_st fresh_frame_st bot0_st s0_st) v \<sigma>)
        = eq (side_cfg_T_eff_digest dg_abs cmb_abs g etf
                (fun_of_st fresh_frame_st) (fun_of_st bot0_st) (fun_of_st s0_st)) v (\<lambda>k. fun_of_st (\<sigma> k))"
    using fun_of_st_eq_side_cfg_T_eff_digest_st[where v="fst v" and ctx="snd v"]
    by (cases v) simp
next
  fix v \<sigma> k
  show "fun_of_st (sides_of_rhs (side_cfg_T_eff_digest_st dg_st cmb_st g etf_st fresh_frame_st bot0_st s0_st v) \<sigma> k)
        = sides_of_rhs (side_cfg_T_eff_digest dg_abs cmb_abs g etf
                (fun_of_st fresh_frame_st) (fun_of_st bot0_st) (fun_of_st s0_st) v) (\<lambda>k. fun_of_st (\<sigma> k)) k"
    using fun_of_st_sides_side_cfg_T_eff_digest_st[where v="fst v" and ctx="snd v" and k=k]
    by (cases v) simp
next
  fix v \<sigma>
  show "dep_aux \<sigma> (side_cfg_T_eff_digest_st dg_st cmb_st g etf_st fresh_frame_st bot0_st s0_st v)
        = dep_aux (\<lambda>k. fun_of_st (\<sigma> k)) (side_cfg_T_eff_digest dg_abs cmb_abs g etf
