...
| `DG_Framework.thy` | D/G framework core: the `dg_state` solver adapter, heterogeneous `dg_edge_tree`/`dg_combine_tree`/`dg_spec`, seeded CMP generator, slot-purity guarantees, and the homogeneous `unit_dg_spec` specialization |
| `DG_Soundness.thy` | canonical heterogeneous soundness locale, solver post-fixpoint extraction, collecting endpoint, independent-transfer instance, and homogeneous interpretation |
| `Retain_Analysis.thy` | retain analysis (`D` = locals x snapshot, `G` = globals): homogeneous sound analysis, keyed snapshot invariant, executable mirror, and ordinary heterogeneous `retain_dg_spec` instance |
