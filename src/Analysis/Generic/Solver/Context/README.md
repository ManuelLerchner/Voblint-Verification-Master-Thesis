...
| `Retain_Analysis.thy` | Goblint-faithful boundary: `step_edge_tree` (value-opaque framework edge shape), `unit_step`/`retain_step` recovering the unit/retain trees definitionally, `dg_state` componentwise lattice copy type, and the retain *analysis* with `D` = locals x snapshot proven equivalent to `retain_edge_tree` (see `docs/SPLIT_STATE_MIGRATION.md` §6) |
