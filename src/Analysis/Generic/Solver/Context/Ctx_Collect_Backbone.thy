...
theorem trace_ctx_sound_meaning:
  ...
  assumes ENTRY: "\<And>ctx s. s \<in> S \<Longrightarrow> cmp (dg [s]) ctx \<Longrightarrow> s \<in> M (cfg_entry g, ctx)"
    ...
    and COMB: "\<And>ctx cl ex v dst tau rho. (cl, ex, v, dst) \<in> combines g
        ...
        \<Longrightarrow> combine_collect dst (last tau) (last rho) \<in> M (v, ctx)"
...
proof -
  ...
  next
    ...
    thus ?case by simp
  next
    case (combine cl ex v dst tau rho r)
    ...
    have ctau: "cmp (dg tau) ctx"
      using combine.prems DG_RETURN[OF tau_ne, of rho dst] combine.hyps(4) by simp
    have caller_sound: "last tau \<in> M (cl, ctx)"
      using combine.IH(1)[OF ctau] .
    have crho: "cmp (dg rho) (rt cl ctx (rd (cl, ctx)))"
    proof -
      have "dg rho = entdg (last tau)" using DG_CALLEE[OF rho_ne combine.hyps(5)] .
    ...
    have "combine_collect dst (last tau) (last rho) \<in> M (v, ctx)"
      using COMB[OF combine.hyps(1) caller_sound callee_sound] .
    thus ?case
      using combine.hyps(4) by simp
  qed
