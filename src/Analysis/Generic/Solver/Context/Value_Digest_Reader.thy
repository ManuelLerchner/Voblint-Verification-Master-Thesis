...
text \<open>
  ...
  The digest is a \<^class>\<open>finite\<close> key carried in a ghost local variable and read back
  out of the local slot via a domain-supplied \<open>decode\<close>.

  The locale fixes only \<open>decode\<close> (the projection from an abstract value to the finite
  digest) and \<open>ghost\<close> (the ghost variable name); everything else --- the read, its
...
definition vd_reader ::
  ...
  "vd_reader \<sigma> v ctx = decode (\<sigma> (Inl (v, ctx)) ghost)"
...
lemma vd_obs_reduce:
  "vd_obs \<sigma> (v, ctx)
     = \<sigma> (Inl (v, ctx)) \<squnion> \<sigma> (Inr (decode (\<sigma> (Inl (v, ctx)) ghost)))"
...
lemma vd_obs_global:
  ...
  shows "vd_obs \<sigma> (v, ctx) x = \<sigma> (Inr (decode (\<sigma> (Inl (v, ctx)) ghost))) x"
proof -
...
text \<open>
  ...
  two slot invariants and digest stability \<open>DIGEST_AGREE\<close> --- the ghost read at a
...
theorem vd_collect_ctx_sound_bot_reduced:
  ...
  assumes ENTRY: "\<And>ctx s. s \<in> S \<Longrightarrow> cmp (dg [s]) ctx \<Longrightarrow> s \<in> \<lbrakk>vd_obs sigma (cfg_entry g, ctx)\<rbrakk>"
    ...
    and DIGEST_AGREE: "\<And>ctx cl ex v. (cl, ex, v) \<in> combines g
        \<Longrightarrow> decode (sigma (Inl (ex, rt cl ctx (sigma (Inl (cl, ctx))))) ghost)
              = decode (sigma (Inl (v, ctx)) ghost)"
    and DG_INTRA: "\<And>tr s' ctx. tr \<noteq> [] \<Longrightarrow> cmp (dg (tr @ [s'])) ctx \<Longrightarrow> cmp (dg tr) ctx"
    and DG_RETURN: "\<And>tau rho. tau \<noteq> [] \<Longrightarrow> dg (tau @ tl rho @ [<last tau|last rho>]) = dg tau"
    and DG_CALLEE: "\<And>tau rho. rho \<noteq> [] \<Longrightarrow> hd rho = enter_state (last tau) \<Longrightarrow> dg rho = entdg (last tau)"
    and ENTER_MONO: "\<And>ctx cl s. s \<in> \<lbrakk>vd_obs sigma (cl, ctx)\<rbrakk>
...
proof -
  ...
  proof -
    ...
    have ag: "decode (sigma (Inl (ex, rt cl ctx (sigma (Inl (cl, ctx))))) ghost)
                = decode (sigma (Inl (v, ctx)) ghost)"
      by (rule DIGEST_AGREE[OF comb])
    have "vd_obs sigma (ex, rt cl ctx (sigma (Inl (cl, ctx)))) x
            = sigma (Inr (decode (sigma (Inl (ex, rt cl ctx (sigma (Inl (cl, ctx))))) ghost))) x"
      by (rule vd_obs_global[OF INL_BOT gx])
    also have "\<dots> = sigma (Inr (decode (sigma (Inl (v, ctx)) ghost))) x" by (simp add: ag)
...
text \<open>
  Under the alignment invariant \<open>ctx = decode (sigma (Inl (v, ctx)) ghost)\<close> --- the
  context channel carries the ghost digest (Goblint's \<open>C.t = context D\<close>) --- the
...
lemma vd_obs_eq_side_env_cmp:
  ...
  assumes align: "decode (sigma (Inl (v, ctx)) ghost) = ctx"
