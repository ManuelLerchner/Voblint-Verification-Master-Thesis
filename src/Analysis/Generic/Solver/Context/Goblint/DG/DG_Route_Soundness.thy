...
theorem dg_collect_ctx_sound:
  ...
  assumes ENTRY: "\<And>ctx s. s \<in> S0 \<Longrightarrow> cmp (dg [s]) ctx \<Longrightarrow> s \<in> dg_gamma_c sigma ctx (cfg_entry g)"
    and PROC_ENTRY: "\<And>ctx v s xs es. (cfg_entry g, EA_Enter xs es, v) \<in> edges g
        \<Longrightarrow> s \<in> edge_collect (EA_Enter xs es) S0
    ...
    and COMB: "\<And>ctx cl ex v dst tau rho. (cl, ex, v, dst) \<in> combines g
        ...
        \<Longrightarrow> combine_collect dst (last tau) (last rho) \<in> dg_gamma_c sigma ctx v"
...
proof -
  ...
  next
    ...
    have cr': "last tau \<in> dg_gamma_c sigma ctx cl"
      using cr by simp
    have ce': "last rho \<in> dg_gamma_c sigma (rt cl ctx (dg_D_c sigma ctx cl)) ex"
      using ce by simp
    show "combine_collect dst (last tau) (last rho) \<in> (\<lambda>(p, c). dg_gamma_c sigma c p) (v', ctx)"
