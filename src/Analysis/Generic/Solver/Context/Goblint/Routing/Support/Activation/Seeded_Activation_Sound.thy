theory Seeded_Activation_Sound
  imports Activation_Backbone Seeded_Clean_Ctx_Collect
begin

section \<open>Discharging the activation obligations from a seeded post-solution\<close>

text \<open>
  The domain-independent backbone --- \<open>activation_trace_sound\<close> and
  \<open>activation_collect_sound\<close> --- lives in
  \<^theory>\<open>Voblint_Analysis.Activation_Backbone\<close>.  Here the four semantic obligations
  (\<open>ENTRY_G\<close> / \<open>EDGE\<close> / \<open>SEED_G\<close> / \<open>COMB\<close>) are discharged from a
  \<^const>\<open>side_cfg_T_eff_cmp_seed\<close> post-solution over a clean transfer, packaging a
  seeded context-sensitive analysis whose result over-approximates the
  activation-indexed collecting semantics \<^const>\<open>cfg_collect_ctx_act\<close> at every
  \<open>(v, ctx)\<close>.  The context is threaded structurally by the witness, so there is no
  digest-propagation machinery and no \<open>'c :: finite\<close>.
\<close>
