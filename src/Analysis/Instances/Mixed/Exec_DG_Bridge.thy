...
text \<open>
  \<open>placed_hook_se_join_edge\<close> is the join-node counterpart of
  \<open>placed_hook_se_edge\<close>: a node with exactly two intra predecessors, as at a
  loop head or an if/else merge, rather than exactly one. It is fully
  generic in the classifier, owner/scope/policy functions, domain transfer,
  CFG, and seeds -- identical in every parameter to \<open>placed_hook_se_edge\<close>
  except that the single predecessor \<open>(u, a)\<close> becomes two, \<open>(u1, a1)\<close> and
  \<open>(u2, a2)\<close>, each with its own transport-agreement hypothesis. The
  generator's own fold law (\<open>side_cfg_T_eff_keyed_seed_trees_two_edges\<close>)
  reduces the join to a plain sup of the two edges' individually-projected
  contributions, so the proof reuses \<open>placed_hook_se_edge\<close>'s structure
  twice -- once per edge, via \<open>dg_refines_on_project_strict\<close> -- and combines
  the two resulting facts with the existing \<open>dg_refines_on_sup\<close>
  (@{thm dg_refines_on_sup}, already proved generically above) before
  finishing exactly as \<open>placed_hook_se_edge\<close> does.
\<close>

lemma placed_hook_se_join_edge:
  fixes gs :: "vname => bool"
    and owner_of :: "pp => pname"
    and locations_of :: "pp => location list"
    and keep_local publish_side :: "scoped_location => bool"
    ...
    and bot0_abs :: "'a abs_state" and s0d_abs s0g_abs :: "'a abs_state" and top_val :: 'a
    and sigma_exec :: "pp \<times> unit + unit => ('a exec_dg_st, 'a exec_dg_st) dg_state"
    and v u1 u2 :: pp and a1 a2 :: edge_action
  defines "sigma_abs \<equiv> completed_sigma_abs gs locations_of top_val sigma_exec"
  assumes not_entry: "v \<noteq> cfg_entry g"
    and pred: "intra_predecessor_list g v = [(u1, a1), (u2, a2)]"
    and no_combine: "return_call_action_list g v = []"
    and no_enter: "entry_call_list g v = []"
    and bot0_eq: "bot0 = bot" and bot0_abs_eq: "bot0_abs = bot"
    and top_ge: "\<And>y. y \<le> top_val"
    ...
    and canonical: "\<And>location. location \<in> set (locations_of v) \<Longrightarrow>
      location = location_of gs (location_vname location)"
    and raw1: "\<And>location. location \<in> set (locations_of v) \<Longrightarrow>
      lookup_resolved_st_q (transfer_st a1 (dg_hook_D sigma_exec u1 \<squnion> dg_hook_G sigma_exec)) location =
      transfer_abs a1 (dg_hook_D sigma_abs u1 \<squnion> dg_hook_G sigma_abs) (location_vname location)"
    and raw2: "\<And>location. location \<in> set (locations_of v) \<Longrightarrow>
      lookup_resolved_st_q (transfer_st a2 (dg_hook_D sigma_exec u2 \<squnion> dg_hook_G sigma_exec)) location =
      transfer_abs a2 (dg_hook_D sigma_abs u2 \<squnion> dg_hook_G sigma_abs) (location_vname location)"
    and side_outside_raw1: "\<And>x. location_of gs x \<notin> set (locations_of v) \<Longrightarrow>
      publish_side (owner_of v, location_of gs x) \<longrightarrow>
      transfer_abs a1 (dg_hook_D sigma_abs u1 \<squnion> dg_hook_G sigma_abs) x \<le> bot"
    and side_outside_raw2: "\<And>x. location_of gs x \<notin> set (locations_of v) \<Longrightarrow>
      publish_side (owner_of v, location_of gs x) \<longrightarrow>
      transfer_abs a2 (dg_hook_D sigma_abs u2 \<squnion> dg_hook_G sigma_abs) x \<le> bot"
  shows "se_constraint_holds
    (placed_abs_dg_gen_of gs owner_of keep_local publish_side transfer_abs enter_abs g
      bot0_abs s0d_abs s0g_abs (v, ())) sigma_abs (v, ())"
proof -
  have exec_eq: "eq (placed_dg_gen_of_strict gs owner_of locations_of keep_local publish_side
        transfer_st enter_st g bot0 s0d s0g) (v, ()) sigma_exec =
      DG (project_resolved_on_strict (owner_of v) (locations_of v) keep_local
            (transfer_st a1 (dg_hook_D sigma_exec u1 \<squnion> dg_hook_G sigma_exec))
          \<squnion> project_resolved_on_strict (owner_of v) (locations_of v) keep_local
            (transfer_st a2 (dg_hook_D sigma_exec u2 \<squnion> dg_hook_G sigma_exec))) bot"
    unfolding placed_dg_gen_of_strict_def
    by (simp add: not_entry pred no_combine no_enter bot0_eq
      side_cfg_T_eff_keyed_seed_trees_def Let_def sides_of_rhs_seqcomp traverse_seqcomp
      traverse_side_rhs_fold_dg sides_of_rhs_side_rhs_fold_dg_acc_indep sup_assoc
      traverse_rhs_placed_dg_edge_of_strict dg_hook_D_def dg_hook_G_def)
  have exec_sides: "sides_of_rhs (placed_dg_gen_of_strict gs owner_of locations_of keep_local publish_side
        transfer_st enter_st g bot0 s0d s0g (v, ())) sigma_exec (Inr ()) =
      DG bot (project_resolved_on_strict (owner_of v) (locations_of v) publish_side
            (transfer_st a1 (dg_hook_D sigma_exec u1 \<squnion> dg_hook_G sigma_exec))
          \<squnion> project_resolved_on_strict (owner_of v) (locations_of v) publish_side
            (transfer_st a2 (dg_hook_D sigma_exec u2 \<squnion> dg_hook_G sigma_exec)))"
    unfolding placed_dg_gen_of_strict_def
    by (simp add: not_entry pred no_combine no_enter bot0_eq
      side_cfg_T_eff_keyed_seed_trees_def Let_def sides_of_rhs_seqcomp traverse_seqcomp
      traverse_side_rhs_fold_dg sides_of_rhs_side_rhs_fold_dg_acc_indep sup_assoc
      sides_of_rhs_placed_dg_edge_of_strict dg_hook_D_def dg_hook_G_def
      sup_dg_state_def bot_dg_state_def)
  have abs_eq: "eq (placed_abs_dg_gen_of gs owner_of keep_local publish_side transfer_abs enter_abs g
        bot0_abs s0d_abs s0g_abs) (v, ()) sigma_abs =
      DG (project_abs_on (owner_of v) gs keep_local
            (transfer_abs a1 (dg_hook_D sigma_abs u1 \<squnion> dg_hook_G sigma_abs))
          \<squnion> project_abs_on (owner_of v) gs keep_local
            (transfer_abs a2 (dg_hook_D sigma_abs u2 \<squnion> dg_hook_G sigma_abs))) bot"
    unfolding placed_abs_dg_gen_of_def
    by (simp add: not_entry pred no_combine no_enter bot0_abs_eq
      side_cfg_T_eff_keyed_seed_trees_def Let_def sides_of_rhs_seqcomp traverse_seqcomp
      traverse_side_rhs_fold_dg sides_of_rhs_side_rhs_fold_dg_acc_indep sup_assoc
  ...
  have abs_sides: "sides_of_rhs (placed_abs_dg_gen_of gs owner_of keep_local publish_side transfer_abs enter_abs g
      ...
      DG bot (project_abs_on (owner_of v) gs publish_side
            (transfer_abs a1 (dg_hook_D sigma_abs u1 \<squnion> dg_hook_G sigma_abs))
          \<squnion> project_abs_on (owner_of v) gs publish_side
            (transfer_abs a2 (dg_hook_D sigma_abs u2 \<squnion> dg_hook_G sigma_abs)))"
    unfolding placed_abs_dg_gen_of_def
    by (simp add: not_entry pred no_combine no_enter bot0_abs_eq
      side_cfg_T_eff_keyed_seed_trees_def Let_def sides_of_rhs_seqcomp traverse_seqcomp
      traverse_side_rhs_fold_dg sides_of_rhs_side_rhs_fold_dg_acc_indep sup_assoc
      sides_of_rhs_placed_abs_dg_edge_of dg_hook_D_def dg_hook_G_def
      sup_dg_state_def bot_dg_state_def)
  have bridge1: "dg_refines_on (set (locations_of v))
      (DG (project_resolved_on_strict (owner_of v) (locations_of v) keep_local
            (transfer_st a1 (dg_hook_D sigma_exec u1 \<squnion> dg_hook_G sigma_exec)))
        (project_resolved_on_strict (owner_of v) (locations_of v) publish_side
            (transfer_st a1 (dg_hook_D sigma_exec u1 \<squnion> dg_hook_G sigma_exec))))
      (DG (project_abs_on (owner_of v) gs keep_local
        ...
        (project_abs_on (owner_of v) gs publish_side
            (transfer_abs a1 (dg_hook_D sigma_abs u1 \<squnion> dg_hook_G sigma_abs))))"
    by (rule dg_refines_on_project_strict[OF raw1 canonical])
  have bridge2: "dg_refines_on (set (locations_of v))
      (DG (project_resolved_on_strict (owner_of v) (locations_of v) keep_local
            (transfer_st a2 (dg_hook_D sigma_exec u2 \<squnion> dg_hook_G sigma_exec)))
        (project_resolved_on_strict (owner_of v) (locations_of v) publish_side
            (transfer_st a2 (dg_hook_D sigma_exec u2 \<squnion> dg_hook_G sigma_exec))))
      (DG (project_abs_on (owner_of v) gs keep_local
        ...
        (project_abs_on (owner_of v) gs publish_side
            (transfer_abs a2 (dg_hook_D sigma_abs u2 \<squnion> dg_hook_G sigma_abs))))"
    by (rule dg_refines_on_project_strict[OF raw2 canonical])
  have bridge: "dg_refines_on (set (locations_of v))
      (DG (project_resolved_on_strict (owner_of v) (locations_of v) keep_local
            (transfer_st a1 (dg_hook_D sigma_exec u1 \<squnion> dg_hook_G sigma_exec)))
        (project_resolved_on_strict (owner_of v) (locations_of v) publish_side
            (transfer_st a1 (dg_hook_D sigma_exec u1 \<squnion> dg_hook_G sigma_exec)))
       \<squnion> DG (project_resolved_on_strict (owner_of v) (locations_of v) keep_local
            (transfer_st a2 (dg_hook_D sigma_exec u2 \<squnion> dg_hook_G sigma_exec)))
        (project_resolved_on_strict (owner_of v) (locations_of v) publish_side
            (transfer_st a2 (dg_hook_D sigma_exec u2 \<squnion> dg_hook_G sigma_exec))))
      (DG (project_abs_on (owner_of v) gs keep_local
        ...
        (project_abs_on (owner_of v) gs publish_side
            (transfer_abs a1 (dg_hook_D sigma_abs u1 \<squnion> dg_hook_G sigma_abs)))
       \<squnion> DG (project_abs_on (owner_of v) gs keep_local
            (transfer_abs a2 (dg_hook_D sigma_abs u2 \<squnion> dg_hook_G sigma_abs)))
        (project_abs_on (owner_of v) gs publish_side
            (transfer_abs a2 (dg_hook_D sigma_abs u2 \<squnion> dg_hook_G sigma_abs))))"
    by (rule dg_refines_on_sup[OF bridge1 bridge2])
  have dg_ref: "dg_refines_on (set (locations_of v))
      (DG (locals (eq (placed_dg_gen_of_strict gs owner_of locations_of keep_local publish_side
              transfer_st enter_st g bot0 s0d s0g) (v, ()) sigma_exec))
        (globs (sides_of_rhs (placed_dg_gen_of_strict gs owner_of locations_of keep_local publish_side
      ...
      (DG (locals (eq (placed_abs_dg_gen_of gs owner_of keep_local publish_side transfer_abs enter_abs g
        ...
        (globs (sides_of_rhs (placed_abs_dg_gen_of gs owner_of keep_local publish_side transfer_abs enter_abs g
              bot0_abs s0d_abs s0g_abs (v, ())) sigma_abs (Inr ()))))"
    using bridge by (simp add: exec_eq exec_sides abs_eq abs_sides sup_dg_state_def)
  have exec_le: "locals (eq (placed_dg_gen_of_strict gs owner_of locations_of keep_local publish_side
        transfer_st enter_st g bot0 s0d s0g) (v, ()) sigma_exec) \<le>
      locals (sigma_exec (Inl (v, ())))"
    using se_constraint_holds_local[OF se] by (simp add: less_eq_dg_state_def)
  ...
  have side_outside_le: "\<And>x. location_of gs x \<notin> set (locations_of v) \<Longrightarrow>
      globs (sides_of_rhs (placed_abs_dg_gen_of gs owner_of keep_local publish_side transfer_abs enter_abs g
        bot0_abs s0d_abs s0g_abs (v, ())) sigma_abs (Inr ())) x \<le> bot"
    by (simp add: abs_sides project_abs_on_def project_component_def side_outside_raw1 side_outside_raw2)
  have dg_ref_side: "\<And>location. location \<in> set (locations_of v) \<Longrightarrow>
      lookup_resolved_st_q (globs (sides_of_rhs (placed_dg_gen_of_strict gs owner_of locations_of keep_local publish_side
          transfer_st enter_st g bot0 s0d s0g (v, ())) sigma_exec (Inr ()))) location =
      globs (sides_of_rhs (placed_abs_dg_gen_of gs owner_of keep_local publish_side transfer_abs enter_abs g
