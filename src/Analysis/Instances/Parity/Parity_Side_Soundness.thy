...
lemma parity_etf_combine_tree:
  "etf_combine_collect (parity_etf gs) ci cc ex = unit_combine_tree gs (combine\<^sup># gs (ci_dst ci)) cc ex"
...
theorem side_parity_analysis_sound:
  ...
  assumes side_solve_dom:
    "side_cfg_solve_dom_eff gs (compile_prog \<Pi> ps mnm main) (parity_etf gs) bot s0 ()
       (cfg_exit (compile_prog \<Pi> ps mnm main))"
  shows "t \<in> gamma_state_lift (side_analyse_eff gs \<Pi> ps mnm main (parity_etf gs) bot s0 ()
         (cfg_exit (compile_prog \<Pi> ps mnm main)))"
proof -
  ...
  have collect:
    "ltr_collect gs (compile_prog \<Pi> ps mnm main) {s}
       (cfg_exit (compile_prog \<Pi> ps mnm main))
     \<le> gamma_state_lift (side_analyse_eff gs \<Pi> ps mnm main (parity_etf gs) bot s0 ()
           (cfg_exit (compile_prog \<Pi> ps mnm main)))"
