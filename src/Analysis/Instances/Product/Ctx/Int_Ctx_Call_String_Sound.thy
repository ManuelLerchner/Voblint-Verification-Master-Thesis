...
definition ics_eqs ::
    "nat \<Rightarrow> refine_mode \<Rightarrow> (vname \<Rightarrow> bool) \<Rightarrow> (int_dom exec_dg_st \<Rightarrow> bool) \<Rightarrow> tyenv \<Rightarrow> proc_table \<Rightarrow> pname list
...
definition ics_sol ::
  ...
  "ics_sol k mode gs is_bot_pred \<Gamma> Pi ps mnm main =
     TD_side_always_join_Interp_solve (ics_eqs k mode gs is_bot_pred \<Gamma> Pi ps mnm main)
       (cfg_exit (compile_prog \<Gamma> Pi ps mnm main), [])"

definition ics_terminates ::
    "nat \<Rightarrow> refine_mode \<Rightarrow> (vname \<Rightarrow> bool) \<Rightarrow> (int_dom exec_dg_st \<Rightarrow> bool) \<Rightarrow> tyenv \<Rightarrow> proc_table \<Rightarrow> pname list
       \<Rightarrow> pname \<Rightarrow> com \<Rightarrow> bool" where
  "ics_terminates k mode gs is_bot_pred \<Gamma> Pi ps mnm main =
     TD_side_always_join_Interp.solve_dom TYPE(call_string_gk) TYPE((int_dom exec_dg_st lifted, int_dom exec_dg_st lifted) dg_state)
       (ics_eqs k mode gs is_bot_pred \<Gamma> Pi ps mnm main) (cfg_exit (compile_prog \<Gamma> Pi ps mnm main), [])"

lemma ics_terminates_via_solve_c:
  assumes "TD_side_always_join_Interp_solve_c (ics_eqs k mode gs is_bot_pred \<Gamma> Pi ps mnm main)
             (cfg_exit (compile_prog \<Gamma> Pi ps mnm main), []) \<noteq> None"
  shows "ics_terminates k mode gs is_bot_pred \<Gamma> Pi ps mnm main"
...
definition ics_sol_warrow ::
    "nat \<Rightarrow> refine_mode \<Rightarrow> (vname \<Rightarrow> bool) \<Rightarrow> (int_dom exec_dg_st \<Rightarrow> bool) \<Rightarrow> tyenv \<Rightarrow> proc_table \<Rightarrow> pname list
       \<Rightarrow> pname \<Rightarrow> com
       \<Rightarrow> (pp \<times> call_string) set \<times> (pp \<times> call_string + call_string_gk \<Rightarrow> (int_dom exec_dg_st lifted, int_dom exec_dg_st lifted) dg_state)" where
  "ics_sol_warrow k mode gs is_bot_pred \<Gamma> Pi ps mnm main =
     TD_side_warrowing_apinis_Interp_solve (ics_eqs k mode gs is_bot_pred \<Gamma> Pi ps mnm main)
       (cfg_exit (compile_prog \<Gamma> Pi ps mnm main), [])"

definition ics_terminates_warrow ::
    "nat \<Rightarrow> refine_mode \<Rightarrow> (vname \<Rightarrow> bool) \<Rightarrow> (int_dom exec_dg_st \<Rightarrow> bool) \<Rightarrow> tyenv \<Rightarrow> proc_table \<Rightarrow> pname list
       \<Rightarrow> pname \<Rightarrow> com \<Rightarrow> bool" where
  "ics_terminates_warrow k mode gs is_bot_pred \<Gamma> Pi ps mnm main =
...
lemma ics_terminates_warrow_via_solve_c:
  assumes "TD_side_warrowing_apinis_Interp_solve_c (ics_eqs k mode gs is_bot_pred \<Gamma> Pi ps mnm main)
             (cfg_exit (compile_prog \<Gamma> Pi ps mnm main), []) \<noteq> None"
  shows "ics_terminates_warrow k mode gs is_bot_pred \<Gamma> Pi ps mnm main"
...
context
  fixes mode :: refine_mode and gs :: "vname \<Rightarrow> bool" and is_bot_pred :: "int_dom exec_dg_st \<Rightarrow> bool"
    and \<Gamma> :: tyenv and Pi :: proc_table and ps :: "pname list" and mnm :: pname and main :: com and k :: nat
  assumes solves: "ics_terminates k mode gs is_bot_pred \<Gamma> Pi ps mnm main"
...
lemma ics_pp_st:
  "part_post_solution (ics_eqs k mode gs is_bot_pred \<Gamma> Pi ps mnm main) (cfg_exit (compile_prog \<Gamma> Pi ps mnm main), [])
     (snd (ics_sol k mode gs is_bot_pred \<Gamma> Pi ps mnm main)) (fst (ics_sol k mode gs is_bot_pred \<Gamma> Pi ps mnm main))"
  using TD_side_always_join_Interp.partial_post_solution
          [OF ics_solve_dom, of "fst (ics_sol k mode gs is_bot_pred \<Gamma> Pi ps mnm main)"
             "snd (ics_sol k mode gs is_bot_pred \<Gamma> Pi ps mnm main)"]
...
theorem ics_pp_abs:
  "part_post_solution
     ...
     (fun_of_dg_st_gen (map_lift (fun_of_resolved_st_q_for gs)) (map_lift (fun_of_resolved_st_q_for gs))
        \<circ> snd (ics_sol k mode gs is_bot_pred \<Gamma> Pi ps mnm main))
     (fst (ics_sol k mode gs is_bot_pred \<Gamma> Pi ps mnm main))"
proof -
...
context
  fixes mode :: refine_mode and gs :: "vname \<Rightarrow> bool" and is_bot_pred :: "int_dom exec_dg_st \<Rightarrow> bool"
    and \<Gamma> :: tyenv and Pi :: proc_table and ps :: "pname list" and mnm :: pname and main :: com and k :: nat
  assumes solves: "ics_terminates_warrow k mode gs is_bot_pred \<Gamma> Pi ps mnm main"
    and exact: "\<And>s. is_bot_pred s = is_bot_state (fun_of_resolved_st_q_for gs s)"
begin
...
lemma ics_pp_st_warrow:
  "part_post_solution (ics_eqs k mode gs is_bot_pred \<Gamma> Pi ps mnm main) (cfg_exit (compile_prog \<Gamma> Pi ps mnm main), [])
     (snd (ics_sol_warrow k mode gs is_bot_pred \<Gamma> Pi ps mnm main)) (fst (ics_sol_warrow k mode gs is_bot_pred \<Gamma> Pi ps mnm main))"
  using TD_side_warrowing_apinis_Interp.partial_post_solution
          [OF ics_solve_dom_warrow, of "fst (ics_sol_warrow k mode gs is_bot_pred \<Gamma> Pi ps mnm main)"
             "snd (ics_sol_warrow k mode gs is_bot_pred \<Gamma> Pi ps mnm main)"]
...
theorem ics_pp_abs_warrow:
  "part_post_solution
     ...
     (cfg_exit (compile_prog \<Gamma> Pi ps mnm main), [])
     (fun_of_dg_st_gen (map_lift (fun_of_resolved_st_q_for gs)) (map_lift (fun_of_resolved_st_q_for gs))
        \<circ> snd (ics_sol_warrow k mode gs is_bot_pred \<Gamma> Pi ps mnm main))
     (fst (ics_sol_warrow k mode gs is_bot_pred \<Gamma> Pi ps mnm main))"
proof -
  have pp_buf: "part_post_solution
       (side_cfg_T_eff_keyed_seed_dg_buffered intra_predecessor_addr_list
          (\<lambda>_. Call_String_Context.Global) (cs_route k)
