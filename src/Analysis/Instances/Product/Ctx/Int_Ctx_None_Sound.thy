...
definition ictx_eqs ::
    "refine_mode \<Rightarrow> (int_dom exec_dg_st \<Rightarrow> bool) \<Rightarrow> (vname \<Rightarrow> bool) \<Rightarrow> tyenv \<Rightarrow> proc_table \<Rightarrow> pname list \<Rightarrow> pname \<Rightarrow> com
       \<Rightarrow> (pp \<times> unit, gk, (int_dom exec_dg_st lifted, int_dom exec_dg_st lifted) dg_state) eqsT" where
  "ictx_eqs mode is_bot_pred gs \<Gamma> Pi ps mnm main =
     side_cfg_T_eff_keyed_seed_dg_buffered intra_predecessor_addr_list (\<lambda>_. Global)
       ...
       (compile_prog \<Gamma> Pi ps mnm main) (ictx_spec mode is_bot_pred gs) Bot (Lifted cinit_int_dom_st) Bot"
...
definition ictx_sol ::
  ...
  "ictx_sol mode is_bot_pred gs \<Gamma> Pi ps mnm main =
     TD_side_always_join_Interp_solve (ictx_eqs mode is_bot_pred gs \<Gamma> Pi ps mnm main)
       (cfg_exit (compile_prog \<Gamma> Pi ps mnm main), ())"

definition ictx_terminates ::
    "refine_mode \<Rightarrow> (int_dom exec_dg_st \<Rightarrow> bool) \<Rightarrow> (vname \<Rightarrow> bool) \<Rightarrow> tyenv \<Rightarrow> proc_table \<Rightarrow> pname list \<Rightarrow> pname \<Rightarrow> com \<Rightarrow> bool" where
  "ictx_terminates mode is_bot_pred gs \<Gamma> Pi ps mnm main =
     TD_side_always_join_Interp.solve_dom TYPE(gk) TYPE((int_dom exec_dg_st lifted, int_dom exec_dg_st lifted) dg_state)
       (ictx_eqs mode is_bot_pred gs \<Gamma> Pi ps mnm main) (cfg_exit (compile_prog \<Gamma> Pi ps mnm main), ())"

lemma ictx_terminates_via_solve_c:
  assumes "TD_side_always_join_Interp_solve_c (ictx_eqs mode is_bot_pred gs \<Gamma> Pi ps mnm main)
             (cfg_exit (compile_prog \<Gamma> Pi ps mnm main), ()) \<noteq> None"
  shows "ictx_terminates mode is_bot_pred gs \<Gamma> Pi ps mnm main"
...
locale ictx_solved =
  fixes ictx_sol :: "refine_mode \<Rightarrow> (int_dom exec_dg_st \<Rightarrow> bool) \<Rightarrow> (vname \<Rightarrow> bool) \<Rightarrow> tyenv \<Rightarrow> proc_table
    ...
    and ictx_terminates :: "refine_mode \<Rightarrow> (int_dom exec_dg_st \<Rightarrow> bool) \<Rightarrow> (vname \<Rightarrow> bool) \<Rightarrow> tyenv \<Rightarrow> proc_table
                         \<Rightarrow> pname list \<Rightarrow> pname \<Rightarrow> com \<Rightarrow> bool"
  assumes pp_st:
    "ictx_terminates mode is_bot_pred gs \<Gamma> Pi ps mnm main
       \<Longrightarrow> part_post_solution (ictx_eqs mode is_bot_pred gs \<Gamma> Pi ps mnm main)
             (cfg_exit (compile_prog \<Gamma> Pi ps mnm main), ())
             (snd (ictx_sol mode is_bot_pred gs \<Gamma> Pi ps mnm main))
             (fst (ictx_sol mode is_bot_pred gs \<Gamma> Pi ps mnm main))"
begin

theorem ictx_pp_abs:
  assumes solves: "ictx_terminates mode is_bot_pred gs \<Gamma> Pi ps mnm main"
  ...
  "part_post_solution
     (side_cfg_T_eff_keyed_seed_dg intra_predecessor_addr_list (\<lambda>_. Global) route_unit
        (routed_cmb_g (ictx_abs_spec mode gs) Global Seed
           (static_resolve (compile_prog \<Gamma> Pi ps mnm main)))
        (routed_extra_g Seed Global)
        (compile_prog \<Gamma> Pi ps mnm main) (ictx_abs_spec mode gs)
     ...
     (fun_of_dg_st_gen (map_lift (fun_of_resolved_st_q_for gs)) (map_lift (fun_of_resolved_st_q_for gs))
        \<circ> snd (ictx_sol mode is_bot_pred gs \<Gamma> Pi ps mnm main))
     (fst (ictx_sol mode is_bot_pred gs \<Gamma> Pi ps mnm main))"
proof -
  have pp_buf: "part_post_solution
       (side_cfg_T_eff_keyed_seed_dg_buffered intra_predecessor_addr_list (\<lambda>_. Global)
          ...
          (routed_cmb_g_contribution (ictx_spec mode is_bot_pred gs) Global Seed
             (static_resolve (compile_prog \<Gamma> Pi ps mnm main)))
          (routed_extra_g Seed Global)
          (compile_prog \<Gamma> Pi ps mnm main) (ictx_spec mode is_bot_pred gs)
...
definition ictx_sigma_abs_exec ::
  ...
  "ictx_sigma_abs_exec mode is_bot_pred gs \<Gamma> Pi ps mnm main =
     fun_of_dg_st_gen (map_lift (fun_of_resolved_st_q_for gs)) (map_lift (fun_of_resolved_st_q_for gs))
       \<circ> snd (ictx_sol mode is_bot_pred gs \<Gamma> Pi ps mnm main)"
...
definition ictx_sg_exec ::
  ...
  "ictx_sg_exec mode is_bot_pred gs \<Gamma> Pi ps mnm main k =
     (case k of
        Inl (v, ctx) \<Rightarrow>
          (if (v, ctx) \<in> fst (ictx_sol mode is_bot_pred gs \<Gamma> Pi ps mnm main)
           then locals (ictx_sigma_abs_exec mode is_bot_pred gs \<Gamma> Pi ps mnm main (Inl (v, ctx)))
           else Bot)
...
context
  fixes mode :: refine_mode and is_bot_pred :: "int_dom exec_dg_st \<Rightarrow> bool" and gs :: "vname \<Rightarrow> bool"
    and \<Gamma> :: tyenv and Pi :: proc_table and ps :: "pname list" and mnm :: pname and main :: com
  assumes solves: "ictx_terminates mode is_bot_pred gs \<Gamma> Pi ps mnm main"
    and exact: "\<And>s. is_bot_pred s = is_bot_state (fun_of_resolved_st_q_for gs s)"
    and entry_cov: "(cfg_entry (compile_prog \<Gamma> Pi ps mnm main), ()) \<in> fst (ictx_sol mode is_bot_pred gs \<Gamma> Pi ps mnm main)"
    and fwd_ok: "\<And>u a v ctx. (u, ctx) \<in> fst (ictx_sol mode is_bot_pred gs \<Gamma> Pi ps mnm main)
                   \<Longrightarrow> (u, a, v) \<in> intra (compile_prog \<Gamma> Pi ps mnm main)
                   \<Longrightarrow> (v, ctx) \<in> fst (ictx_sol mode is_bot_pred gs \<Gamma> Pi ps mnm main)"
    and call_fwd_ok: "\<And>u ctx dst pars args p cont.
        (u, ctx) \<in> fst (ictx_sol mode is_bot_pred gs \<Gamma> Pi ps mnm main)
        \<Longrightarrow> (u, CallEdge dst pars args, FunctionEntry p, cont) \<in> calls (compile_prog \<Gamma> Pi ps mnm main)
        \<Longrightarrow> (FunctionEntry p, ()) \<in> fst (ictx_sol mode is_bot_pred gs \<Gamma> Pi ps mnm main)"
    and comb_fwd_ok: "\<And>cl c1 dst pars args p cont.
        (cl, c1) \<in> fst (ictx_sol mode is_bot_pred gs \<Gamma> Pi ps mnm main)
        \<Longrightarrow> (cl, CallEdge dst pars args, FunctionEntry p, cont) \<in> calls (compile_prog \<Gamma> Pi ps mnm main)
        \<Longrightarrow> (cont, c1) \<in> fst (ictx_sol mode is_bot_pred gs \<Gamma> Pi ps mnm main)"
begin
...
definition ictx_sigma_abs :: "pp \<times> unit + gk \<Rightarrow> (int_dom abs_state lifted, int_dom abs_state lifted) dg_state" where
  "ictx_sigma_abs = ictx_sigma_abs_exec mode is_bot_pred gs \<Gamma> Pi ps mnm main"

definition ictx_sg :: "pp \<times> unit + gk \<Rightarrow> int_dom abs_state lifted" where
  "ictx_sg = ictx_sg_exec mode is_bot_pred gs \<Gamma> Pi ps mnm main"

lemma ictx_fin: "finite (intra (compile_prog \<Gamma> Pi ps mnm main))"
  using compile_prog_finite by blast

lemma ictx_finC: "finite (calls (compile_prog \<Gamma> Pi ps mnm main))"
  using compile_prog_finite by blast

lemma ictx_sg_covered:
  "(v, ctx) \<in> fst (ictx_sol mode is_bot_pred gs \<Gamma> Pi ps mnm main)
...
lemma ictx_sg_uncovered_empty:
  "(v, ctx) \<notin> fst (ictx_sol mode is_bot_pred gs \<Gamma> Pi ps mnm main)
...
interpretation ictx_dg: dg_ctx_activation_base "ictx_abs_spec mode gs" gamma_dg_base gs \<Gamma>
    ...
    "map_lift (fun_of_resolved_st_q_for gs) (Lifted cinit_int_dom_st)"
    "map_lift (fun_of_resolved_st_q_for gs) (Bot::int_dom exec_dg_st lifted)"
    ictx_sigma_abs "fst (ictx_sol mode is_bot_pred gs \<Gamma> Pi ps mnm main)"
    "(cfg_exit (compile_prog \<Gamma> Pi ps mnm main), ())" ictx_sg gamma_state_lift
proof unfold_locales
  show "finite (intra (compile_prog \<Gamma> Pi ps mnm main))" by (rule ictx_fin)
next
  show "part_post_solution
          (side_cfg_T_eff_keyed_seed_dg intra_predecessor_addr_list (\<lambda>_. Global) route_unit
             ...
             (map_lift (fun_of_resolved_st_q_for gs) (Lifted cinit_int_dom_st))
             (map_lift (fun_of_resolved_st_q_for gs) (Bot::int_dom exec_dg_st lifted)))
          (cfg_exit (compile_prog \<Gamma> Pi ps mnm main), ()) ictx_sigma_abs
          (fst (ictx_sol mode is_bot_pred gs \<Gamma> Pi ps mnm main))"
    unfolding ictx_sigma_abs_def ictx_sigma_abs_exec_def
    by (rule ictx_pp_abs[OF solves exact])
next
  fix v ctx assume "(v, ctx) \<in> fst (ictx_sol mode is_bot_pred gs \<Gamma> Pi ps mnm main)"
...
next
  fix v ctx assume "(v, ctx) \<notin> fst (ictx_sol mode is_bot_pred gs \<Gamma> Pi ps mnm main)"
  thus "gamma_state_lift (ictx_sg (Inl (v, ctx))) = {}"
    by (rule ictx_sg_uncovered_empty)
next
  fix u a v ctx assume "(u, ctx) \<in> fst (ictx_sol mode is_bot_pred gs \<Gamma> Pi ps mnm main)"
    "(u, a, v) \<in> intra (compile_prog \<Gamma> Pi ps mnm main)"
  thus "(v, ctx) \<in> fst (ictx_sol mode is_bot_pred gs \<Gamma> Pi ps mnm main)" by (rule fwd_ok)
qed

subsection \<open>The routed interpretation and its CALL/COMB corollaries\<close>

interpretation ictx_routed: unit_routed_context "ictx_abs_spec mode gs" gamma_dg_base gs \<Gamma>
    "compile_prog \<Gamma> Pi ps mnm main" Global
    "map_lift (fun_of_resolved_st_q_for gs) (Bot::int_dom exec_dg_st lifted)"
    "map_lift (fun_of_resolved_st_q_for gs) (Lifted cinit_int_dom_st)"
    "map_lift (fun_of_resolved_st_q_for gs) (Bot::int_dom exec_dg_st lifted)"
    ictx_sigma_abs "fst (ictx_sol mode is_bot_pred gs \<Gamma> Pi ps mnm main)"
    "(cfg_exit (compile_prog \<Gamma> Pi ps mnm main), ())" ictx_sg
...
lemma ictx_sg_seed:
  assumes "(u, CallEdge dst xs es, FunctionEntry p, cont) \<in> calls (compile_prog \<Gamma> Pi ps mnm main)"
...
lemma ictx_sg_comb:
  assumes "(cl, CallEdge dst pars args, FunctionEntry p, v) \<in> calls (compile_prog \<Gamma> Pi ps mnm main)"
    and "s \<in> gamma_state_lift (ictx_sg (Inl (cl, c1)))"
    and "t \<in> gamma_state_lift (ictx_sg (Inl (FunctionResult p, ())))"
    and "call_enter_store \<Gamma> gs (compile_prog \<Gamma> Pi ps mnm main) cl s es"
...
lemma ictx_locals_ge_s0d:
  "map_lift (fun_of_resolved_st_q_for gs) (Lifted cinit_int_dom_st)
     \<le> locals (ictx_sigma_abs (Inl (cfg_entry (compile_prog \<Gamma> Pi ps mnm main), ())))"
proof -
  have "map_lift (fun_of_resolved_st_q_for gs) (Lifted cinit_int_dom_st)
      \<le> locals (eq ictx_dg.Gen (cfg_entry (compile_prog \<Gamma> Pi ps mnm main), ()) ictx_sigma_abs)"
    by (simp add: eq_side_cfg_T_eff_keyed_seed_dg)
       (rule order_trans[OF _ side_acc_dg_ge_acc], simp add: le_supI2)
  also have "\<dots> \<le> locals (ictx_sigma_abs (Inl (cfg_entry (compile_prog \<Gamma> Pi ps mnm main), ())))"
    using ictx_dg.pp_eq_bound[OF entry_cov] by (simp add: less_eq_dg_state_def)
  finally show ?thesis .
qed

theorem ictx_activation_collect_sound:
  "activation_collect \<Gamma> gs (admiss_exact enterc_unit) () (compile_prog \<Gamma> Pi ps mnm main) (cinit_stores gs) v ctx
...
proof (rule activation_collect_sound_gen[where sg = ictx_sg and gammaM = gamma_state_lift
  ...
  also have "\<dots> \<subseteq> gamma_dg_base (locals (ictx_sigma_abs (Inl (cfg_entry (compile_prog \<Gamma> Pi ps mnm main), ()))))
                   (globs (ictx_sigma_abs (Inr Global)))"
    by (rule gamma_dg_base_mono[OF ictx_locals_ge_s0d ictx_dg.pp_entry_s0g_bound[OF entry_cov]])
  also have "\<dots> = gamma_state_lift (ictx_sg (Inl (cfg_entry (compile_prog \<Gamma> Pi ps mnm main), ())))"
    unfolding ictx_sg_covered[OF entry_cov] gamma_dg_base_def by (rule refl)
  finally show "s \<in> gamma_state_lift (ictx_sg (Inl (cfg_entry (compile_prog \<Gamma> Pi ps mnm main), ())))" .
next
...
next
  ...
  assume ce: "(cl, CallEdge dst pars args, FunctionEntry p, cont) \<in> calls (compile_prog \<Gamma> Pi ps mnm main)"
    ...
    and ces: "call_enter_store \<Gamma> gs (compile_prog \<Gamma> Pi ps mnm main) cl s es"
...
interpretation ictx_adapter: dg_analysis_adapter
  where S = "ictx_abs_spec mode gs" and gs = gs
    and g = "compile_prog \<Gamma> Pi ps mnm main" and gk0 = Global and route = route_unit
    ...
    and sigma = ictx_sigma_abs and vars = "fst (ictx_sol mode is_bot_pred gs \<Gamma> Pi ps mnm main)"
    and x0 = "(cfg_exit (compile_prog \<Gamma> Pi ps mnm main), ())" and sg = ictx_sg
...
lemma ictx_analyse_result_eq:
  "lookup_context ictx_adapter.analyse_result v ctx =
     (if (v, ctx) \<in> fst (ictx_sol mode is_bot_pred gs \<Gamma> Pi ps mnm main)
      then normalize_point gs
             (canonicalize_lift is_bot_pred (locals (snd (ictx_sol mode is_bot_pred gs \<Gamma> Pi ps mnm main) (Inl (v, ctx)))))
      else Unreachable)"
  ...
  by (cases "locals (snd (ictx_sol mode is_bot_pred gs \<Gamma> Pi ps mnm main) (Inl (v, ctx)))")
...
definition ictx_sol_per_origin ::
    "refine_mode \<Rightarrow> (int_dom exec_dg_st \<Rightarrow> bool) \<Rightarrow> (vname \<Rightarrow> bool) \<Rightarrow> tyenv \<Rightarrow> proc_table \<Rightarrow> pname list \<Rightarrow> pname \<Rightarrow> com
       \<Rightarrow> (pp \<times> unit) set \<times> (pp \<times> unit + gk \<Rightarrow> (int_dom exec_dg_st lifted, int_dom exec_dg_st lifted) dg_state)" where
  "ictx_sol_per_origin mode is_bot_pred gs \<Gamma> Pi ps mnm main =
     TD_side_per_origin_Interp_solve (ictx_eqs mode is_bot_pred gs \<Gamma> Pi ps mnm main)
       (cfg_exit (compile_prog \<Gamma> Pi ps mnm main), ())"

definition ictx_terminates_per_origin ::
    "refine_mode \<Rightarrow> (int_dom exec_dg_st \<Rightarrow> bool) \<Rightarrow> (vname \<Rightarrow> bool) \<Rightarrow> tyenv \<Rightarrow> proc_table \<Rightarrow> pname list \<Rightarrow> pname \<Rightarrow> com \<Rightarrow> bool" where
  "ictx_terminates_per_origin mode is_bot_pred gs \<Gamma> Pi ps mnm main =
...
lemma ictx_terminates_per_origin_via_solve_c:
  assumes "TD_side_per_origin_Interp_solve_c (ictx_eqs mode is_bot_pred gs \<Gamma> Pi ps mnm main)
             (cfg_exit (compile_prog \<Gamma> Pi ps mnm main), ()) \<noteq> None"
  shows "ictx_terminates_per_origin mode is_bot_pred gs \<Gamma> Pi ps mnm main"
...
lemma ictx_per_origin_pp_st:
  "ictx_terminates_per_origin mode is_bot_pred gs \<Gamma> Pi ps mnm main
     \<Longrightarrow> part_post_solution (ictx_eqs mode is_bot_pred gs \<Gamma> Pi ps mnm main)
           (cfg_exit (compile_prog \<Gamma> Pi ps mnm main), ())
           (snd (ictx_sol_per_origin mode is_bot_pred gs \<Gamma> Pi ps mnm main))
           (fst (ictx_sol_per_origin mode is_bot_pred gs \<Gamma> Pi ps mnm main))"
...
definition ictx_sol_warrow ::
    "refine_mode \<Rightarrow> (int_dom exec_dg_st \<Rightarrow> bool) \<Rightarrow> (vname \<Rightarrow> bool) \<Rightarrow> tyenv \<Rightarrow> proc_table \<Rightarrow> pname list \<Rightarrow> pname \<Rightarrow> com
       \<Rightarrow> (pp \<times> unit) set \<times> (pp \<times> unit + gk \<Rightarrow> (int_dom exec_dg_st lifted, int_dom exec_dg_st lifted) dg_state)" where
  "ictx_sol_warrow mode is_bot_pred gs \<Gamma> Pi ps mnm main =
     TD_side_warrowing_apinis_Interp_solve (ictx_eqs mode is_bot_pred gs \<Gamma> Pi ps mnm main)
       (cfg_exit (compile_prog \<Gamma> Pi ps mnm main), ())"

definition ictx_terminates_warrow ::
    "refine_mode \<Rightarrow> (int_dom exec_dg_st \<Rightarrow> bool) \<Rightarrow> (vname \<Rightarrow> bool) \<Rightarrow> tyenv \<Rightarrow> proc_table \<Rightarrow> pname list \<Rightarrow> pname \<Rightarrow> com \<Rightarrow> bool" where
  "ictx_terminates_warrow mode is_bot_pred gs \<Gamma> Pi ps mnm main =
...
lemma ictx_terminates_warrow_via_solve_c:
  assumes "TD_side_warrowing_apinis_Interp_solve_c (ictx_eqs mode is_bot_pred gs \<Gamma> Pi ps mnm main)
             (cfg_exit (compile_prog \<Gamma> Pi ps mnm main), ()) \<noteq> None"
  shows "ictx_terminates_warrow mode is_bot_pred gs \<Gamma> Pi ps mnm main"
...
lemma ictx_warrow_pp_st:
  "ictx_terminates_warrow mode is_bot_pred gs \<Gamma> Pi ps mnm main
     \<Longrightarrow> part_post_solution (ictx_eqs mode is_bot_pred gs \<Gamma> Pi ps mnm main)
           (cfg_exit (compile_prog \<Gamma> Pi ps mnm main), ())
           (snd (ictx_sol_warrow mode is_bot_pred gs \<Gamma> Pi ps mnm main))
           (fst (ictx_sol_warrow mode is_bot_pred gs \<Gamma> Pi ps mnm main))"
...
definition ictx_sol_wpo ::
    "refine_mode \<Rightarrow> (int_dom exec_dg_st \<Rightarrow> bool) \<Rightarrow> (vname \<Rightarrow> bool) \<Rightarrow> tyenv \<Rightarrow> proc_table \<Rightarrow> pname list \<Rightarrow> pname \<Rightarrow> com
       \<Rightarrow> (pp \<times> unit) set \<times> (pp \<times> unit + gk \<Rightarrow> (int_dom exec_dg_st lifted, int_dom exec_dg_st lifted) dg_state)" where
  "ictx_sol_wpo mode is_bot_pred gs \<Gamma> Pi ps mnm main =
     TD_side_warrowing_per_origin_Interp_solve (ictx_eqs mode is_bot_pred gs \<Gamma> Pi ps mnm main)
       (cfg_exit (compile_prog \<Gamma> Pi ps mnm main), ())"

definition ictx_terminates_wpo ::
    "refine_mode \<Rightarrow> (int_dom exec_dg_st \<Rightarrow> bool) \<Rightarrow> (vname \<Rightarrow> bool) \<Rightarrow> tyenv \<Rightarrow> proc_table \<Rightarrow> pname list \<Rightarrow> pname \<Rightarrow> com \<Rightarrow> bool" where
  "ictx_terminates_wpo mode is_bot_pred gs \<Gamma> Pi ps mnm main =
...
lemma ictx_terminates_wpo_via_solve_c:
  assumes "TD_side_warrowing_per_origin_Interp_solve_c (ictx_eqs mode is_bot_pred gs \<Gamma> Pi ps mnm main)
             (cfg_exit (compile_prog \<Gamma> Pi ps mnm main), ()) \<noteq> None"
  shows "ictx_terminates_wpo mode is_bot_pred gs \<Gamma> Pi ps mnm main"
...
lemma ictx_wpo_pp_st:
  "ictx_terminates_wpo mode is_bot_pred gs \<Gamma> Pi ps mnm main
     \<Longrightarrow> part_post_solution (ictx_eqs mode is_bot_pred gs \<Gamma> Pi ps mnm main)
           (cfg_exit (compile_prog \<Gamma> Pi ps mnm main), ())
           (snd (ictx_sol_wpo mode is_bot_pred gs \<Gamma> Pi ps mnm main))
           (fst (ictx_sol_wpo mode is_bot_pred gs \<Gamma> Pi ps mnm main))"
