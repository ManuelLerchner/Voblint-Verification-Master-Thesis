...
definition ictx_eqs ::
  ...
  "ictx_eqs mode is_bot_pred gs Pi ps main_name main =
     side_cfg_T_eff_keyed_seed_dg_buffered intra_predecessor_addr_list (\<lambda>_. Global)
       ...
       (compile_prog Pi ps main_name main) (ictx_spec mode is_bot_pred gs) Bot (Lifted cinit_int_dom_st) Bot"
...
definition ictx_sol ::
  ...
  "ictx_sol mode is_bot_pred gs Pi ps main_name main =
     TD_side_always_join_Interp_solve (ictx_eqs mode is_bot_pred gs Pi ps main_name main)
...
definition ictx_terminates ::
  ...
  "ictx_terminates mode is_bot_pred gs Pi ps main_name main =
     TD_side_always_join_Interp.solve_dom TYPE(gk) TYPE((int_dom exec_dg_st lifted, int_dom exec_dg_st lifted) dg_state)
       (ictx_eqs mode is_bot_pred gs Pi ps main_name main) (cfg_exit (compile_prog Pi ps main_name main), ())"

lemma ictx_terminates_via_solve_c:
  assumes "TD_side_always_join_Interp_solve_c (ictx_eqs mode is_bot_pred gs Pi ps main_name main)
             (cfg_exit (compile_prog Pi ps main_name main), ()) \<noteq> None"
  shows "ictx_terminates mode is_bot_pred gs Pi ps main_name main"
...
locale ictx_solved =
  fixes ictx_sol :: "refine_mode \<Rightarrow> (int_dom exec_dg_st \<Rightarrow> bool) \<Rightarrow> (vname \<Rightarrow> bool) \<Rightarrow> proc_table
                  \<Rightarrow> pname list \<Rightarrow> pname \<Rightarrow> com \<Rightarrow> (pp \<times> unit) set
    ...
    and ictx_terminates :: "refine_mode \<Rightarrow> (int_dom exec_dg_st \<Rightarrow> bool) \<Rightarrow> (vname \<Rightarrow> bool) \<Rightarrow> proc_table
                         \<Rightarrow> pname list \<Rightarrow> pname \<Rightarrow> com \<Rightarrow> bool"
  assumes pp_st:
    "ictx_terminates mode is_bot_pred gs Pi ps main_name main
       \<Longrightarrow> part_post_solution (ictx_eqs mode is_bot_pred gs Pi ps main_name main)
             (cfg_exit (compile_prog Pi ps main_name main), ())
             (snd (ictx_sol mode is_bot_pred gs Pi ps main_name main))
             (fst (ictx_sol mode is_bot_pred gs Pi ps main_name main))"
begin

theorem ictx_pp_abs:
  assumes solves: "ictx_terminates mode is_bot_pred gs Pi ps main_name main"
  ...
  "part_post_solution
     (side_cfg_T_eff_keyed_seed_dg intra_predecessor_addr_list (\<lambda>_. Global) route_unit
        (routed_cmb_g (ictx_abs_spec mode gs) Global Seed
           (static_resolve (compile_prog Pi ps main_name main)))
        (routed_extra_g Seed Global)
        (compile_prog Pi ps main_name main) (ictx_abs_spec mode gs)
     ...
     (fun_of_dg_st_gen (map_lift (fun_of_resolved_st_q_for gs)) (map_lift (fun_of_resolved_st_q_for gs))
        \<circ> snd (ictx_sol mode is_bot_pred gs Pi ps main_name main))
     (fst (ictx_sol mode is_bot_pred gs Pi ps main_name main))"
proof -
  have pp_buf: "part_post_solution
       (side_cfg_T_eff_keyed_seed_dg_buffered intra_predecessor_addr_list (\<lambda>_. Global)
          ...
          (routed_cmb_g_contribution (ictx_spec mode is_bot_pred gs) Global Seed
             (static_resolve (compile_prog Pi ps main_name main)))
          (routed_extra_g Seed Global)
          (compile_prog Pi ps main_name main) (ictx_spec mode is_bot_pred gs)
...
definition ictx_sigma_abs_exec ::
  ...
  "ictx_sigma_abs_exec mode is_bot_pred gs Pi ps main_name main =
     fun_of_dg_st_gen (map_lift (fun_of_resolved_st_q_for gs)) (map_lift (fun_of_resolved_st_q_for gs))
       \<circ> snd (ictx_sol mode is_bot_pred gs Pi ps main_name main)"
...
definition ictx_sg_exec ::
  ...
  "ictx_sg_exec mode is_bot_pred gs Pi ps main_name main k =
     (case k of
        Inl (v, ctx) \<Rightarrow>
          (if (v, ctx) \<in> fst (ictx_sol mode is_bot_pred gs Pi ps main_name main)
           then locals (ictx_sigma_abs_exec mode is_bot_pred gs Pi ps main_name main (Inl (v, ctx)))
           else Bot)
...
context
  ...
  assumes solves: "ictx_terminates mode is_bot_pred gs Pi ps main_name main"
    and exact: "\<And>s. is_bot_pred s = is_bot_state (fun_of_resolved_st_q_for gs s)"
    and entry_cov: "(cfg_entry (compile_prog Pi ps main_name main), ()) \<in> fst (ictx_sol mode is_bot_pred gs Pi ps main_name main)"
    and fwd_ok: "\<And>u a v ctx. (u, ctx) \<in> fst (ictx_sol mode is_bot_pred gs Pi ps main_name main)
                   \<Longrightarrow> (u, a, v) \<in> intra (compile_prog Pi ps main_name main)
                   \<Longrightarrow> (v, ctx) \<in> fst (ictx_sol mode is_bot_pred gs Pi ps main_name main)"
    and call_fwd_ok: "\<And>u ctx dst pars args p cont.
        (u, ctx) \<in> fst (ictx_sol mode is_bot_pred gs Pi ps main_name main)
        \<Longrightarrow> (u, CallEdge dst pars args, FunctionEntry p, cont) \<in> calls (compile_prog Pi ps main_name main)
        \<Longrightarrow> (FunctionEntry p, ()) \<in> fst (ictx_sol mode is_bot_pred gs Pi ps main_name main)"
    and comb_fwd_ok: "\<And>cl c1 dst pars args p cont.
        (cl, c1) \<in> fst (ictx_sol mode is_bot_pred gs Pi ps main_name main)
        \<Longrightarrow> (cl, CallEdge dst pars args, FunctionEntry p, cont) \<in> calls (compile_prog Pi ps main_name main)
        \<Longrightarrow> (cont, c1) \<in> fst (ictx_sol mode is_bot_pred gs Pi ps main_name main)"
begin
...
definition ictx_sigma_abs :: "pp \<times> unit + gk \<Rightarrow> (int_dom abs_state lifted, int_dom abs_state lifted) dg_state" where
  "ictx_sigma_abs = ictx_sigma_abs_exec mode is_bot_pred gs Pi ps main_name main"

definition ictx_sg :: "pp \<times> unit + gk \<Rightarrow> int_dom abs_state lifted" where
  "ictx_sg = ictx_sg_exec mode is_bot_pred gs Pi ps main_name main"

lemma ictx_fin: "finite (intra (compile_prog Pi ps main_name main))"
  using compile_prog_finite by blast

lemma ictx_finC: "finite (calls (compile_prog Pi ps main_name main))"
  using compile_prog_finite by blast

lemma ictx_sg_covered:
  "(v, ctx) \<in> fst (ictx_sol mode is_bot_pred gs Pi ps main_name main)
...
lemma ictx_sg_uncovered_empty:
  "(v, ctx) \<notin> fst (ictx_sol mode is_bot_pred gs Pi ps main_name main)
...
interpretation ictx_dg: dg_ctx_activation_base "ictx_abs_spec mode gs" gamma_dg_base gs
    "compile_prog Pi ps main_name main" Global route_unit
    "routed_cmb_g (ictx_abs_spec mode gs) Global Seed
       (static_resolve (compile_prog Pi ps main_name main))"
    "routed_extra_g Seed Global"
    "map_lift (fun_of_resolved_st_q_for gs) (Bot::int_dom exec_dg_st lifted)"
    "map_lift (fun_of_resolved_st_q_for gs) (Lifted cinit_int_dom_st)"
    "map_lift (fun_of_resolved_st_q_for gs) (Bot::int_dom exec_dg_st lifted)"
    ictx_sigma_abs "fst (ictx_sol mode is_bot_pred gs Pi ps main_name main)"
    "(cfg_exit (compile_prog Pi ps main_name main), ())" ictx_sg gamma_state_lift
proof unfold_locales
  show "finite (intra (compile_prog Pi ps main_name main))" by (rule ictx_fin)
next
  show "part_post_solution
          (side_cfg_T_eff_keyed_seed_dg intra_predecessor_addr_list (\<lambda>_. Global) route_unit
             ...
             (map_lift (fun_of_resolved_st_q_for gs) (Lifted cinit_int_dom_st))
             (map_lift (fun_of_resolved_st_q_for gs) (Bot::int_dom exec_dg_st lifted)))
          (cfg_exit (compile_prog Pi ps main_name main), ()) ictx_sigma_abs
          (fst (ictx_sol mode is_bot_pred gs Pi ps main_name main))"
    unfolding ictx_sigma_abs_def ictx_sigma_abs_exec_def
    by (rule ictx_pp_abs[OF solves exact])
next
  fix v ctx assume "(v, ctx) \<in> fst (ictx_sol mode is_bot_pred gs Pi ps main_name main)"
...
next
  fix v ctx assume "(v, ctx) \<notin> fst (ictx_sol mode is_bot_pred gs Pi ps main_name main)"
  thus "gamma_state_lift (ictx_sg (Inl (v, ctx))) = {}"
    by (rule ictx_sg_uncovered_empty)
next
  fix u a v ctx assume "(u, ctx) \<in> fst (ictx_sol mode is_bot_pred gs Pi ps main_name main)"
    "(u, a, v) \<in> intra (compile_prog Pi ps main_name main)"
  thus "(v, ctx) \<in> fst (ictx_sol mode is_bot_pred gs Pi ps main_name main)" by (rule fwd_ok)
qed
...
interpretation ictx_routed: unit_routed_context "ictx_abs_spec mode gs" gamma_dg_base gs
    "compile_prog Pi ps main_name main" Global
    "map_lift (fun_of_resolved_st_q_for gs) (Bot::int_dom exec_dg_st lifted)"
    "map_lift (fun_of_resolved_st_q_for gs) (Lifted cinit_int_dom_st)"
    "map_lift (fun_of_resolved_st_q_for gs) (Bot::int_dom exec_dg_st lifted)"
    ictx_sigma_abs "fst (ictx_sol mode is_bot_pred gs Pi ps main_name main)"
    "(cfg_exit (compile_prog Pi ps main_name main), ())" ictx_sg
...
next
  ...
  obtain dst' pars' args' p' cont' where
      ce': "(cl, CallEdge dst' pars' args', FunctionEntry p', cont') \<in> calls (compile_prog Pi ps main_name main)"
...
lemma ictx_sg_seed:
  assumes "(u, CallEdge dst xs es, FunctionEntry p, cont) \<in> calls (compile_prog Pi ps main_name main)"
...
lemma ictx_sg_comb:
  assumes "(cl, CallEdge dst pars args, FunctionEntry p, v) \<in> calls (compile_prog Pi ps main_name main)"
    and "s \<in> gamma_state_lift (ictx_sg (Inl (cl, c1)))"
    and "t \<in> gamma_state_lift (ictx_sg (Inl (FunctionResult p, ())))"
    and "call_enter_store gs (compile_prog Pi ps main_name main) cl s es"
...
lemma ictx_locals_ge_s0d:
  "map_lift (fun_of_resolved_st_q_for gs) (Lifted cinit_int_dom_st)
     \<le> locals (ictx_sigma_abs (Inl (cfg_entry (compile_prog Pi ps main_name main), ())))"
proof -
  have "map_lift (fun_of_resolved_st_q_for gs) (Lifted cinit_int_dom_st)
      \<le> locals (eq ictx_dg.Gen (cfg_entry (compile_prog Pi ps main_name main), ()) ictx_sigma_abs)"
    by (simp add: eq_side_cfg_T_eff_keyed_seed_dg)
       (rule order_trans[OF _ side_acc_dg_ge_acc], simp add: le_supI2)
  also have "\<dots> \<le> locals (ictx_sigma_abs (Inl (cfg_entry (compile_prog Pi ps main_name main), ())))"
    using ictx_dg.pp_eq_bound[OF entry_cov] by (simp add: less_eq_dg_state_def)
  finally show ?thesis .
qed

theorem ictx_activation_collect_sound:
  "activation_collect gs enterc_unit () (compile_prog Pi ps main_name main) (cinit_stores gs) v ctx
...
proof (rule activation_collect_sound_gen[where sg = ictx_sg and gammaM = gamma_state_lift
        ...
        and initial_ctx = "()" and S = "cinit_stores gs" and g = "compile_prog Pi ps main_name main" and gs = gs])
  ...
  also have "\<dots> \<subseteq> gamma_dg_base (locals (ictx_sigma_abs (Inl (cfg_entry (compile_prog Pi ps main_name main), ()))))
                   (globs (ictx_sigma_abs (Inr Global)))"
    by (rule gamma_dg_base_mono[OF ictx_locals_ge_s0d ictx_dg.pp_entry_s0g_bound[OF entry_cov]])
  also have "\<dots> = gamma_state_lift (ictx_sg (Inl (cfg_entry (compile_prog Pi ps main_name main), ())))"
    unfolding ictx_sg_covered[OF entry_cov] gamma_dg_base_def by (rule refl)
  finally show "s \<in> gamma_state_lift (ictx_sg (Inl (cfg_entry (compile_prog Pi ps main_name main), ())))" .
next
  \<comment> \<open>EDGE\<close>
  show "\<And>u a v c s s'. (u, a, v) \<in> intra (compile_prog Pi ps main_name main)
...
next
  ...
  assume ce: "(u, CallEdge dst pars args, FunctionEntry p, cont) \<in> calls (compile_prog Pi ps main_name main)"
...
next
  ...
  assume ce: "(cl, CallEdge dst pars args, FunctionEntry p, cont) \<in> calls (compile_prog Pi ps main_name main)"
    ...
    and ces: "call_enter_store gs (compile_prog Pi ps main_name main) cl s es"
...
interpretation ictx_adapter: dg_analysis_adapter
  where S = "ictx_abs_spec mode gs" and gs = gs
    and g = "compile_prog Pi ps main_name main" and gk0 = Global and route = route_unit
    ...
    and sigma = ictx_sigma_abs and vars = "fst (ictx_sol mode is_bot_pred gs Pi ps main_name main)"
    and x0 = "(cfg_exit (compile_prog Pi ps main_name main), ())" and sg = ictx_sg
...
lemma ictx_analyse_result_eq:
  "lookup_context ictx_adapter.analyse_result v ctx =
     (if (v, ctx) \<in> fst (ictx_sol mode is_bot_pred gs Pi ps main_name main)
      then normalize_point gs
             (canonicalize_lift is_bot_pred (locals (snd (ictx_sol mode is_bot_pred gs Pi ps main_name main) (Inl (v, ctx)))))
      else Unreachable)"
  ...
  by (cases "locals (snd (ictx_sol mode is_bot_pred gs Pi ps main_name main) (Inl (v, ctx)))")
...
definition ictx_sol_per_origin ::
    "refine_mode \<Rightarrow> (int_dom exec_dg_st \<Rightarrow> bool) \<Rightarrow> (vname \<Rightarrow> bool) \<Rightarrow> proc_table \<Rightarrow> pname list \<Rightarrow> pname \<Rightarrow> com
       \<Rightarrow> (pp \<times> unit) set \<times> (pp \<times> unit + gk \<Rightarrow> (int_dom exec_dg_st lifted, int_dom exec_dg_st lifted) dg_state)" where
  "ictx_sol_per_origin mode is_bot_pred gs Pi ps main_name main =
     TD_side_per_origin_Interp_solve (ictx_eqs mode is_bot_pred gs Pi ps main_name main)
       (cfg_exit (compile_prog Pi ps main_name main), ())"

definition ictx_terminates_per_origin ::
    "refine_mode \<Rightarrow> (int_dom exec_dg_st \<Rightarrow> bool) \<Rightarrow> (vname \<Rightarrow> bool) \<Rightarrow> proc_table \<Rightarrow> pname list \<Rightarrow> pname \<Rightarrow> com \<Rightarrow> bool" where
  "ictx_terminates_per_origin mode is_bot_pred gs Pi ps main_name main =
...
lemma ictx_terminates_per_origin_via_solve_c:
  assumes "TD_side_per_origin_Interp_solve_c (ictx_eqs mode is_bot_pred gs Pi ps main_name main)
             (cfg_exit (compile_prog Pi ps main_name main), ()) \<noteq> None"
  shows "ictx_terminates_per_origin mode is_bot_pred gs Pi ps main_name main"
...
lemma ictx_per_origin_pp_st:
  "ictx_terminates_per_origin mode is_bot_pred gs Pi ps main_name main
     \<Longrightarrow> part_post_solution (ictx_eqs mode is_bot_pred gs Pi ps main_name main)
           (cfg_exit (compile_prog Pi ps main_name main), ())
           (snd (ictx_sol_per_origin mode is_bot_pred gs Pi ps main_name main))
           (fst (ictx_sol_per_origin mode is_bot_pred gs Pi ps main_name main))"
...
definition ictx_sol_warrow ::
    "refine_mode \<Rightarrow> (int_dom exec_dg_st \<Rightarrow> bool) \<Rightarrow> (vname \<Rightarrow> bool) \<Rightarrow> proc_table \<Rightarrow> pname list \<Rightarrow> pname \<Rightarrow> com
       \<Rightarrow> (pp \<times> unit) set \<times> (pp \<times> unit + gk \<Rightarrow> (int_dom exec_dg_st lifted, int_dom exec_dg_st lifted) dg_state)" where
  "ictx_sol_warrow mode is_bot_pred gs Pi ps main_name main =
     TD_side_warrowing_apinis_Interp_solve (ictx_eqs mode is_bot_pred gs Pi ps main_name main)
       (cfg_exit (compile_prog Pi ps main_name main), ())"

definition ictx_terminates_warrow ::
    "refine_mode \<Rightarrow> (int_dom exec_dg_st \<Rightarrow> bool) \<Rightarrow> (vname \<Rightarrow> bool) \<Rightarrow> proc_table \<Rightarrow> pname list \<Rightarrow> pname \<Rightarrow> com \<Rightarrow> bool" where
  "ictx_terminates_warrow mode is_bot_pred gs Pi ps main_name main =
...
lemma ictx_terminates_warrow_via_solve_c:
  assumes "TD_side_warrowing_apinis_Interp_solve_c (ictx_eqs mode is_bot_pred gs Pi ps main_name main)
             (cfg_exit (compile_prog Pi ps main_name main), ()) \<noteq> None"
  shows "ictx_terminates_warrow mode is_bot_pred gs Pi ps main_name main"
...
lemma ictx_warrow_pp_st:
  "ictx_terminates_warrow mode is_bot_pred gs Pi ps main_name main
     \<Longrightarrow> part_post_solution (ictx_eqs mode is_bot_pred gs Pi ps main_name main)
           (cfg_exit (compile_prog Pi ps main_name main), ())
           (snd (ictx_sol_warrow mode is_bot_pred gs Pi ps main_name main))
           (fst (ictx_sol_warrow mode is_bot_pred gs Pi ps main_name main))"
...
definition ictx_sol_wpo ::
    "refine_mode \<Rightarrow> (int_dom exec_dg_st \<Rightarrow> bool) \<Rightarrow> (vname \<Rightarrow> bool) \<Rightarrow> proc_table \<Rightarrow> pname list \<Rightarrow> pname \<Rightarrow> com
       \<Rightarrow> (pp \<times> unit) set \<times> (pp \<times> unit + gk \<Rightarrow> (int_dom exec_dg_st lifted, int_dom exec_dg_st lifted) dg_state)" where
  "ictx_sol_wpo mode is_bot_pred gs Pi ps main_name main =
     TD_side_warrowing_per_origin_Interp_solve (ictx_eqs mode is_bot_pred gs Pi ps main_name main)
       (cfg_exit (compile_prog Pi ps main_name main), ())"

definition ictx_terminates_wpo ::
    "refine_mode \<Rightarrow> (int_dom exec_dg_st \<Rightarrow> bool) \<Rightarrow> (vname \<Rightarrow> bool) \<Rightarrow> proc_table \<Rightarrow> pname list \<Rightarrow> pname \<Rightarrow> com \<Rightarrow> bool" where
  "ictx_terminates_wpo mode is_bot_pred gs Pi ps main_name main =
...
lemma ictx_terminates_wpo_via_solve_c:
  assumes "TD_side_warrowing_per_origin_Interp_solve_c (ictx_eqs mode is_bot_pred gs Pi ps main_name main)
             (cfg_exit (compile_prog Pi ps main_name main), ()) \<noteq> None"
  shows "ictx_terminates_wpo mode is_bot_pred gs Pi ps main_name main"
...
lemma ictx_wpo_pp_st:
  "ictx_terminates_wpo mode is_bot_pred gs Pi ps main_name main
     \<Longrightarrow> part_post_solution (ictx_eqs mode is_bot_pred gs Pi ps main_name main)
           (cfg_exit (compile_prog Pi ps main_name main), ())
           (snd (ictx_sol_wpo mode is_bot_pred gs Pi ps main_name main))
           (fst (ictx_sol_wpo mode is_bot_pred gs Pi ps main_name main))"
...
text \<open>
  ...
  so a caller supplies only \<open>mode\<close>, \<open>gs\<close>, \<open>main_name\<close>, and the \<^typ>\<open>imp_prog\<close> value.
\<close>
...
definition ictx_sol_prog ::
    "refine_mode \<Rightarrow> (vname \<Rightarrow> bool) \<Rightarrow> pname \<Rightarrow> imp_prog
       \<Rightarrow> (pp \<times> unit) set \<times> (pp \<times> unit + gk \<Rightarrow> (int_dom exec_dg_st lifted, int_dom exec_dg_st lifted) dg_state)" where
  "ictx_sol_prog mode gs main_name p =
     TD_side_always_join_Interp_solve (ictx_eqs_prog mode gs main_name p) (cfg_exit (prog_cfg main_name p), ())"
...
definition ictx_terminates_prog :: "refine_mode \<Rightarrow> (vname \<Rightarrow> bool) \<Rightarrow> pname \<Rightarrow> imp_prog \<Rightarrow> bool" where
  "ictx_terminates_prog mode gs main_name p =
...
lemma ictx_terminates_prog_via_solve_c:
  assumes "TD_side_always_join_Interp_solve_c
             (ictx_eqs mode (resolved_st_q_is_bot_for (declared_global_vars p))
                gs (prog_table p) (prog_procs p) main_name (prog_main p))
             (cfg_exit (compile_prog (prog_table p) (prog_procs p) main_name (prog_main p)), ()) \<noteq> None"
  shows "ictx_terminates_prog mode gs main_name p"
...
definition analyse_int_ctx_result_for ::
  ...
  "analyse_int_ctx_result_for mode gs main_name p =
     Analysis_Result
       (fst (ictx_sol_prog mode gs main_name p))
       (\<lambda>v ctx. normalize_point gs
                  (canonicalize_lift (resolved_st_q_is_bot_for (declared_global_vars p))
                    (locals (snd (ictx_sol_prog mode gs main_name p) (Inl (v, ctx))))))"

declare analyse_int_ctx_result_for_def [code del]

lemma analyse_int_ctx_result_for_code [code]:
  "analyse_int_ctx_result_for mode gs main_name p =
     (let sol = ictx_sol_prog mode gs main_name p; gl = declared_global_vars p
...
definition ictx_sol_prog_per_origin ::
    "refine_mode \<Rightarrow> (vname \<Rightarrow> bool) \<Rightarrow> pname \<Rightarrow> imp_prog
       \<Rightarrow> (pp \<times> unit) set \<times> (pp \<times> unit + gk \<Rightarrow> (int_dom exec_dg_st lifted, int_dom exec_dg_st lifted) dg_state)" where
  "ictx_sol_prog_per_origin mode gs main_name p =
     TD_side_per_origin_Interp_solve (ictx_eqs_prog mode gs main_name p) (cfg_exit (prog_cfg main_name p), ())"
...
definition ictx_terminates_prog_per_origin :: "refine_mode \<Rightarrow> (vname \<Rightarrow> bool) \<Rightarrow> pname \<Rightarrow> imp_prog \<Rightarrow> bool" where
  "ictx_terminates_prog_per_origin mode gs main_name p =
...
lemma ictx_terminates_prog_per_origin_via_solve_c:
  assumes "TD_side_per_origin_Interp_solve_c
             (ictx_eqs mode (resolved_st_q_is_bot_for (declared_global_vars p))
                gs (prog_table p) (prog_procs p) main_name (prog_main p))
             (cfg_exit (compile_prog (prog_table p) (prog_procs p) main_name (prog_main p)), ()) \<noteq> None"
  shows "ictx_terminates_prog_per_origin mode gs main_name p"
...
definition analyse_int_ctx_result_per_origin_for ::
    "refine_mode \<Rightarrow> (vname \<Rightarrow> bool) \<Rightarrow> pname \<Rightarrow> imp_prog \<Rightarrow> (unit, int_dom abs_state) analysis_result" where
  "analyse_int_ctx_result_per_origin_for mode gs main_name p =
     Analysis_Result
       (fst (ictx_sol_prog_per_origin mode gs main_name p))
       (\<lambda>v ctx. normalize_point gs
                  (canonicalize_lift (resolved_st_q_is_bot_for (declared_global_vars p))
                    (locals (snd (ictx_sol_prog_per_origin mode gs main_name p) (Inl (v, ctx))))))"

declare analyse_int_ctx_result_per_origin_for_def [code del]

lemma analyse_int_ctx_result_per_origin_for_code [code]:
  "analyse_int_ctx_result_per_origin_for mode gs main_name p =
     (let sol = ictx_sol_prog_per_origin mode gs main_name p; gl = declared_global_vars p
      in Analysis_Result (fst sol)
           (\<lambda>v ctx. normalize_point gs
                      (canonicalize_lift (resolved_st_q_is_bot_for gl)
                        (locals (snd sol (Inl (v, ctx)))))))"
...
definition ictx_sol_prog_warrow ::
    "refine_mode \<Rightarrow> (vname \<Rightarrow> bool) \<Rightarrow> pname \<Rightarrow> imp_prog
       \<Rightarrow> (pp \<times> unit) set \<times> (pp \<times> unit + gk \<Rightarrow> (int_dom exec_dg_st lifted, int_dom exec_dg_st lifted) dg_state)" where
  "ictx_sol_prog_warrow mode gs main_name p =
     TD_side_warrowing_apinis_Interp_solve (ictx_eqs_prog mode gs main_name p) (cfg_exit (prog_cfg main_name p), ())"
...
definition ictx_terminates_prog_warrow :: "refine_mode \<Rightarrow> (vname \<Rightarrow> bool) \<Rightarrow> pname \<Rightarrow> imp_prog \<Rightarrow> bool" where
  "ictx_terminates_prog_warrow mode gs main_name p =
...
lemma ictx_terminates_prog_warrow_via_solve_c:
  assumes "TD_side_warrowing_apinis_Interp_solve_c
             (ictx_eqs mode (resolved_st_q_is_bot_for (declared_global_vars p))
                gs (prog_table p) (prog_procs p) main_name (prog_main p))
             (cfg_exit (compile_prog (prog_table p) (prog_procs p) main_name (prog_main p)), ()) \<noteq> None"
  shows "ictx_terminates_prog_warrow mode gs main_name p"
...
definition analyse_int_ctx_result_warrow_for ::
    "refine_mode \<Rightarrow> (vname \<Rightarrow> bool) \<Rightarrow> pname \<Rightarrow> imp_prog \<Rightarrow> (unit, int_dom abs_state) analysis_result" where
  "analyse_int_ctx_result_warrow_for mode gs main_name p =
     Analysis_Result
       (fst (ictx_sol_prog_warrow mode gs main_name p))
       (\<lambda>v ctx. normalize_point gs
                  (canonicalize_lift (resolved_st_q_is_bot_for (declared_global_vars p))
                    (locals (snd (ictx_sol_prog_warrow mode gs main_name p) (Inl (v, ctx))))))"
...
lemma fst_analyse_int_ctx_solved_warrow_for:
  "fst (analyse_int_ctx_solved_warrow_for mode gs main_name p)
     = analyse_int_ctx_result_warrow_for mode gs main_name p"
...
lemma analyse_int_ctx_result_warrow_for_code [code]:
  "analyse_int_ctx_result_warrow_for mode gs main_name p =
     (let sol = ictx_sol_prog_warrow mode gs main_name p; gl = declared_global_vars p
      in Analysis_Result (fst sol)
           (\<lambda>v ctx. normalize_point gs
                      (canonicalize_lift (resolved_st_q_is_bot_for gl)
                        (locals (snd sol (Inl (v, ctx)))))))"
...
definition ictx_sol_prog_wpo ::
    "refine_mode \<Rightarrow> (vname \<Rightarrow> bool) \<Rightarrow> pname \<Rightarrow> imp_prog
       \<Rightarrow> (pp \<times> unit) set \<times> (pp \<times> unit + gk \<Rightarrow> (int_dom exec_dg_st lifted, int_dom exec_dg_st lifted) dg_state)" where
  "ictx_sol_prog_wpo mode gs main_name p =
     TD_side_warrowing_per_origin_Interp_solve (ictx_eqs_prog mode gs main_name p) (cfg_exit (prog_cfg main_name p), ())"
...
definition ictx_terminates_prog_wpo :: "refine_mode \<Rightarrow> (vname \<Rightarrow> bool) \<Rightarrow> pname \<Rightarrow> imp_prog \<Rightarrow> bool" where
  "ictx_terminates_prog_wpo mode gs main_name p =
...
lemma ictx_terminates_prog_wpo_via_solve_c:
  assumes "TD_side_warrowing_per_origin_Interp_solve_c
             (ictx_eqs mode (resolved_st_q_is_bot_for (declared_global_vars p))
                gs (prog_table p) (prog_procs p) main_name (prog_main p))
             (cfg_exit (compile_prog (prog_table p) (prog_procs p) main_name (prog_main p)), ()) \<noteq> None"
  shows "ictx_terminates_prog_wpo mode gs main_name p"
...
definition analyse_int_ctx_result_wpo_for ::
    "refine_mode \<Rightarrow> (vname \<Rightarrow> bool) \<Rightarrow> pname \<Rightarrow> imp_prog \<Rightarrow> (unit, int_dom abs_state) analysis_result" where
  "analyse_int_ctx_result_wpo_for mode gs main_name p =
     Analysis_Result
       (fst (ictx_sol_prog_wpo mode gs main_name p))
       (\<lambda>v ctx. normalize_point gs
                  (canonicalize_lift (resolved_st_q_is_bot_for (declared_global_vars p))
                    (locals (snd (ictx_sol_prog_wpo mode gs main_name p) (Inl (v, ctx))))))"

declare analyse_int_ctx_result_wpo_for_def [code del]

lemma analyse_int_ctx_result_wpo_for_code [code]:
  "analyse_int_ctx_result_wpo_for mode gs main_name p =
     (let sol = ictx_sol_prog_wpo mode gs main_name p; gl = declared_global_vars p
      in Analysis_Result (fst sol)
           (\<lambda>v ctx. normalize_point gs
                      (canonicalize_lift (resolved_st_q_is_bot_for gl)
                        (locals (snd sol (Inl (v, ctx)))))))"
