...
text \<open>
  ...
  \<open>Sign_Exec_Sound\<close> at the interval domain:
    ...
    \<^item> \<open>ivl_exec \<Pi> ps main\<close>      -- the analyzer's computed abstract state at the
      program exit (an \<open>ivl abs_state\<close>), read back through \<open>fun_of_resolved_st_q_for is_global\<close>;
...
definition ivl_exec_eqs ::
    "proc_table \<Rightarrow> pname list \<Rightarrow> pname \<Rightarrow> com \<Rightarrow> (pp, unit, ivl resolved_st_q) eqsT" where
  "ivl_exec_eqs \<Pi> ps mnm main =
     side_cfg_T_eff_st (compile_prog \<Pi> ps mnm main) ivl_etf_st bot cinit_ivl_st ()"

definition ivl_exec_raw ::
    "proc_table \<Rightarrow> pname list \<Rightarrow> pname \<Rightarrow> com \<Rightarrow> (pp + unit \<Rightarrow> ivl resolved_st_q)" where
  "ivl_exec_raw \<Pi> ps mnm main =
     snd (TD_side_always_join_Interp_solve (ivl_exec_eqs \<Pi> ps mnm main)
            (cfg_exit (compile_prog \<Pi> ps mnm main)))"

definition ivl_exec_at ::
    "proc_table \<Rightarrow> pname list \<Rightarrow> pname \<Rightarrow> com \<Rightarrow> pp \<Rightarrow> ivl abs_state" where
  "ivl_exec_at \<Pi> ps mnm main v =
     side_env (fun_of_resolved_st_q_for is_global \<circ> ivl_exec_raw \<Pi> ps mnm main) v"

definition ivl_exec ::
    "proc_table \<Rightarrow> pname list \<Rightarrow> pname \<Rightarrow> com \<Rightarrow> ivl abs_state" where
  "ivl_exec \<Pi> ps mnm main = ivl_exec_at \<Pi> ps mnm main (cfg_exit (compile_prog \<Pi> ps mnm main))"

definition ivl_exec_terminates ::
    "proc_table \<Rightarrow> pname list \<Rightarrow> pname \<Rightarrow> com \<Rightarrow> bool" where
  "ivl_exec_terminates \<Pi> ps mnm main =
     TD_side_always_join_Interp.solve_dom TYPE(unit) TYPE(ivl resolved_st_q)
        (ivl_exec_eqs \<Pi> ps mnm main) (cfg_exit (compile_prog \<Pi> ps mnm main))"
...
lemma ivl_exec_terminates_via_solve_c:
  assumes "TD_side_always_join_Interp_solve_c (ivl_exec_eqs \<Pi> ps mnm main)
             (cfg_exit (compile_prog \<Pi> ps mnm main)) \<noteq> None"
  shows "ivl_exec_terminates \<Pi> ps mnm main"
...
theorem ivl_exec_sound_collecting_at:
  ...
  assumes solves: "ivl_exec_terminates \<Pi> ps mnm main"
  ...
  shows "ltr_collect is_global (compile_prog \<Pi> ps mnm main) (cinit_stores is_global) v
         \<le> \<lbrakk>ivl_exec_at \<Pi> ps mnm main v\<rbrakk>"
proof -
  define g :: cfg where "g = compile_prog \<Pi> ps mnm main"
  define sol :: "pp set \<times> (pp + unit \<Rightarrow> ivl resolved_st_q)" where
    "sol = TD_side_always_join_Interp_solve (ivl_exec_eqs \<Pi> ps mnm main) (cfg_exit g)"
  define \<sigma> :: "pp + unit \<Rightarrow> ivl abs_state" where "\<sigma> = fun_of_resolved_st_q_for is_global \<circ> snd sol"
  ...
  have pp_eff: "part_post_solution
    ...
    using part_post_solution_st_to_abs_eff_unit_transfer
            [OF ivl_etf_edge_tree ivl_etf_combine_tree
                ivl_etf_st_edge_tree ivl_etf_st_combine_tree ivl_tf_st_commute
                ivl_etf_enter_tree ivl_etf_st_enter_tree ivl_enter_st_commute pp_st]
    by (simp add: \<sigma>_def fun_of_st_cinit_ivl_st bot_fun_def)
  have cone: "cone_compatible_etf ivl_etf" by (rule ivl_etf_cone_compatible)
  have srz: "\<And>z. side_rg (ivl_exec_eqs \<Pi> ps mnm main z)"
    unfolding ivl_exec_eqs_def
    by (rule side_rg_side_cfg_T_eff_st_unit
          [OF ivl_etf_st_exists_unit ivl_etf_st_enter_exists_unit ivl_etf_st_combine_tree])
  have solpair: "TD_side_always_join_Interp_solve (ivl_exec_eqs \<Pi> ps mnm main) (cfg_exit g)
                   = (fst sol, snd sol)"
    unfolding sol_def by simp
  ...
  have entry_le: "(\<lambda>x. if is_global x then Ivl (Fin 0) (Fin 0) else Ivl MinInf PlusInf)
                    \<le> side_env \<sigma> (cfg_entry g)"
    by (rule s0_le_side_env_entry_eff[OF pp_eff entry_in])
  have seed_cov: "cinit_stores is_global
                    \<subseteq> \<lbrakk>\<lambda>x. if is_global x then Ivl (Fin 0) (Fin 0) else Ivl MinInf PlusInf\<rbrakk>"
...
corollary ivl_exec_sound_collecting:
  ...
  shows "ltr_collect is_global (compile_prog \<Pi> ps mnm main) (cinit_stores is_global) (cfg_exit (compile_prog \<Pi> ps mnm main))
         \<le> \<lbrakk>ivl_exec \<Pi> ps mnm main\<rbrakk>"
...
definition ivl_exec_prog_at :: "pname \<Rightarrow> imp_prog \<Rightarrow> pp \<Rightarrow> ivl abs_state" where
  "ivl_exec_prog_at mnm p v = ivl_exec_at (prog_table p) (prog_procs p) mnm (prog_main p) v"

definition ivl_exec_prog :: "pname \<Rightarrow> imp_prog \<Rightarrow> ivl abs_state" where
  "ivl_exec_prog mnm p = ivl_exec_prog_at mnm p (cfg_exit (prog_cfg mnm p))"

definition ivl_terminates_prog :: "pname \<Rightarrow> imp_prog \<Rightarrow> bool" where
  "ivl_terminates_prog mnm p = ivl_exec_terminates (prog_table p) (prog_procs p) mnm (prog_main p)"

lemma ivl_terminates_prog_via_solve_c:
  assumes "TD_side_always_join_Interp_solve_c
             (ivl_exec_eqs (prog_table p) (prog_procs p) mnm (prog_main p))
             (cfg_exit (compile_prog (prog_table p) (prog_procs p) mnm (prog_main p))) \<noteq> None"
  shows "ivl_terminates_prog mnm p"
...
corollary ivl_exec_prog_sound_collecting_at:
  assumes "ivl_terminates_prog mnm p"
  ...
  shows "ltr_collect is_global (prog_cfg mnm p) (cinit_stores is_global) v
           \<le> \<lbrakk>ivl_exec_prog_at mnm p v\<rbrakk>"
...
corollary ivl_exec_prog_sound_collecting:
  ...
  shows "ltr_collect is_global (prog_cfg mnm p) (cinit_stores is_global) (cfg_exit (prog_cfg mnm p))
           \<le> \<lbrakk>ivl_exec_prog mnm p\<rbrakk>"
...
definition ivl_graph_config ::
  ...
  "ivl_graph_config \<Pi> ps mnm main =
    \<lparr> local_of = id,
...
definition ivl_graph_solution ::
  "(pp + unit \<Rightarrow> ivl resolved_st_q) \<Rightarrow> (pp \<times> unit + unit \<Rightarrow> ivl abs_state)" where
  "ivl_graph_solution sol z =
...
definition ivl_annotated_dot_lit ::
  ...
  "ivl_annotated_dot_lit \<Pi> ps mnm main =
    String.implode
      (let g = compile_prog \<Pi> ps mnm main;
           domain = contextual_graph_domain g (\<lambda>_. [()]) @ [Inr ()];
           sol = ivl_graph_solution (ivl_exec_raw \<Pi> ps mnm main)
       in contextual_analysis_dot (ivl_graph_config \<Pi> ps mnm main) g domain sol)"

definition ivl_annotated_dot_prog_lit :: "pname \<Rightarrow> imp_prog \<Rightarrow> String.literal" where
  "ivl_annotated_dot_prog_lit mnm p =
     ivl_annotated_dot_lit (prog_table p) (prog_procs p) mnm (prog_main p)"
