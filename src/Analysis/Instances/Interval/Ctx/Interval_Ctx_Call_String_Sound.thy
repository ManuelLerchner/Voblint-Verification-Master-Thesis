...
definition cs_call_string_eqs ::
  ...
  "cs_call_string_eqs k gs \<Gamma> is_bot_pred Pi ps mnm main =
     side_cfg_T_eff_keyed_seed_dg_buffered intra_predecessor_addr_list (\<lambda>_. Call_String_Context.Global)
       (cs_route k)
       (routed_cmb_g_contribution (ectx_spec gs \<Gamma> is_bot_pred)
...
definition cs_call_string_terminates ::
  ...
  "cs_call_string_terminates k gs \<Gamma> is_bot_pred Pi ps mnm main =
     TD_side_warrowing_apinis_Interp.solve_dom TYPE(call_string_gk) TYPE((ivl exec_dg_st lifted, ivl exec_dg_st lifted) dg_state)
       (cs_call_string_eqs k gs \<Gamma> is_bot_pred Pi ps mnm main) (cfg_exit (compile_prog Pi ps mnm main), [])"
...
definition cs_call_string_eqs_prog ::
    "nat \<Rightarrow> (vname \<Rightarrow> bool) \<Rightarrow> tyenv \<Rightarrow> pname \<Rightarrow> imp_prog
       \<Rightarrow> (pp \<times> call_string, call_string_gk, (ivl exec_dg_st lifted, ivl exec_dg_st lifted) dg_state) eqsT" where
  "cs_call_string_eqs_prog k gs \<Gamma> mnm p =
     cs_call_string_eqs k gs \<Gamma> (resolved_st_q_is_bot_for (declared_global_vars p))
       (prog_table p) (prog_procs p) mnm (prog_main p)"

definition cs_call_string_sol_prog ::
    "nat \<Rightarrow> (vname \<Rightarrow> bool) \<Rightarrow> tyenv \<Rightarrow> pname \<Rightarrow> imp_prog
       \<Rightarrow> (pp \<times> call_string) set \<times> (pp \<times> call_string + call_string_gk \<Rightarrow> (ivl exec_dg_st lifted, ivl exec_dg_st lifted) dg_state)" where
  "cs_call_string_sol_prog k gs \<Gamma> mnm p =
     cs_call_string_sol k gs \<Gamma> (resolved_st_q_is_bot_for (declared_global_vars p))
       (prog_table p) (prog_procs p) mnm (prog_main p)"

definition cs_call_string_terminates_prog :: "nat \<Rightarrow> (vname \<Rightarrow> bool) \<Rightarrow> tyenv \<Rightarrow> pname \<Rightarrow> imp_prog \<Rightarrow> bool" where
  "cs_call_string_terminates_prog k gs \<Gamma> mnm p =
     cs_call_string_terminates k gs \<Gamma> (resolved_st_q_is_bot_for (declared_global_vars p))
       (prog_table p) (prog_procs p) mnm (prog_main p)"
...
lemma cs_call_string_terminates_prog_via_solve_c:
  ...
  shows "cs_call_string_terminates_prog k gs \<Gamma> mnm p"
  using assms
...
definition analyse_interval_call_string_result_for ::
    "nat \<Rightarrow> (vname \<Rightarrow> bool) \<Rightarrow> tyenv \<Rightarrow> pname \<Rightarrow> imp_prog \<Rightarrow> (call_string, ivl abs_state) analysis_result" where
  "analyse_interval_call_string_result_for k gs \<Gamma> mnm p =
     Analysis_Result
       (fst (cs_call_string_sol_prog k gs \<Gamma> mnm p))
       (\<lambda>v ctx. normalize_point gs
                  (canonicalize_lift (resolved_st_q_is_bot_for (declared_global_vars p))
                    (locals (snd (cs_call_string_sol_prog k gs \<Gamma> mnm p) (Inl (v, ctx))))))"

declare analyse_interval_call_string_result_for_def [code del]

lemma analyse_interval_call_string_result_for_code [code]:
  "analyse_interval_call_string_result_for k gs \<Gamma> mnm p =
     (let sol = cs_call_string_sol_prog k gs \<Gamma> mnm p; gl = declared_global_vars p
...
definition analyse_interval_call_string_result ::
  ...
  "analyse_interval_call_string_result k p =
     analyse_interval_call_string_result_for k (declared_global p) (prog_tyenv p) prog_main_name p"
...
definition cs_call_string_check_projection ::
  ...
  "cs_call_string_check_projection k \<Gamma> mnm p =
     classify_checks_ctx (prog_cfg mnm p)
       (analyse_interval_call_string_result_for k (declared_global p) \<Gamma> mnm p)
...
definition cs_call_string_verdict_report_prog ::
  ...
  "cs_call_string_verdict_report_prog k \<Gamma> mnm p =
...
definition analyse_interval_call_string_report ::
  ...
  "analyse_interval_call_string_report k p =
     cs_call_string_verdict_report_prog k (prog_tyenv p) prog_main_name p"
...
definition cs_call_string_sol_prog_join ::
    "nat \<Rightarrow> (vname \<Rightarrow> bool) \<Rightarrow> tyenv \<Rightarrow> pname \<Rightarrow> imp_prog
       \<Rightarrow> (pp \<times> call_string) set \<times> (pp \<times> call_string + call_string_gk \<Rightarrow> (ivl exec_dg_st lifted, ivl exec_dg_st lifted) dg_state)" where
  "cs_call_string_sol_prog_join k gs \<Gamma> mnm p =
     TD_side_always_join_Interp_solve (cs_call_string_eqs_prog k gs \<Gamma> mnm p)
       (cfg_exit (prog_cfg mnm p), [])"

definition cs_call_string_sol_prog_per_origin ::
    "nat \<Rightarrow> (vname \<Rightarrow> bool) \<Rightarrow> tyenv \<Rightarrow> pname \<Rightarrow> imp_prog
       \<Rightarrow> (pp \<times> call_string) set \<times> (pp \<times> call_string + call_string_gk \<Rightarrow> (ivl exec_dg_st lifted, ivl exec_dg_st lifted) dg_state)" where
  "cs_call_string_sol_prog_per_origin k gs \<Gamma> mnm p =
     TD_side_per_origin_Interp_solve (cs_call_string_eqs_prog k gs \<Gamma> mnm p)
       (cfg_exit (prog_cfg mnm p), [])"

definition analyse_interval_call_string_result_for_join ::
    "nat \<Rightarrow> (vname \<Rightarrow> bool) \<Rightarrow> tyenv \<Rightarrow> pname \<Rightarrow> imp_prog \<Rightarrow> (call_string, ivl abs_state) analysis_result" where
  "analyse_interval_call_string_result_for_join k gs \<Gamma> mnm p =
     Analysis_Result
       (fst (cs_call_string_sol_prog_join k gs \<Gamma> mnm p))
       (\<lambda>v ctx. normalize_point gs
                  (canonicalize_lift (resolved_st_q_is_bot_for (declared_global_vars p))
                    (locals (snd (cs_call_string_sol_prog_join k gs \<Gamma> mnm p) (Inl (v, ctx))))))"
...
lemma analyse_interval_call_string_result_for_join_code [code]:
  "analyse_interval_call_string_result_for_join k gs \<Gamma> mnm p =
     (let sol = cs_call_string_sol_prog_join k gs \<Gamma> mnm p; gl = declared_global_vars p
      in Analysis_Result (fst sol)
           (\<lambda>v ctx. normalize_point gs
                      (canonicalize_lift (resolved_st_q_is_bot_for gl)
                        (locals (snd sol (Inl (v, ctx)))))))"
...
definition analyse_interval_call_string_result_for_per_origin ::
    "nat \<Rightarrow> (vname \<Rightarrow> bool) \<Rightarrow> tyenv \<Rightarrow> pname \<Rightarrow> imp_prog \<Rightarrow> (call_string, ivl abs_state) analysis_result" where
  "analyse_interval_call_string_result_for_per_origin k gs \<Gamma> mnm p =
     Analysis_Result
       (fst (cs_call_string_sol_prog_per_origin k gs \<Gamma> mnm p))
       (\<lambda>v ctx. normalize_point gs
                  (canonicalize_lift (resolved_st_q_is_bot_for (declared_global_vars p))
                    (locals (snd (cs_call_string_sol_prog_per_origin k gs \<Gamma> mnm p) (Inl (v, ctx))))))"
...
lemma analyse_interval_call_string_result_for_per_origin_code [code]:
  "analyse_interval_call_string_result_for_per_origin k gs \<Gamma> mnm p =
     (let sol = cs_call_string_sol_prog_per_origin k gs \<Gamma> mnm p; gl = declared_global_vars p
      in Analysis_Result (fst sol)
           (\<lambda>v ctx. normalize_point gs
                      (canonicalize_lift (resolved_st_q_is_bot_for gl)
                        (locals (snd sol (Inl (v, ctx)))))))"
...
definition cs_call_string_verdict_report_prog_join ::
    "nat \<Rightarrow> tyenv \<Rightarrow> pname \<Rightarrow> imp_prog \<Rightarrow> (pp \<times> exp \<times> contextual_verdict) list" where
  "cs_call_string_verdict_report_prog_join k \<Gamma> mnm p =
     classify_checks_verdicts (prog_cfg mnm p)
       (analyse_interval_call_string_result_for_join k (declared_global p) \<Gamma> mnm p)
       interval_classify_check"

definition cs_call_string_verdict_report_prog_per_origin ::
    "nat \<Rightarrow> tyenv \<Rightarrow> pname \<Rightarrow> imp_prog \<Rightarrow> (pp \<times> exp \<times> contextual_verdict) list" where
  "cs_call_string_verdict_report_prog_per_origin k \<Gamma> mnm p =
     classify_checks_verdicts (prog_cfg mnm p)
       (analyse_interval_call_string_result_for_per_origin k (declared_global p) \<Gamma> mnm p)
       interval_classify_check"

definition analyse_interval_call_string_report_join ::
    "nat \<Rightarrow> imp_prog \<Rightarrow> (pp \<times> exp \<times> contextual_verdict) list" where
  "analyse_interval_call_string_report_join k p =
     cs_call_string_verdict_report_prog_join k (prog_tyenv p) prog_main_name p"

definition analyse_interval_call_string_report_per_origin ::
    "nat \<Rightarrow> imp_prog \<Rightarrow> (pp \<times> exp \<times> contextual_verdict) list" where
  "analyse_interval_call_string_report_per_origin k p =
     cs_call_string_verdict_report_prog_per_origin k (prog_tyenv p) prog_main_name p"

definition cs_call_string_sol_prog_wpo ::
    "nat \<Rightarrow> (vname \<Rightarrow> bool) \<Rightarrow> tyenv \<Rightarrow> pname \<Rightarrow> imp_prog
       \<Rightarrow> (pp \<times> call_string) set \<times> (pp \<times> call_string + call_string_gk \<Rightarrow> (ivl exec_dg_st lifted, ivl exec_dg_st lifted) dg_state)" where
  "cs_call_string_sol_prog_wpo k gs \<Gamma> mnm p =
     TD_side_warrowing_per_origin_Interp_solve (cs_call_string_eqs_prog k gs \<Gamma> mnm p)
       (cfg_exit (prog_cfg mnm p), [])"

definition analyse_interval_call_string_result_for_wpo ::
    "nat \<Rightarrow> (vname \<Rightarrow> bool) \<Rightarrow> tyenv \<Rightarrow> pname \<Rightarrow> imp_prog \<Rightarrow> (call_string, ivl abs_state) analysis_result" where
  "analyse_interval_call_string_result_for_wpo k gs \<Gamma> mnm p =
     Analysis_Result
       (fst (cs_call_string_sol_prog_wpo k gs \<Gamma> mnm p))
       (\<lambda>v ctx. normalize_point gs
                  (canonicalize_lift (resolved_st_q_is_bot_for (declared_global_vars p))
                    (locals (snd (cs_call_string_sol_prog_wpo k gs \<Gamma> mnm p) (Inl (v, ctx))))))"
...
lemma analyse_interval_call_string_result_for_wpo_code [code]:
  "analyse_interval_call_string_result_for_wpo k gs \<Gamma> mnm p =
     (let sol = cs_call_string_sol_prog_wpo k gs \<Gamma> mnm p; gl = declared_global_vars p
      in Analysis_Result (fst sol)
           (\<lambda>v ctx. normalize_point gs
                      (canonicalize_lift (resolved_st_q_is_bot_for gl)
                        (locals (snd sol (Inl (v, ctx)))))))"
...
definition cs_call_string_verdict_report_prog_wpo ::
    "nat \<Rightarrow> tyenv \<Rightarrow> pname \<Rightarrow> imp_prog \<Rightarrow> (pp \<times> exp \<times> contextual_verdict) list" where
  "cs_call_string_verdict_report_prog_wpo k \<Gamma> mnm p =
     classify_checks_verdicts (prog_cfg mnm p)
       (analyse_interval_call_string_result_for_wpo k (declared_global p) \<Gamma> mnm p)
       interval_classify_check"

definition analyse_interval_call_string_report_wpo ::
    "nat \<Rightarrow> imp_prog \<Rightarrow> (pp \<times> exp \<times> contextual_verdict) list" where
  "analyse_interval_call_string_report_wpo k p =
     cs_call_string_verdict_report_prog_wpo k (prog_tyenv p) prog_main_name p"
