...
definition sctx_entry_eqs ::
  ...
  "sctx_entry_eqs gs \<Gamma> is_bot_pred Pi ps mnm main =
     side_cfg_T_eff_keyed_seed_dg_buffered intra_predecessor_addr_list (\<lambda>_. Global)
       ...
       (routed_cmb_g_contribution (sctx_spec gs \<Gamma> is_bot_pred) Global Seed
...
definition sctx_entry_terminates ::
  ...
  "sctx_entry_terminates gs \<Gamma> is_bot_pred Pi ps mnm main =
     TD_side_always_join_Interp.solve_dom TYPE(gk) TYPE((sign exec_dg_st lifted, sign exec_dg_st lifted) dg_state)
       (sctx_entry_eqs gs \<Gamma> is_bot_pred Pi ps mnm main) (cfg_exit (compile_prog Pi ps mnm main), [])"
...
lemma sctx_entry_route_gen_commute:
  "formals_route_lifted_gen (sctx_abs_spec gs \<Gamma>) u ctx (map_lift (fun_of_resolved_st_q_for gs) d) ca
...
interpretation sign_es: routed_domain_exec
  ...
  "formals_route_lifted_gen (sctx_abs_spec gs \<Gamma>)"
...
lemma sctx_entry_pp_st:
  "part_post_solution (sctx_entry_eqs gs \<Gamma> is_bot_pred Pi ps mnm main) (cfg_exit (compile_prog Pi ps mnm main), [])
...
theorem sctx_entry_pp_abs:
  "part_post_solution
     (side_cfg_T_eff_keyed_seed_dg intra_predecessor_addr_list (\<lambda>_. Global)
        (formals_route_lifted_gen (sctx_abs_spec gs \<Gamma>))
        (routed_cmb_g (sctx_abs_spec gs \<Gamma>) Global Seed
           (static_resolve (compile_prog Pi ps mnm main)))
        (routed_extra_g Seed Global)
...
context
  ...
  assumes solves: "sctx_entry_terminates gs \<Gamma> is_bot_pred Pi ps mnm main"
    and exact: "\<And>s. is_bot_pred s = is_bot_state (fun_of_resolved_st_q_for gs s)"
    and entry_cov: "(cfg_entry (compile_prog Pi ps mnm main), []) \<in> fst (sctx_entry_sol gs \<Gamma> is_bot_pred Pi ps mnm main)"
    ...
    and call_fwd_ok: "\<And>u ctx dst pars args p cont.
        ...
        \<Longrightarrow> (FunctionEntry p,
               formals_route_lifted_gen (sctx_abs_spec gs \<Gamma>) u ctx
                 (enter_local (sctx_abs_spec gs \<Gamma>) pars args
...
lemma sctx_entry_fin: "finite (intra (compile_prog Pi ps mnm main))"
  using compile_prog_finite by blast

lemma sctx_entry_finC: "finite (calls (compile_prog Pi ps mnm main))"
  using compile_prog_finite by blast
...
\<close>
interpretation sctx_entry_dg_base: sound_dg_spec "sctx_abs_spec gs \<Gamma>" gamma_dg_base gs \<Gamma>
  unfolding sctx_abs_spec_def
  apply (rule base_dg_spec_sound[OF sign_is_sound_transfer_for is_bot_state_gamma_state_empty])
  sorry

interpretation sctx_entry_routed: entry_state_routed_context "sctx_abs_spec gs \<Gamma>" gs \<Gamma>
    Pi ps mnm main Global
    "map_lift (fun_of_resolved_st_q_for gs) (Bot::sign exec_dg_st lifted)"
    "map_lift (fun_of_resolved_st_q_for gs) (Lifted cinit_sign_st)"
    "map_lift (fun_of_resolved_st_q_for gs) (Bot::sign exec_dg_st lifted)"
    sctx_entry_sigma_abs "fst (sctx_entry_sol gs \<Gamma> is_bot_pred Pi ps mnm main)"
    "(cfg_exit (compile_prog Pi ps mnm main), [])" sctx_entry_sg
    Seed
...
next
  show "part_post_solution
          (side_cfg_T_eff_keyed_seed_dg intra_predecessor_addr_list (\<lambda>_. Global)
             ...
             (map_lift (fun_of_resolved_st_q_for gs) (Lifted cinit_sign_st))
             (map_lift (fun_of_resolved_st_q_for gs) (Bot::sign exec_dg_st lifted)))
          (cfg_exit (compile_prog Pi ps mnm main), []) sctx_entry_sigma_abs
          (fst (sctx_entry_sol gs \<Gamma> is_bot_pred Pi ps mnm main))"
...
next
  fix u a v ctx assume "(u, ctx) \<in> fst (sctx_entry_sol gs \<Gamma> is_bot_pred Pi ps mnm main)" "(u, a, v) \<in> intra (compile_prog Pi ps mnm main)"
...
lemma sctx_entry_locals_ge_s0d:
  "map_lift (fun_of_resolved_st_q_for gs) (Lifted cinit_sign_st)
     \<le> locals (sctx_entry_sigma_abs (Inl (cfg_entry (compile_prog Pi ps mnm main), [])))"
proof -
  have "map_lift (fun_of_resolved_st_q_for gs) (Lifted cinit_sign_st)
      \<le> locals (eq sctx_entry_routed.Gen (cfg_entry (compile_prog Pi ps mnm main), []) sctx_entry_sigma_abs)"
    by (simp add: eq_side_cfg_T_eff_keyed_seed_dg)
       (rule order_trans[OF _ side_acc_dg_ge_acc], simp add: le_supI2)
  also have "\<dots> \<le> locals (sctx_entry_sigma_abs (Inl (cfg_entry (compile_prog Pi ps mnm main), [])))"
...
definition sctx_entry_enterc ::
  ...
  "sctx_entry_enterc = route_enterc_of_sigma (sctx_abs_spec gs \<Gamma>)
     (formals_route_lifted_gen (sctx_abs_spec gs \<Gamma>)) sctx_entry_sigma_abs Global
...
theorem sctx_entry_activation_collect_sound:
  "activation_collect \<Gamma> gs (admiss_exact sctx_entry_enterc) [] (compile_prog Pi ps mnm main) (cinit_stores gs) v ctx
...
proof (rule activation_collect_sound_gen[where sg = sctx_entry_sg and gammaM = gamma_state_lift
  ...
  also have "\<dots> \<subseteq> gamma_dg_base (locals (sctx_entry_sigma_abs (Inl (cfg_entry (compile_prog Pi ps mnm main), []))))
  ...
  also have "\<dots> = gamma_state_lift (sctx_entry_sg (Inl (cfg_entry (compile_prog Pi ps mnm main), [])))"
    unfolding sctx_entry_sg_covered[OF entry_cov] gamma_dg_base_def by (rule refl)
  finally show "s \<in> gamma_state_lift (sctx_entry_sg (Inl (cfg_entry (compile_prog Pi ps mnm main), [])))" .
next
  show "\<And>u a v c s s'. (u, a, v) \<in> intra (compile_prog Pi ps mnm main)
        \<Longrightarrow> s \<in> gamma_state_lift (sctx_entry_sg (Inl (u, c))) \<Longrightarrow> s' \<in> edge_step \<Gamma> a s
...
next
  ...
  assume ce: "(u, CallEdge dst pars args, FunctionEntry p, cont) \<in> calls (compile_prog Pi ps mnm main)"
    ...
    and adm: "admiss_exact sctx_entry_enterc u c (call_enter \<Gamma> gs (CallEdge dst pars args) s) c'"
  show "call_enter \<Gamma> gs (CallEdge dst pars args) s \<in> gamma_state_lift (sctx_entry_sg (Inl (FunctionEntry p, c')))"
...
next
  ...
  assume ce: "(cl, CallEdge dst pars args, FunctionEntry p, cont) \<in> calls (compile_prog Pi ps mnm main)"
    ...
    and ces: "call_enter_store \<Gamma> gs (compile_prog Pi ps mnm main) cl s es"  show "combine_collect \<Gamma> gs dst s t \<in> gamma_state_lift (sctx_entry_sg (Inl (cont, c1)))"
...
definition sctx_entry_eqs_prog ::
    "(vname \<Rightarrow> bool) \<Rightarrow> tyenv \<Rightarrow> pname \<Rightarrow> imp_prog
       \<Rightarrow> (pp \<times> sign list, gk, (sign exec_dg_st lifted, sign exec_dg_st lifted) dg_state) eqsT" where
  "sctx_entry_eqs_prog gs \<Gamma> mnm p =
     sctx_entry_eqs gs \<Gamma> (resolved_st_q_is_bot_for (declared_global_vars p))
       (prog_table p) (prog_procs p) mnm (prog_main p)"

definition sctx_entry_sol_prog ::
    "(vname \<Rightarrow> bool) \<Rightarrow> tyenv \<Rightarrow> pname \<Rightarrow> imp_prog
       \<Rightarrow> (pp \<times> sign list) set \<times> (pp \<times> sign list + gk \<Rightarrow> (sign exec_dg_st lifted, sign exec_dg_st lifted) dg_state)" where
  "sctx_entry_sol_prog gs \<Gamma> mnm p =
     sctx_entry_sol gs \<Gamma> (resolved_st_q_is_bot_for (declared_global_vars p))
       (prog_table p) (prog_procs p) mnm (prog_main p)"

definition sctx_entry_terminates_prog :: "(vname \<Rightarrow> bool) \<Rightarrow> tyenv \<Rightarrow> pname \<Rightarrow> imp_prog \<Rightarrow> bool" where
  "sctx_entry_terminates_prog gs \<Gamma> mnm p =
     sctx_entry_terminates gs \<Gamma> (resolved_st_q_is_bot_for (declared_global_vars p))
       (prog_table p) (prog_procs p) mnm (prog_main p)"
...
lemma sctx_entry_terminates_prog_via_solve_c:
  ...
  shows "sctx_entry_terminates_prog gs \<Gamma> mnm p"
  using assms
...
definition analyse_sign_entry_state_result_for ::
    "(vname \<Rightarrow> bool) \<Rightarrow> tyenv \<Rightarrow> pname \<Rightarrow> imp_prog \<Rightarrow> (sign list, sign abs_state) analysis_result" where
  "analyse_sign_entry_state_result_for gs \<Gamma> mnm p =
     Analysis_Result
       (fst (sctx_entry_sol_prog gs \<Gamma> mnm p))
       (\<lambda>v ctx. normalize_point gs
                  (canonicalize_lift (resolved_st_q_is_bot_for (declared_global_vars p))
                    (locals (snd (sctx_entry_sol_prog gs \<Gamma> mnm p) (Inl (v, ctx))))))"

declare analyse_sign_entry_state_result_for_def [code del]

lemma analyse_sign_entry_state_result_for_code [code]:
  "analyse_sign_entry_state_result_for gs \<Gamma> mnm p =
     (let sol = sctx_entry_sol_prog gs \<Gamma> mnm p; gl = declared_global_vars p
...
definition analyse_sign_entry_state_result :: "imp_prog \<Rightarrow> (sign list, sign abs_state) analysis_result" where
  "analyse_sign_entry_state_result p =
     analyse_sign_entry_state_result_for (declared_global p) (prog_tyenv p) prog_main_name p"
...
definition sctx_entry_check_projection ::
    "tyenv \<Rightarrow> pname \<Rightarrow> imp_prog \<Rightarrow> (pp \<times> exp \<times> (sign list \<times> contextual_verdict) set) list" where
  "sctx_entry_check_projection \<Gamma> mnm p =
     classify_checks_ctx (prog_cfg mnm p)
       (analyse_sign_entry_state_result_for (declared_global p) \<Gamma> mnm p)
       sign_classify_check"
...
definition sctx_entry_verdict_report_prog ::
  ...
  "sctx_entry_verdict_report_prog \<Gamma> mnm p =
...
definition analyse_sign_entry_state_report :: "imp_prog \<Rightarrow> (pp \<times> exp \<times> contextual_verdict) list" where
  "analyse_sign_entry_state_report p = sctx_entry_verdict_report_prog (prog_tyenv p) prog_main_name p"
