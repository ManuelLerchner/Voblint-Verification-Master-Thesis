...
interpretation sign_dg: sound_dg_spec "unit_dg_spec_for gs (sign_tf_for gs)" "gamma_unit gs" gs
...
interpretation sign_dg_privatized:
  sound_dg_spec "unit_dg_spec_placed gs sign_keep_all gs (sign_tf_for gs)" gamma_join gs
...
definition sign_dg_gamma ::
  "(vname \<Rightarrow> bool)
   \<Rightarrow> (pp \<times> unit + unit \<Rightarrow> (sign abs_state, sign abs_state) dg_state)
   \<Rightarrow> pp \<Rightarrow> store set"
where
  "sign_dg_gamma gs \<equiv> sound_dg_spec.dg_gamma (gamma_unit gs)"
...
sublocale sign_dg_api \<subseteq> sound_dg_spec_ltr_for "unit_dg_spec_for gs (sign_tf_for gs)" "gamma_unit gs" gs
  rewrites "sound_dg_spec.dg_gamma (gamma_unit gs) = sign_dg_gamma gs"
       and "sound_dg_spec.dg_gen (unit_dg_spec_for gs (sign_tf_for gs)) = sign_dg_generator gs"
  apply (unfold sound_dg_spec_ltr_for_def)
   apply (rule sound_dg_spec_unit_for[OF sign_is_sound_transfer_for reserved])
...
theorem sign_dg_post_solution_collect_sound:
  ...
  shows "ltr_collect gs g S0 v \<subseteq> sign_dg_gamma gs sigma v"
