...
section \<open>Sign instance of the Goblint-inspired call specification\<close>
...
text \<open>
  ...
  The merge is fixed to \<open>combine_abs\<close>.  Every assumption is discharged
  trivially: \<open>reads_own_slot\<close> and the digest
...
subsection \<open>Soundness from a post-fixpoint alone\<close>
