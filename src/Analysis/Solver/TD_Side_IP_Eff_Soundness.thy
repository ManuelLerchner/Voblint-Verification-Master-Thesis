theory TD_Side_IP_Eff_Soundness
  imports TD_Side_IP_Eff_Pipeline "Voblint_CFG.CFG_Prune"
begin
...
text \<open>
  ...
  side_cfg_T_ip_eff with no reference to the pure side_cfg_T_ip pipeline.  The
  ...
  from TD_Side_IP_Eff_Bounds and the collecting step from
  post_fixpoint_sound_at_ip_eff.
...
lemma dep_aux_side_rhs_fold_ip_eff_acc_indep:
  ...
  shows "dep_aux \<sigma> (side_rhs_fold_ip_eff etf acc1 es cs)
       = dep_aux \<sigma> (side_rhs_fold_ip_eff etf acc2 es cs)"
  by (rule dep_aux_side_rhs_fold_ip_eff_indep[OF edge_static comb_static])

text \<open>The source local of an edge in the list is a dependency of the fold.\<close>

lemma Inl_dep_aux_side_rhs_fold_ip_eff_edge:
  fixes etf :: "(unit, 'a::bounded_semilattice_sup_bot) effectful_domain_transfer"
  assumes edge_dep: "\<And>b w \<sigma>'. Inl w \<in> dep_aux \<sigma>' (apply_etf etf b w)"
  assumes mem: "(u, a) \<in> set es"
  shows "Inl u \<in> dep_aux \<sigma> (side_rhs_fold_ip_eff etf acc es cs)"
  using mem
...
next
  ...
  proof (cases "(w, b) = (u, a)")
    ...
    thus ?thesis unfolding x side_rhs_fold_ip_eff.simps dep_aux_seqcomp by simp
  next
    case False
    have mem': "(u, a) \<in> set es" using Cons.prems x False by auto
    have "Inl u \<in> dep_aux \<sigma> (side_rhs_fold_ip_eff etf
...
lemma dep_aux_side_rhs_fold_ip_eff_nil_sub_es:
  fixes etf :: "(unit, 'a::bounded_semilattice_sup_bot) effectful_domain_transfer"
  assumes edge_static: "\<And>a u. static_deps (apply_etf etf a u)"
      and comb_static: "\<And>cc ex. static_deps (etf_combine etf cc ex)"
  shows "dep_aux \<sigma> (side_rhs_fold_ip_eff etf acc [] cs)
       \<subseteq> dep_aux \<sigma> (side_rhs_fold_ip_eff etf acc es cs)"
proof (induction es arbitrary: acc)
...
next
  ...
  have "dep_aux \<sigma> (side_rhs_fold_ip_eff etf acc [] cs)
        = dep_aux \<sigma> (side_rhs_fold_ip_eff etf ?acc' [] cs)"
    by (rule dep_aux_side_rhs_fold_ip_eff_acc_indep[OF edge_static comb_static])
  also have "... \<subseteq> dep_aux \<sigma> (side_rhs_fold_ip_eff etf ?acc' es cs)"
    by (rule Cons.IH)
  also have "... \<subseteq> dep_aux \<sigma> (side_rhs_fold_ip_eff etf acc (x # es) cs)"
    unfolding x side_rhs_fold_ip_eff.simps dep_aux_seqcomp by auto
  finally show ?case .
qed

lemma Inl_dep_aux_side_rhs_fold_ip_eff_call:
  fixes etf :: "(unit, 'a::bounded_semilattice_sup_bot) effectful_domain_transfer"
  assumes comb_dep: "\<And>c2 e2 \<sigma>'. Inl c2 \<in> dep_aux \<sigma>' (etf_combine etf c2 e2)"
  assumes edge_static: "\<And>a u. static_deps (apply_etf etf a u)"
      and comb_static: "\<And>cc ex. static_deps (etf_combine etf cc ex)"
  assumes mem: "(cc, ex) \<in> set cs"
  shows "Inl cc \<in> dep_aux \<sigma> (side_rhs_fold_ip_eff etf acc es cs)"
proof -
  have nil: "Inl cc \<in> dep_aux \<sigma> (side_rhs_fold_ip_eff etf acc [] cs)"
    using mem
  ...
  next
    ...
    next
      ...
      have "Inl cc \<in> dep_aux \<sigma> (side_rhs_fold_ip_eff etf
  ...
  qed
  show ?thesis
    using nil dep_aux_side_rhs_fold_ip_eff_nil_sub_es[OF edge_static comb_static]
    by blast
qed

lemma Inl_dep_aux_side_rhs_fold_ip_eff_exit:
  fixes etf :: "(unit, 'a::bounded_semilattice_sup_bot) effectful_domain_transfer"
  assumes comb_dep: "\<And>c2 e2 \<sigma>'. Inl e2 \<in> dep_aux \<sigma>' (etf_combine etf c2 e2)"
  assumes edge_static: "\<And>a u. static_deps (apply_etf etf a u)"
      and comb_static: "\<And>cc ex. static_deps (etf_combine etf cc ex)"
  assumes mem: "(cc, ex) \<in> set cs"
  shows "Inl ex \<in> dep_aux \<sigma> (side_rhs_fold_ip_eff etf acc es cs)"
proof -
  have nil: "Inl ex \<in> dep_aux \<sigma> (side_rhs_fold_ip_eff etf acc [] cs)"
    using mem
  proof (induction cs arbitrary: acc)
    case Nil thus ?case by simp
  next
    ...
    next
      case False
      have mem': "(cc, ex) \<in> set cs" using Cons.prems x False by auto
      have "Inl ex \<in> dep_aux \<sigma> (side_rhs_fold_ip_eff etf
              (acc \<squnion> traverse_rhs (etf_combine etf c2 e2) \<sigma>) [] cs)"
        by (rule Cons.IH[OF mem'])
      thus ?thesis unfolding x side_rhs_fold_ip_eff.simps dep_aux_seqcomp by simp
    qed
...
lemma dep_side_rhs_tree_ip_eff_edge:
  fixes etf :: "(unit, 'a::bounded_semilattice_sup_bot) effectful_domain_transfer"
  assumes fin: "finite (edges g)"
  assumes ed: "(u, a, w) \<in> edges g"
  assumes edge_dep: "\<And>b z \<sigma>'. Inl z \<in> dep_aux \<sigma>' (apply_etf etf b z)"
  shows "u \<in> dep\<^sub>L (side_cfg_T_ip_eff g etf bot0 s0) \<sigma> w"
proof -
  have mem: "(u, a) \<in> set (predecessor_list g w)"
    using ed by (simp add: set_predecessor_list[OF fin] predecessors_def)
  have "Inl u \<in> dep_aux \<sigma> (make_side_rhs_tree_ip_eff g etf bot0 s0 w)"
    unfolding dep_aux_make_side_rhs_tree_ip_eff
    by (rule Inl_dep_aux_side_rhs_fold_ip_eff_edge[OF edge_dep mem])
  thus ?thesis unfolding side_cfg_T_ip_eff_def dep\<^sub>L_def dep_def by simp
qed

lemma dep_side_rhs_tree_ip_eff_combine:
  fixes etf :: "(unit, 'a::bounded_semilattice_sup_bot) effectful_domain_transfer"
    and c ex w :: pp
  ...
  shows "c \<in> dep\<^sub>L (side_cfg_T_ip_eff g etf bot0 s0) \<sigma> w
       \<and> ex \<in> dep\<^sub>L (side_cfg_T_ip_eff g etf bot0 s0) \<sigma> w"
proof -
  ...
  have dc: "Inl c \<in> dep_aux \<sigma> (make_side_rhs_tree_ip_eff g etf bot0 s0 w)"
    unfolding dep_aux_make_side_rhs_tree_ip_eff
    by (rule Inl_dep_aux_side_rhs_fold_ip_eff_call[OF comb_dep1 edge_static comb_static mem])
  have de: "Inl ex \<in> dep_aux \<sigma> (make_side_rhs_tree_ip_eff g etf bot0 s0 w)"
    unfolding dep_aux_make_side_rhs_tree_ip_eff
    by (rule Inl_dep_aux_side_rhs_fold_ip_eff_exit[OF comb_dep2 edge_static comb_static mem])
  show ?thesis using dc de unfolding side_cfg_T_ip_eff_def dep\<^sub>L_def dep_def by simp
qed

subsection \<open>Backward IP reachability lands in the effectful solver's dependency cone\<close>

lemma ip_reaches_imp_trans_dep_or_eq_side_eff:
  ...
  shows "w = v0 \<or> w \<in> trans_dep\<^sub>L (side_cfg_T_ip_eff g etf bot0 s0) \<sigma> v0"
proof -
  from reach have "(w, v0) \<in> {(u, z). ip_succ g u z}\<^sup>*"
  ...
  next
    ...
    have ydep: "y \<in> dep\<^sub>L (side_cfg_T_ip_eff g etf bot0 s0) \<sigma> z"
    proof -
      from su consider
      ...
      proof cases
        case e
        thus ?thesis using dep_side_rhs_tree_ip_eff_edge[OF fin _ edge_dep] by blast
      next
        case cc
        thus ?thesis
          using dep_side_rhs_tree_ip_eff_combine[OF finC _ comb_dep1 comb_dep2
    ...
    next
      case False
      have z_in: "z \<in> trans_dep\<^sub>L (side_cfg_T_ip_eff g etf bot0 s0) \<sigma> v0"
...
lemma side_ip_cone_in_vars_eff:
  fixes g :: cfg and etf :: "(unit, 'a::bounded_semilattice_sup_bot) effectful_domain_transfer"
    and bot0 s0 :: "'a abs_state" and \<sigma> :: "pp + unit \<Rightarrow> 'a abs_state"
...
proof -
  ...
  consider (eq) "w = v0"
    | (td) "w \<in> trans_dep\<^sub>L (side_cfg_T_ip_eff g etf bot0 s0) \<sigma> v0"
    using ip_reaches_imp_trans_dep_or_eq_side_eff[OF fin finC edge_dep comb_dep1
  ...
  next
    case td
    have "trans_dep\<^sub>L (side_cfg_T_ip_eff g etf bot0 s0) \<sigma> v0 \<subseteq> vars"
...
lemma restrict_global_s0_le_global_ip_eff:
  fixes etf :: "(unit, 'a::bounded_semilattice_sup_bot) effectful_domain_transfer"
  assumes pp: "part_post_solution (side_cfg_T_ip_eff g etf bot0 s0) x \<sigma> vars"
      and entry_in: "cfg_entry g \<in> vars"
  shows "restrict_global s0 \<le> \<sigma> (Inr ())"
proof -
  from pp entry_in
  have "sides_of_rhs (side_cfg_T_ip_eff g etf bot0 s0 (cfg_entry g)) \<sigma> \<le> \<sigma>" by auto
  hence le: "sides_of_rhs (side_cfg_T_ip_eff g etf bot0 s0 (cfg_entry g)) \<sigma> (Inr ())
             \<le> \<sigma> (Inr ())"
    by (simp add: le_fun_def)
  have e: "sides_of_rhs (side_cfg_T_ip_eff g etf bot0 s0 (cfg_entry g)) \<sigma> (Inr ())
           = sides_of_rhs (side_rhs_fold_ip_eff etf (bot0 \<squnion> restrict_local s0)
  ...
  have "restrict_global s0
        \<le> sides_of_rhs (side_cfg_T_ip_eff g etf bot0 s0 (cfg_entry g)) \<sigma> (Inr ())"
...
\<close>
lemma s0_le_side_env_entry_ip_eff:
  fixes etf :: "(unit, 'a::bounded_semilattice_sup_bot) effectful_domain_transfer"
  assumes pp: "part_post_solution (side_cfg_T_ip_eff g etf bot0 s0) v0 \<sigma> vars"
  assumes entry_in: "cfg_entry g \<in> vars"
  shows "s0 \<le> side_env \<sigma> (cfg_entry g)"
proof -
  have acc_le: "side_acc_ip_eff etf (bot0 \<squnion> restrict_local s0) \<sigma>
                  (predecessor_list g (cfg_entry g))
                  (combine_predecessor_list g (cfg_entry g))
                \<le> \<sigma> (Inl (cfg_entry g))"
    using side_post_solution_le_local_ip_eff[OF pp entry_in] by simp
  have "restrict_local s0 \<le> bot0 \<squnion> restrict_local s0" by simp
  also have "... \<le> side_acc_ip_eff etf (bot0 \<squnion> restrict_local s0) \<sigma>
                     (predecessor_list g (cfg_entry g))
                     (combine_predecessor_list g (cfg_entry g))"
    by (rule side_acc_ip_eff_ge_acc)
  ...
  have rg: "restrict_global s0 \<le> \<sigma> (Inr ())"
    by (rule restrict_global_s0_le_global_ip_eff[OF pp entry_in])
...
theorem side_collect_sound_ip_exit_pruned_eff:
  ...
  assumes pp: "part_post_solution (side_cfg_T_ip_eff g etf bot0 s0)
  ...
  assumes edge_static: "\<And>a u. static_deps (apply_etf etf a u)"
  assumes comb_static: "\<And>cc ex. static_deps (etf_combine etf cc ex)"
  shows "cfg_collect_ip g S (cfg_exit g)
...
proof -
  ...
  proof -
    ...
    have wv: "w \<in> vars"
      by (rule side_ip_cone_in_vars_eff[OF pp fin finC edge_dep comb_dep1 comb_dep2
            edge_static comb_static w_cone])
    show "etf_full (apply_etf etf a u) \<sigma> \<le> side_env \<sigma> w"
      by (rule etf_combined_le_ip_eff[OF pp wv ed_g fin])
  qed
  have combine_le:
  ...
  proof -
    ...
    show "etf_full (etf_combine etf c ex) \<sigma> \<le> side_env \<sigma> ret"
      by (rule etf_combine_combined_le_ip_eff[OF pp rv uce_g finC])
  qed
  have entry_pg: "S \<le> se.gamma_state (side_env \<sigma> (cfg_entry pg))"
    using entry by (simp add: pg_def prune_cfg_def)
  have collect_pg: "cfg_collect_ip pg S (cfg_exit g) \<le> se.gamma_state (side_env \<sigma> (cfg_exit g))"
    by (rule se.post_fixpoint_sound_at_ip_eff[OF entry_pg step_le combine_le order_refl])
...
text \<open>
  ...
  combined env (side_analyse_ip_eff), and over-approximate the IP collecting
...
\<close>
theorem side_analyse_ip_eff_collect_sound_exit_pruned_gen:
  fixes \<Pi> ps main and s0 :: "'a::bounded_semilattice_sup_bot abs_state"
    and S :: "store set"
    and \<gamma> :: "'a \<Rightarrow> int set" and etf :: "(unit, 'a) effectful_domain_transfer"
  assumes se: "sound_effectful_transfer \<gamma> etf"
  assumes mono_eq: "is_mono_eq (side_cfg_T_ip_eff (compile_prog \<Pi> ps main) etf bot s0)"
  assumes mono_sides: "mono_sides (side_cfg_T_ip_eff (compile_prog \<Pi> ps main) etf bot s0)"
  assumes mono_deps: "mono_deps (side_cfg_T_ip_eff (compile_prog \<Pi> ps main) etf bot s0)"
  assumes dom: "side_cfg_ip_solve_dom_eff (compile_prog \<Pi> ps main) etf bot s0
  ...
  shows "cfg_collect_ip (compile_prog \<Pi> ps main) S (cfg_exit (compile_prog \<Pi> ps main))
         \<le> sound_domain.gamma_state \<gamma> (side_analyse_ip_eff \<Pi> ps main etf bot s0
              (cfg_exit (compile_prog \<Pi> ps main)))"
proof -
  interpret se: sound_effectful_transfer \<gamma> etf by (rule se)
  define g where "g = compile_prog \<Pi> ps main"
  define v0 where "v0 = cfg_exit g"
  interpret ip: td_cfg_side_ip_solver_eff g etf bot s0
  ...
  have dom': "side_cfg_ip_solve_dom_eff g etf bot s0 v0"
    using dom unfolding g_def v0_def by simp
  have pp: "part_post_solution (side_cfg_T_ip_eff g etf bot s0) v0 \<sigma> (ip.stabl_at v0)"
  ...
  have entry_le: "s0 \<le> side_env \<sigma> (cfg_entry g)"
    by (rule s0_le_side_env_entry_ip_eff[OF pp entry_in])
  have entry_cov: "S \<le> se.gamma_state (side_env \<sigma> (cfg_entry g))"
    using S_sound se.gamma_state_mono[OF entry_le] by blast
  have collect: "cfg_collect_ip g S (cfg_exit g) \<le> se.gamma_state (side_env \<sigma> (cfg_exit g))"
    by (rule side_collect_sound_ip_exit_pruned_eff[OF se pp[unfolded v0_def] fin finC
          entry_cov edge_dep comb_dep1 comb_dep2 edge_static comb_static])
  have analyse_eq:
    "side_analyse_ip_eff \<Pi> ps main etf bot s0 (cfg_exit g) = side_env \<sigma> (cfg_exit g)"
    unfolding side_analyse_ip_eff_def \<sigma>_def v0_def g_def by simp
...
lemma side_cfg_T_ip_eff_is_mono_eq:
  ...
  apply (rule side_cfg_T_ip_eff_is_mono_eq_gen)
...
lemma side_cfg_T_ip_eff_mono_sides:
  ...
  apply (rule side_cfg_T_ip_eff_mono_sides_gen)
  subgoal for a u s1 s2
...
lemma side_cfg_T_ip_eff_mono_deps:
  ...
  apply (rule side_cfg_T_ip_eff_mono_deps_gen)
  by (simp add: static_deps_def pure_edge_tree_def Let_def)
