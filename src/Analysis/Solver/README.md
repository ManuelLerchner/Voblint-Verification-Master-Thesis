...
| `TD_Side_IP_Tree.thy` | `side_cfg_T` (pure) and `side_cfg_T_eff` (effectful) interprocedural strategy trees; folds `side_acc_ip` / `side_acc_eff`; the `etf_from_tf` bridges |
...
| `TD_Side_IP_Eff_Sound.thy` | `post_fixpoint_sound_at_eff` — a post-fixpoint of the effectful system over-approximates `cfg_collect` (in `sound_effectful_transfer`) |
