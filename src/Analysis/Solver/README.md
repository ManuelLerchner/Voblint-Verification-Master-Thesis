...
records. Unit-global domains use the same effectful record interface as named
global domains; executable transport assumes the record's concrete tree
equations directly. The old pure-transfer adapter layer was retired (see
...
| `TD_Side_CFG.thy` | `restrict_local`, `restrict_global`, `side_env`; unit-global effectful tree constructors (`unit_edge_tree`, `unit_combine_tree`); `trans_dep\<^sub>L` step/trans lemmas |
| `TD_Side_Tree.thy` | `side_cfg_T_eff` effectful interprocedural strategy trees; folds `side_acc_eff` |
| `TD_Side_Eff_Bounds.thy` + `TD_Side_Eff_Soundness.thy` | monotonicity + dependency stability; TD_side preconditions `side_cfg_T_eff_is_mono_eq` / `_mono_sides` / `_mono_deps`, plus unit-global record discharges |
