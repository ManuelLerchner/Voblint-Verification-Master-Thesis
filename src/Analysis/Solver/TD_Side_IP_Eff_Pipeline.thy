theory TD_Side_IP_Eff_Pipeline
  imports TD_Side_IP_Eff_Bounds TD_Side_IP_Eff_Sound TD_Side_IP_Eff_Interface
begin
...
text \<open>
  ...
  pure side_cfg_T_ip / pure bounds / etf_from_tf shim:

    * the TD_side solver interface (td_cfg_side_ip_solver_eff) is discharged from a
...
lemma td_cfg_side_ip_solver_eff_gen:
  fixes g :: cfg
  ...
  shows "td_cfg_side_ip_solver_eff g etf bot0 s0"
proof
  show "is_mono_eq (side_cfg_T_ip_eff g etf bot0 s0)"
    by (rule side_cfg_T_ip_eff_is_mono_eq_gen[OF edge_mono comb_mono])
  show "mono_sides (side_cfg_T_ip_eff g etf bot0 s0)"
    by (rule side_cfg_T_ip_eff_mono_sides_gen[OF edge_sides_mono comb_sides_mono])
  show "mono_deps (side_cfg_T_ip_eff g etf bot0 s0)"
    by (rule side_cfg_T_ip_eff_mono_deps_gen[OF edge_static comb_static])
qed
...
proof -
  ...
  proof -
    ...
    show "etf_full (apply_etf etf a u) \<sigma> \<le> side_env \<sigma> w"
      by (rule etf_combined_le_ip_eff[OF pp edge_cov[OF ed] ed fin])
  qed
  have combine_le:
