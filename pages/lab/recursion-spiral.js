{
  ...
  const SPIRAL_PROGRAMS = {
    grows: {
      ...
      grid: {
        ...
        warrow: { none: "[0,+∞]", cs1: "[0,0] | [1,+∞]", cs2: "[0,0] | [1,1] | [2,+∞]", cs3: "[0,0] | [1,1] | [2,2] | [3,+∞]", entry: NO },
        "warrow-per-origin": { none: "[0,+∞]", cs1: "[0,0] | [1,+∞]", cs2: "[0,0] | [1,1] | [2,+∞]", cs3: "[0,0] | [1,1] | [2,2] | [3,+∞]", entry: NO },
      },
    },
    bounded: {
      ...
      endless: false,
      code: "fun f(x) {\n  __voblint_check(x >= 0);\n  if (x < 10) {\n    f(x + 1);\n  }\n}\n\nfun main() {\n  f(0);\n}",
      grid: Object.fromEntries(["join", "per-origin", "warrow", "warrow-per-origin"].map((g) => [g, {
        none: "[0,10]", cs1: "[0,0] | [1,10]", cs2: "[0,0] | [1,1] | [2,10]", cs3: "[0,0] | [1,1] | [2,2] | [3,10]", entry: "[0,0] | [1,1] | … | [10,10]",
      }])),
    },
  };

  const GLOBALS = [["join", "Join"], ["per-origin", "Join per origin"], ["warrow", "Warrow"], ["warrow-per-origin", "Warrow per origin"]];
  const CONTEXTS = [["none", "none"], ["cs1", "call string k=1"], ["cs2", "k=2"], ["cs3", "k=3"], ["entry", "entry state"]];
  ...
  const TEXT = {
    grows: {
      ...
      warrow: "One context covers every depth. Its entry value keeps growing, so warrowing jumps to `[0, +∞]` instead of following the spiral outward, and `x >= 0` is proved for every turn.",
      ...
      entry: "Entry-state contexts key each call by its entry state, and every depth enters with a new value: `x = 0`, `x = 1`, `x = 2`, … Each turn asks for yet another context, and the solve does not finish, even with warrowing. The growing chips illustrate that mechanism; only the time-out was measured.",
    },
    bounded: {
      ...
      warrow: "Warrow reaches the same `[0, 10]` as Join on this program, so the settings that differ so sharply on the endless spiral agree once the recursion is bounded.",
      cs: "The first `k` depths get exact contexts, and the deeper calls share one context holding `[k, 10]`.",
      entry: "Every depth enters with a different value, but there are only eleven of them, so entry-state contexts finish with one exact context per turn, `[0, 0]` through `[10, 10]`.",
    },
  };
  ...
  for (const figure of document.querySelectorAll(".scene-recursion-spiral")) {
    ...
    /* Narrow at depth 0, widening outward; spacing and sizes adapt to the number of turns. */
    const geometry = (turns) => {
      ...
      return {
        ...
        ry: Math.min(22, gap * 0.4),
        r: Math.min(20, gap * 0.44),
      };
    };
    ...
    const drawSpiral = () => {
      ...
      for (let d = 0; d < cfg.turns; d++) {
        ...
        if (!last || cfg.endless) {
          ...
          turnsLayer.append(svgEl("path", {
            class: `turn-back${fade}`,
            d: `M${CX + rx} ${y} C ${CX + rx} ${y - geo.ry * 1.3}, ${CX - nextRx} ${y + geo.gap - geo.ry * 1.3}, ${CX - nextRx} ${y + geo.gap}`,
          }));
        }

        turnsLayer.append(svgEl("path", { class: `turn-front${fade}`, d: `M${CX - rx} ${y} A ${rx} ${geo.ry} 0 0 0 ${CX + rx} ${y}` }));
        const call = svgEl("g", { class: `call${fade}` });
        call.append(svgEl("circle", { cx: CX, cy: y + geo.ry, r: geo.r }));
        call.append(svgEl("text", { x: CX, y: y + geo.ry + 4 }, `f(${d})`));
        callsLayer.append(call);
      }

      const end = geo.y(cfg.turns) + 6;
      callsLayer.append(svgEl("text", { class: "spiral-more", x: CX, y: end + 16 }, cfg.endless ? "⋮ wider, forever" : "x = 10: no further call"));
    };
    ...
    const band = (from, to, label, cls) => {
      const cfg = SPIRAL_PROGRAMS[program];
      const top = callY(from) - geo.r - 4;
      const bottom = to === Infinity ? geo.y(cfg.turns) - 6 : callY(to) + geo.r + 4;
      bands.append(svgEl("rect", { class: `band ${cls}`, x: CX - geo.r - 12, y: top, width: 2 * geo.r + 24, height: bottom - top, rx: geo.r + 8 }));
      const chip = svgEl("g", { class: `chip ${cls}` });
      const y = (top + Math.min(bottom, geo.y(cfg.turns))) / 2;
    ...
    const setBridge = (from) => {
      ...
      if (from !== null) {
        ...
        bridge.querySelector("path").setAttribute("d", `M832 ${y0} C 890 ${y0 + 60}, 890 ${y1 - 60}, 832 ${y1}`);
    ...
    const drawEndless = (animate) => {
      ...
      const step = () => {
        ...
        timer.hidden = false;
        timer.textContent = n >= turns - 1 ? `voblint: killed after ${SPIRAL_BOUND} s without an answer` : `solving… ${seconds} s`;
        n = n >= turns - 1 ? 0 : n + 1;
      };
      if (!animate) {
        n = turns - 1;
    ...
    const renderTable = () => {
      ...
      const rows = GLOBALS.map(([g, label]) => `<tr><th scope="row">${label}</th>${CONTEXTS.map(([c]) => {
        const value = grid[g][c];
        return value === NO ? `<td class="none">no answer</td>` : `<td><code>${escapeText(value)}</code></td>`;
      }).join("")}</tr>`).join("");
    ...
    const render = () => {
      ...
      programTabs.forEach((tab) => tab.setAttribute("aria-selected", String(tab.dataset.program === program)));
      tabs.forEach((tab) => tab.setAttribute("aria-selected", String(tab.dataset.mode === mode)));
      kBox.hidden = mode !== "cs";
      kButtons.forEach((b) => b.setAttribute("aria-pressed", String(Number(b.dataset.k) === k)));
      ...
      if (value === NO) {
        ...
        band(k, cfg.endless ? Infinity : cfg.turns - 1, `[${csSites(k)}] ↦ x ∈ [${k}, ${cfg.endless ? "+∞" : 10}]`, "shared");
        setBridge(cfg.endless ? k : null);
      } else if (mode === "entry") {
        for (let d = 0; d < cfg.turns; d++) band(d, d, `key x = [${d}, ${d}]`, "own");
        setBridge(null);
      } else {
        band(0, cfg.endless ? Infinity : cfg.turns - 1, `one context: x ∈ [0, ${cfg.endless ? "+∞" : 10}]`, "shared");
    ...
    programTabs.forEach((tab) => tab.addEventListener("click", () => selectProgram(tab.dataset.program)));
    tabs.forEach((tab) => tab.addEventListener("click", () => { mode = tab.dataset.mode; render(); }));
    kButtons.forEach((b) => b.addEventListener("click", () => { k = Number(b.dataset.k); render(); }));
