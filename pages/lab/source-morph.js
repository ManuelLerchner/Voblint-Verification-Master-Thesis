{
  ...
  const SOURCE_MORPH_STEPS = [
    ...
    {
      rule: { name: "compile (Assign x a)", clause: "compile Π p (Assign x a) k n =\n  (Suc n, Statement n, {(Statement n, EA_Assign x a, k)}, {})" },
    ...
    {
      rule: { name: "compile (Check c)", clause: "compile Π p (Check c) k n =\n  (Suc n, Statement n, {(Statement n, EA_Check c, k)}, {})" },
  ...
  for (const figure of document.querySelectorAll(".scene-source-morph")) {
    ...
    const render = (i) => {
      ...
      for (const part of parts) {
        ...
        const pending = !built && part.dataset.pending !== undefined && i >= Number(part.dataset.pending);
