{
  ...
  const PT_NODES = {
    ...
    pp4: { x: 190, y: 360, code: "check(x == 1)", lo: 1, hi: 2, value: "[1, 2]", verdict: "UNKNOWN" },
  ...
  const ptRoute = (x) => x > 0
    ? (x < 3 ? ["pp2", "pp3", "pp4", "pp9"] : ["pp2", "pp3", "pp9"])
    : (x > 5 ? ["pp2", "pp6", "pp7", "pp9"] : ["pp2", "pp6", "pp9"]);
  const ptSlot = (node, v) => ({ x: node.x - ((PT_HIGH - PT_LOW) * PT_DX) / 2 + (v - PT_LOW) * PT_DX, y: node.y + 30 });
  const ptReached = (name, v) => mod(v, 3) === 0 && PT_RUNS.some((n) => 3 * n === v && ptRoute(v).includes(name));
  ...
  for (const figure of document.querySelectorAll(".scene-particles")) {
    ...
    for (const edge of figure.querySelectorAll(".pt-edges [data-from]")) {
      edgePaths[`${edge.dataset.from}>${edge.dataset.to}`] = edge.tagName === "line"
    ...
    const hopPath = (a, b, key) => {
      const path = svgEl("path", { d: `M${a.x} ${a.y} ${key ? edgePaths[key] : ""} L${b.x} ${b.y}` });
    ...
    for (const [name, node] of Object.entries(PT_NODES)) {
      ...
      group.append(svgEl("rect", { class: "pt-label-box", x: node.x - 96, y: node.y - 13, width: 192, height: 26, rx: 7 }));
      group.append(svgEl("text", { class: "pt-label", x: node.x, y: node.y + 5 }, `${name} · ${node.code}`));
      ...
      if (!node.bot) {
        ...
        group.append(svgEl("rect", { class: "pt-band", x: bandLeft, y: y - 10, width: bandRight - bandLeft, height: 20, rx: 10 }));

        if (node.lo === -Infinity) {
          group.append(svgEl("path", { class: "pt-band-tail", d: `M${left - 4} ${y - 6} L${left - 13} ${y} L${left - 4} ${y + 6}` }));
        }
        if (node.hi === Infinity) {
          group.append(svgEl("path", { class: "pt-band-tail", d: `M${right + 4} ${y - 6} L${right + 13} ${y} L${right + 4} ${y + 6}` }));
        }
      }

      group.append(svgEl("text", { class: "pt-value", x: node.x, y: y + 40 }, node.bot ? "x ∈ ⊥: no store" : `x ∈ ${node.value}`));
    ...
    const play = () => {
      ...
      const runs = PT_RUNS.map((n, k) => {
        ...
        const points = [{ x: ptSlot(PT_NODES.pp2, v).x, y: -10 }, ...route.map((name) => ptSlot(PT_NODES[name], v))];
        const hops = points.slice(1).map((b, i) => hopPath(points[i], b, i === 0 ? null : `${route[i - 1]}>${route[i]}`));
        const particle = svgEl("circle", { class: "pt-particle", r: 4.5, cx: points[0].x, cy: points[0].y });
