...
`cfg_path` carries actions (needed for transfer-fn composition); `edges_collect` folds actions over store sets. Compound CFGs compile sub-commands at offset `k > 0`, so sub-paths need shifting through `offset_edges k` the shift is invisible to `edges_collect`. See `src/CFG/CFG_Path.thy` for the predicate + offset infrastructure and the IP collecting layer in `src/CFG/Collecting/` (`CFG_Collect_Edges`, `CFG_Collect`, `CFG_Collect_Adeq`, `CFG_Collect_Unified`; import `CFG_Collect_Adeq` for `cfg_runs_to` or `CFG_Collect_Unified` for the locale).
