...
Views (--view, repeatable, default `sessions files`):
  ...
  kinds     declaration kinds, and statement and structure-command kinds, per session
...
# Commands that add structure rather than a named definition or statement:
# counted per theory next to the declaration kinds.
STRUCTURE_KEYWORDS = [
    "function",
    "inductive_cases",
    "interpretation",
...
BOUNDARY_RE = re.compile(
    r"^[ \t]*(?P<kw>"
    + "|".join(sorted(set(BOUNDARY_KEYWORDS), key=len, reverse=True))
    + r")\b",
    re.MULTILINE,
)
...
def mask_docs(text: str, masked: str) -> str:
    ...
    out = list(masked)
    doc_re = re.compile(
        r"\b(" + "|".join(["text"] + HEADING_KEYWORDS) + r")\s*\\<open>"
    )
    i = 0
    while True:
...
def scan_theory(path: Path) -> TheoryStats:
    ...
    proof_lines_src = mask_docs(
        text, mask_comments_and_strings(text, strings=False)
    ).splitlines()

    raw_lines = text.splitlines()
    code_lines = code_only.splitlines()
    blank = sum(1 for line in raw_lines if not line.strip())
    code = sum(1 for line in code_lines if line.strip())
    ...
    long_lines = (
        0
        if path.stem in GENERATED_THEORIES
        else sum(
            1
            for line in raw_lines
            if symbol_length(line) > MAX_LINE_SYMBOLS
            and not EXEMPT_LINE_RE.search(line)
        )
    )
    ...
    for idx, (start, kw, rest) in enumerate(boundaries):
        ...
        if kw in STRUCTURE_KEYWORDS:
            stats.commands[kw] = stats.commands.get(kw, 0) + 1
        ...
        span = len([line for line in body if line.strip()])
        statement = span
...
def text_row(row, widths, aligns) -> str:
    return "  ".join(
        c.rjust(w) if a == "r" else c.ljust(w)
        for c, w, a in zip(row, widths, aligns, strict=True)
    ).rstrip()
...
def render_text(tables, out) -> None:
    for t in tables:
        ...
        print(text_row(t.columns, widths, aligns), file=out)
        print("  ".join("-" * w for w in widths), file=out)
        for row in cells:
            print(text_row(row, widths, aligns), file=out)
...
def file_table(stats, sort: str, top: int) -> Table:
    ...
    suffix = (
        f" (top {len(shown)} of {len(ordered)})" if top and len(ordered) > top else ""
    )
    return Table(
...
def distribution_table(stats) -> Table:
    ...
    rows.append(
        [
            ...
            len(allxs),
            mean_or_zero(allxs),
            median_or_zero(allxs),
            percentile(allxs, 90),
            percentile(allxs, 99),
            max(allxs, default=0),
            sum(1 for x in allxs if x <= 1),
            sum(1 for x in allxs if x > 20),
        ]
    )
    return Table(
...
def statement_kinds(theory: TheoryStats) -> dict:
    """Theorem-like statements by keyword, then the structural commands."""
    counts = {}
    for proof in theory.proofs:
        counts[proof.kind] = counts.get(proof.kind, 0) + 1
    return {**counts, **theory.commands}


def kind_table(
    stats, title="Declaration kinds per session", counts_of=lambda s: s.decls
) -> Table:
    by_session = {}
    kinds = set()
    for s in stats:
        acc = by_session.setdefault(s.session, {})
        for k, n in counts_of(s).items():
            acc[k] = acc.get(k, 0) + n
            kinds.add(k)
    ordered_kinds = sorted(
        kinds, key=lambda k: -sum(a.get(k, 0) for a in by_session.values())
    )
    rows = [
        [session] + [acc.get(k, 0) for k in ordered_kinds] + [sum(acc.values())]
        for session, acc in by_session.items()
    ]
    rows.sort(key=lambda r: -r[-1])
    rows.append(
        ["TOTAL"] + [sum(r[i] for r in rows) for i in range(1, len(ordered_kinds) + 2)]
    )
    return Table(title, ["session"] + ordered_kinds + ["total"], rows)
...
def main() -> int:
    ...
    builders = {
        ...
        "kinds": lambda: [
            kind_table(stats),
            kind_table(
                stats, "Statement and structure kinds per session", statement_kinds
            ),
        ],
        "style": lambda: style_table(stats),
    }
    tables = []
    for view in (v for v in VIEWS if v in views):
        built = builders[view]()
