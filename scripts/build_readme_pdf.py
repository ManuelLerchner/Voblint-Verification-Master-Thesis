...
def build(output):
    if not shutil.which("pandoc"):
        raise SystemExit(
            "pandoc is required for the README pages; install it and retry."
        )
    isabelle = os.environ.get("ISABELLE", "isabelle")
    home = subprocess.check_output(
        [isabelle, "getenv", "-b", "ISABELLE_HOME"], text=True
    ).strip()
    ...
    command = [
        ...
        "--standalone",
        "--lua-filter",
        str(REPO / "scripts/pdf_readme.lua"),
        "--pdf-engine=lualatex",
        no_highlight,
        "--include-in-header",
        str(REPO / "document/readme-header.tex"),
        "--resource-path",
        str(REPO),
        "-V",
        ...
        "papersize:a4",
        "-V",
        "monofont:IsabelleDejaVuSansMono.ttf",
        "-V",
        f"monofontoptions:Path={fonts}/,Scale=0.8",
        "-V",
        "colorlinks:true",
    ]
